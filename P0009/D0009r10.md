...
<b>26.7.�.1 Overview [mdspan.extents.syn]</b>
...
<b>26.7.�.2 Overview [mdspan.extents.overview]</b>
...
<b>26.7.�.3 Constructors and assignment [mdspan.extents.cons]</b>
...
<b>26.7.�.2.1 `layout_left::mapping` constructors [mdspan.layout_left]</b>
...
* [2]{.pnum} *Effects:* Initializes `extents_` with `other.extents()`.

```c++
index_type required_span_size() const noexcept;
```

* [3]{.pnum} *Returns:* The product of `extents().extent(r)` for all `r` in the range $[0,$ `extents().rank()` $)$.


```
template<class... Indices, 
         class... IdxSeq = make_index_sequence<index_type, sizeof...(Indices)>>
  index_type offset_()(Indices... i) const;
```
...
* [5]{.pnum} *Expects:* 0 <= `array{i...}[r]` < `extents().extent(r)` for all `r` in the range $[0,$ `extents().rank()` $)$.
...
* [8]{.pnum} *Returns:* `1` if `r` is zero, otherwise, the product of `extents().extent(k)` for all `k` in the range $[0,$ `r` $)$.
