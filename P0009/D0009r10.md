...
[1]{.pnum} `subspan` creates a `basic_mdspan`
...
[2]{.pnum} The `SliceSpecifier` template argument(s)
   ...
   determine the subset of `src` that the `basic_mdspan` returned by `subspan` views.
...
[4]{.pnum} Define `map_rank` as an `array<ptrdiff_t,src.rank()>`
...
[5]{.pnum} Let `first` and `last` be exposition-only variables of type `array<ptrdiff_t,src.rank()>`.
...
+ otherwise, if `is_convertible_v<`$S_r$`,pair<ptrdiff_t,ptrdiff_t>>`,
...
+ otherwise, if `is_convertible_v<`$S_r$`,all_type>`,
