...
[4]{.pnum} The $r^{th}$ interval of the multidimensional index space represented 
by an `extents` is $[0,E_r)$ if $E_r$ is a static extent or
`[0, dynamic_extents_[dynamic_intex(`_r_`)])`.
...
* *Constraints:*
    * `sizeof...(Extents)` equals `sizeof...(OtherExtents)`.
    * For all `r` where
      `static_extent(r) != dynamic_extent` and
      `other.static_extent(r) != dynamic_extent` are both `true`, then
