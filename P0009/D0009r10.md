...
struct layout_left {
  ...
  public:
    ...
    Extents extents() const noexcept { return extents_; }

    constexpr index_type required_span_size() const noexcept;

    template<class... Indices>
      index_type operator()(Indices... is) const noexcept 
      { return offset(is...); }
    ...
    index_type stride(size_t rank) const noexcept;
  ...
  private:
    // @_exposition only_@
    template<class... Indices,
             class... IdxSeq = make_index_sequence<index_type, sizeof...(Indices)>>
      index_type offset_(Indices... is) const noexcept
...
* [4]{.pnum} *Returns:* The product of `extents().extent(r)` for all `r` in the range $[0,$ `Extents::rank()` $)$.


```
template<class... Indices, 
         class... IdxSeq = make_index_sequence<index_type, sizeof...(Indices)>>
  index_type offset_()(Indices... i) const noexcept;
```

* [5]{.pnum} *Constraints:*
    
    * [5.1]{.pnum} `sizeof...(Indices) == Extents::rank()` is `true`, and
    
    * [5.1]{.pnum} `(is_convertible_v<Indices, index_type> && ...)` is `true`.

* [6]{.pnum} *Expects:* 0 <= `array{i...}[r]` < `extents().extent(r)` for all `r` in the range $[0,$ `Extents::rank()` $)$.

* [7]{.pnum} *Effects:* Let `P...` be the parameter pack such that
              `is_same_v<IdxSeq, integer_sequence<index_type, P...>>` is `true`.
...
* [8]{.pnum} *Returns:* `1` if `r` equals zero, otherwise, the product of `extents().extent(k)` for all `k` in the range $[0,$ `r` $)$.
...
* [9]{.pnum} *Effects:* Equivalent to: `return extents() == other.extents();`.
...
* [5]{.pnum} *Constraints:*
    ...
    * [5.2]{.pnum} `(is_convertible_v<Indices, index_type> && ...)` is `true`.
...
* [8]{.pnum} *Returns:* `1` if `r` equals `Extents::rank()-1`, otherwise, the product of `extents().extent(k)` for all `k` in the range $[$ `r` $,$ `Extents::rank()` $)$.
...
typename Extents::index_type required_span_size() const noexcept;
```
* [6]{.pnum} *Returns:* The maximum of `extents().extent(r) * stride(r)` for all `r` in the range $[0,$ `Extents::rank()` $)$.
...
* [7]{.pnum} *Constraints:* `sizeof...(Indices) == Extents::rank()` is `true`,
              and  `(is_convertible_v<Indices, index_type> && ...)` is `true`.

* [8]{.pnum} *Expects:* 0 <= `array{i...}[r]` < `extents().extent(r)` for all `r` in the range $[0,$ `Extents::rank()` $)$.

* [9]{.pnum} *Effects:* Let `P...` be the parameter pack such that
              `is_same_v<IdxSeq, integer_sequence<index_type, P...>>` is `true`.
              Equivalent to: `return Extents::rank() > 0 ? (i*stride(P()) + ...) : 0;` 
...
[10]{.pnum} Let $P$ be a permutation of the integers $0, ...,$ `Extents::rank()-1` and let $p_i$ be the $i^{th}$ element of $P$.

* [11]{.pnum}*Returns:*
    
    * [11.1]{.pnum} `true` if `Extents::ranks()` is zero.

    * [11.2]{.pnum} Otherwise, `true` if there is a permutation $P$ such that
...
* [12]{.pnum} *Effects:* Equivalent to: `return extents() == other.extents();`.
...
[3]{.pnum} _[Note:_ The type of `reference` need not be `element_type&`.
  The type of `pointer` need not be `element_type*`. _— end note]_

[4]{.pnum} _[Note:_ The constructor of the object representing the contiguous set of objects
...
<tr>
  ...
  <td>Type through which the range of elements are accessed.
      <br/> `A::pointer` meets the requirements of _Cpp17DefaultConstructible_, _Cpp17CopyConstructible_, and _Cpp17CopyAssignable_.
  </td>
</tr>
<tr>
  <td>`A::reference`</td>
...
<br/>
<b>22.7.�.4 `basic_mdspan` observers of the mapping [mdspan.basic.obs]</b>
