...
[1]{.pnum} The class template `extents` defines a _multidimensional index space_,
  a Cartesian product of integer intervals.  Each dimension, or extent, in `extents`
  can be represented by a half-open interval $[0, S_i)$, where $S_i$ is the
  upper bound for the $i^{th}$ dimension. The _rank_ of an `extents` is the number 
  of intervals it represents.
...
<b>26.7.�.2 General [mdspan.extents.general]</b>

[1]{.pnum} For all $i$ in the range $0 \leq i < rank$ let $E_i$ be the value of 
the $i^{th}$ parameter in the template parameter pack `Extents`. Each $E_i$ shall 
either be a non-negative integer or `dynamic_extent`, otherwise, the program is ill-formed.

[2]{.pnum} If $E_i$ is equal to `dynamic_extent` it is a _dynamic extent_, otherwise it is a _static extent_.
For each $E_i$ equivalent to `dynamic_extent`,  
the upper bound of the interval is stored in the exposition only arry `dynamic_extents_` at
`dynamic_extents_[dynamic_index(@_i_@)]`.

[3]{.pnum} A _multidimensional index_ is an element within the a multidimensional
index space and can be represented as a pack of integer types `idx...`.  The pack 
`idx...` refers to an element within the domain of an `extents` `e` if both the following are true:

  * [2.1]{.pnum} `sizeof...(idx)` is equal to `e.rank()`, and
  * [2.2]{.pnum} For all $i$ in the range $0 \leq i < rank$, the $i^{th}$ value
  of `idx` is greater than or equal to $0$ and is less than `e.extent(i)`.
...
* *Returns*: If `i <= sizeof...(Extents)` is `true`, returns the number of arguments 
...
* *Effects:* For each `r` in the range `[0, rank())` where `static_extent(r) == dynamic_extent` is `true`,
             initializes `dynamic_extent[dynamic_index(r)]` with `other.extent(r)`.
* *Throws:* Nothing.
...
```
* *Returns:*
    * If `static_extent(r) == dynamic_extent` is `true`, then `dynamic_extents_[dynamic_index(r)]`;
