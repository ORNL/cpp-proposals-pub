...
- replace uses of SizeType with auto... or equivalent, as all we need is convertibility to size_type
- basic_mdspan is trivially default constructible if access is trivially default constructible

### Open issues
- Rename all_type and all
- Rename decay
- CTAD
...
public:
  ...
  template<class... SizeType>
    constexpr extents(SizeType...) noexcept;
  template<class SizeType>
...
```c++
template<class... SizeType>
  constexpr extents(SizeType... dynamic) noexcept;
```
...
* [4]{.pnum} *Constraints:*
    ...
    * [4.2]{.pnum} `sizeof...(SizeType) == rank_dynamic()` is `true`.
...
public:
  ...
  template<class... SizeType>
    explicit constexpr basic_mdspan(pointer p, SizeType... dynamic_extents);
  ...
  template<class... SizeType>
    constexpr reference operator()(SizeType... indices) const noexcept;
  template<class SizeType, size_t N>
...
```c++
template<class... SizeType>
  explicit constexpr basic_mdspan(pointer ptr, SizeType... dynamic_extents);
```

* [1]{.pnum} *Constraints:*

    + [1.1]{.pnum} `(is_convertible_v<SizeType, size_type> && ...)` is `true`,
...
```c++
template<class... SizeType>
  constexpr reference operator()(SizeType... indices) const;
```

* [4]{.pnum} *Constraints:*
    + [4.1]{.pnum} `(is_convertible_v<SizeType, size_type> && ...)` is `true`, and

    + [4.2]{.pnum} `sizeof...(SizeType) == rank()` is `true`.
...
// Create Subspan
auto a_sub = subspan(a,1,pair<int,int>(4,6),pair<int,int>(1,6));
