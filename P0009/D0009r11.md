...
- Change all the sizes from ptrdiff_t to size_t
- Changed comparisons to just operator== = default

### Changes I made we didn't discuss:
- Made size_type in layout_* non-exposition because it is returned

### TODO
- Rebase against current working draft
...
[2]{.pnum} $E_r$ is a _dynamic extent_ if it is equal to `dynamic_extent`, otherwise $E_r$ is a _static extent_.
...
[3]{.pnum} If $E_r$ is a dynamic extent, let $D_r$ be the value of `dynamic_extents_[dynamic_index(`_r_`)]`.
...
* [4]{.pnum} *Returns*: If `i <= sizeof...(Extents)` is `true`, returns the number of arguments 
...
* [4]{.pnum} *Constraints:*
    ...
    * [4.1]{.pnum} `(is_convertible_v<IndexType, size_type> && ...)` is `true`, and
...
* [6]{.pnum} *Constraints:* `is_convertible_v<IndexType, size_type>` is `true`.
...
struct layout_left {
  ...
  public:
    ...
    template<class... Indices>
      size_type operator()(Indices...) const noexcept; 
    ...
    size_type stride(size_t) const noexcept;
...
```c++
template<class... Indices> 
  size_type operator()(Indices... i) const noexcept;
```
...
```c++
size_type stride(size_t r) const;
```
...
```c++
size_type stride(size_t r) const noexcept;
```
...
struct layout_stride {
  ...
  public:
    ...
    constexpr Extents extents() const noexcept { return extents_; }
    array<typename size_type, Extents::rank()> strides() const noexcept
    { return strides_; }
...
* [1]{.pnum} *Constraints:*
    ...
    + [1.1]{.pnum} `(is_convertible_v<IndexType, size_type> && ...)` is `true`,
...
* [4]{.pnum} *Constraints:*
    ...
    + [4.1]{.pnum} `is_convertible_v<IndexType, size_type>` is `true`,
...
* [8]{.pnum} *Constraints:*
    ...
    + [8.1]{.pnum} `is_convertible_v<IndexType, size_type>` is `true`, and
...
such that `map_rank[j]` equals `-1` if `is_convertible_v<`$S_j$`,size_t>` is `true`, or else
`map_rank[j]` equals the number of $S_k$ with $k < j$ such that
`is_convertible_v<`$S_k$`,pair<size_t,size_t>> || is_convertible_v<`$S_k$`,all_type>` is `true`.
...
* For all `k` in the range `[0, src.rank())`,
  `is_convertible_v<`$S_k$`,size_t> || is_convertible_v<`$S_k$`,pair<size_t,size_t>> || is_convertible_v<`$S_k$`,all_type>` is `true`.
...
* `sub.rank()` equals the number of $k$
  such that `is_convertible_v<`$S_k$`,pair<size_t,size_t>> || is_convertible_v<`$S_k$`,all_type>` is `true`.
