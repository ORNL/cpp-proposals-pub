...
- Improve description and design discussion
...
It also proposes adding the `submdspan` function
that returns an `mdspan` that "slices" (views a subset of) an existing `mdspan`.

The `mdspan` class template has two customization policies:
the *layout mapping* and the *accessor*.
The layout mapping specifies the formula, and properties of the formula,
for mapping a multidimensional index to an elemetn of the array.
The accessor allows modification of how elements are accessed,
for example via a proxy reference type like `atomic_ref`.
For a discussion of the idea of accessors and several examples,
see (Keryell and Falcou 2016).
...
Each of the <math>R</math> indices is in a bounded range whose lower bound is zero.
The offset index ranges over a subset of a bounded contiguous index range
whose lower bound is zero, and whose upper bound is the product of the <math>R</math> index bounds.
...
featured arrays with one, two, or three dimensions (Backus 1956, pp. 10-11).
...
C++ comes with the following approaches
that could be used to represent multidimensional arrays:

1. "native" arrays where all the dimensions are compile-time constants,
  like `int[3][4][5]`;

2. pointer-of-pointers(-of-pointers...) data structures,
  like `int***`;
...
If a multidimensional array has any dimensions
...
the array's run-time dimensions.
...
Users cannot arbitrarily mix compile-time and run-time dimensions.

Approach (3) can mix `vector` and `array` to represent dimensions
known at run time resp. compile time.
However, a `vector` of `vector`s (of `vector`s) is a container
with many separate allocations.
It cannot represent a view of contiguous memory.
...
Furthermore, `gslice` offers no efficient way to address a single element.
The `gslice` constructor takes strides and lengths as `valarray`s
and is meant for array-based computation.

## Mixing compile-time and run-time dimensions
...
such as unrolling and pre-computing of offsets.
These can significantly improve the generated code.
Not storing dimensions at run time
may help conserve registers and stack space.

In many fields, some dimensions are naturally known at compile time.
For many physics and engineering algorithms,
some dimensions are dictated by fundamental properties of the physical world
or the discretization scheme.
For example, the position of a particle in space
requires a three-dimensional array, since physical space has three dimensions.
At the same time, other dimensions are only known at run time,
such as the number of particles in a simulation.
A natural data structure for storing a list of particles
would thus be a two-dimensional array,
where the run-time dimension is the number of particles
and the compile-time dimension is three.
...
This would make a three-dimensional array with two compile-time dimensions
...
* `layout_stride`: a generalization of the two layouts above,
  which stores a separate stride for each dimension.
...
The resulting decomposition may involve matrices with non-unit strides in both dimensions.
...
Our proposal comes with a single `mdspan` accessor, `default_accessor`,
that works much like `vector::operator[]`.
