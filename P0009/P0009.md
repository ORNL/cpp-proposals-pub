...
- Move wording change to [mdspan.layoutstride.ctor] (new precondition `other.stride(r) > 0` on `mapping(const StridedLayoutMapping&)` constructor) from P2553 to P0009
...
[5]{.pnum} *Preconditions:*
   ...
   * [5.1]{.pnum} `StridedLayoutMapping` meets the layout mapping requirements, and

   * [5.2]{.pnum} _`OFFSET`_`(other) == 0` is `true`.

[6]{.pnum} *Effects:* Equivalent to `mapping(other.extents(), other.strides())`.
