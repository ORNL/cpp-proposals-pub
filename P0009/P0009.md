...
<br/>
<b>22.7.�.5 Template alias `dextents`
...
```
* [4]{.pnum} *Result:* `MP`
* [5]{.pnum} *Preconditions:* `is_same_v<MP::template mapping<extents_type>, M>` is `true`.
...
m(i...)
```
* [10]{.pnum} *Preconditions:* $0\:\le$ `array{i...}[r]` $\lt$ `extents().extent(r)`
       for all `r` in the range $[0,$ `Extents::rank()` $)$.

* [11]{.pnum} *Result:* `E::size_type`

* [12]{.pnum} *Value:* A value in the range of $[0,$ `required_span_size()` $)$.
...
```
* [13]{.pnum} *Result:* `E::size_type`
* [14]{.pnum} *Value:* If the multidimensional index space that `e` defines is empty, then zero,
...
```
* [15]{.pnum} *Result:* `bool`
* [16]{.pnum} *Value:* `true` if for every `i...` and `j...` where `(i != j || ...)` is `true`, `m(i...) != m(j...)` is `true`. Otherwise `false`.

```c++
m.is_contiguous()
```
* [17]{.pnum} *Result:* `bool`
* [18]{.pnum} *Value:* `true` if for all $k$ in the range $[0,$ `m.required_span_size()` $)$ there exists an `i...` such that `m(i...)` equals $k$. Otherwise `false`.

```c++
m.is_strided()
```
* [19]{.pnum} *Result:* `bool`
* [20]{.pnum} *Value:* `true` if for every `r` there exists an integer `sr`
...
```
* [21]{.pnum} *Result:* `bool`
* [22]{.pnum} *Value:* `true` if `m.is_unique()` is `true` for all objects `m` of type `M`. Otherwise, `false`.
...
```
* [25]{.pnum} *Result:* `bool`
* [26]{.pnum} *Value:* `true` if `m.is_strided()` is `true` for all objects `m` of type `M`. Otherwise, `false`.

```c++
m.stride(r)
```
* [27]{.pnum} *Preconditions:* `m.is_strided()` is `true`.
* [28]{.pnum} *Result:* `E::size_type`
* [29]{.pnum} *Value:* `sr` as defined in `m.is_strided()` above.


*[30]{.pnum}. A type `MP` meets the *layout mapping policy* requirements if the following types are well formed and have the specified sematnics.

```c++
typename MP::mapping_type
```
*[31]{.pnum} *Result:* `M`
*[32]{.pnum} *Preconditions:* `M` meets the layout mapping requirements.
