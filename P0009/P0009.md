...
of rank equal to `sizeof...(Extents)`. In section 22.7, `extents` will be used synonymously with multidimensional index space.
...
`MP::template mapping<E>` is valid and denotes a type `X` that meets the layout mapping requirements ([mdspan.layout.reqmts]), 
and for which `typename X::layout_type` denotes `MP` and `typename X::extents_type` denotes `E`.
...
<b>22.7.�.5.1 `layout_stride` Exposition-only helpers  [mdspan.layout.stride.expo]</b>
...
<b>22.7.�.5.2 `layout_stride` Mapping Constructors  [mdspan.layout.stride.ctors]</b>
