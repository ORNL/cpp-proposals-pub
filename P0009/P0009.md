...
[11]{.pnum} *Returns:* Product of `e.extent(k)` for all `k` in the range $[0, $ `i` $) $.
...
constexpr size_t @_rev-prod-of-extents_@(extents<Extents>, size_t) noexcept; // @_exposition only_@
```
[12]{.pnum} *Returns:* Product of `e.extent(k)` for all `k` in the range $[$ `i` $, $ `e.rank()` $) $.
...
[9]{.pnum} *Returns:* If `extents_type::rank() > 0` is `true`, _`fwd-prod-of-extents`_(extents(),extents_type::rank())`. Otherwise, 1.
...
[15]{.pnum} *Returns:* `1` if `i` equals zero, otherwise, _`fwd-prod-of-extents`_(extents(), i)`.
...
[14]{.pnum} *Returns:* `1` if `i` equals `extents_type::rank()-1`, otherwise, _`rev-prod-of-extents`_(extents(), i+1)`.
