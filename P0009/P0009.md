...
The _size_ of a multidimensional index space is the product of $I_e-I_b$ for each dimension if its rank is greater than $0$, and $1$ otherwise.
...
[3]{.pnum} Let $E_r$ be the $r^{th}$ element of `Extents`.
           $E_r$ is a _dynamic extent_ if it is equal to `dynamic_extent`, otherwise $E_r$ is a _static extent_.
           Let $D_r$ be the value of _`dynamic-extents`_`[`_`dynamic-index`_`(`_r_`)]` if $E_r$ is a dynamic extent, otherwise $E_r$.

[6]{.pnum} The $r^{th}$ interval of the multidimensional index space represented by an `extents` object is $[0, D_r)$.

<b>22.7.�.3 Exposition only helpers [mdspan.extents.helpers]</b>
...
[11]{.pnum} *Precondition:* `i <= rank()` is `true`

[12]{.pnum} *Returns:* If `i > 0` is `true` product of `extent(k)` for all `k` in the range $[0,$ `i` $)$, otherwise `1`.


```c++
constexpr size_t @_rev-prod-of-extents_@(rank_type i) const noexcept; // @_exposition only_@
```
[13]{.pnum} *Precondition:* `i < rank()` is `true` 

[14]{.pnum} *Returns:* If `i+1 < rank()` is `true` product of `extent(k)` for all `k` in the range $[$ `i` $,$ `e.rank()` $)$, otherwise `1`.


<b>22.7.�.3 Constructors [mdspan.extents.ctors]</b>
...
[2]{.pnum} *Preconditions:* `other.extent(r)` equals $E_r$ for each $r$ for which $E_r$ is a _static extent_.
...
[6]{.pnum} Let `exts_arr` be `array<size_type,sizeof...(SizeTypes)>{static_cast<size_type>(move(exts))...}`
...
[9]{.pnum} *Constraints:*
   ...
   * [9.3]{.pnum} `N==rank_dynamic() || N==rank()` is `true`.
...
```
[12]{.pnum} *Constraints:* `(is_convertible_v<Integrals, size_type> && ...)` is `true`.

[13]{.pnum} *Remarks:* The deduced type is `dextents<sizeof...(Integrals)>`.

<br/>
<b>22.7.�.3 Observers of the multidimensional index space [mdspan.extents.obs]</b>

```c++
constexpr size_type static_extent(rank_type i) const noexcept;
```
...
<br/>
<b>22.7.�.4 `extents` comparison operators [mdspan.extents.cmp]</b>
...
<br/>
<b>22.7.�.5 Template alias `dextents` [mdspan.extents.dextents]</b>
...
[1]{.pnum} In subclause 22.7.�.1 and subclause 22.7.�.2
   ...
   * [1.6]{.pnum} `i` and `j` are packs of (possibly const) integers which are multidimensional indices into `e` ([mdspan.terms]).

   * [1.7]{.pnum} `r` is a (possibly const) rank index of `E`.

   * [1.8]{.pnum} `dr` is a pack of (possibly const) integers for which `sizeof...(dr) == e.rank()` is `true`,
...
[1]{.pnum} A type `M` meets the *layout mapping* requirements if `M` meets the requirements of *Cpp17CopyConstructible*, *Cpp17CopyAssignable*, *Cpp17EqualityComparable*, `is_nothrow_move_constructible_v<M>` is `true`, `is_nothrow_move_assignable_v<M>` is `true`, and the following types and expressions are well-formed and have the specified semantics.
...
```
[6]{.pnum} *Result:* A type `MP` which meets the layout mapping policy requirements ([mdspan.layoutpolicy.reqmts]) and for which 
                     `is_same_v<typename MP::template mapping<typename M::extents_type>, M>` is `true`.
...
[13]{.pnum} *Returns:* If the multidimensional index space that `e` defines has size $0$, then `0`,
...
[15]{.pnum} *Returns:* `true` if for every `i` and `j` where `(i != j || ...)` is `true`, `m(i...) != m(j...)` is `true`. Otherwise `false`.
...
[17]{.pnum} *Returns:* `true` if for all $k$ in the range $[0,$ `m.required_span_size()` $)$ there exists an `i` such that `m(i...)` equals $k$. Otherwise `false`.
...
[19]{.pnum} *Returns:* `true` if for every `r` there exists an integer `sr`
...
[21]{.pnum} *Returns:* `true` if `m.is_unique()` is `true` for all objects `m` of type `M`. Otherwise, `false`.
...
[24]{.pnum} *Returns:* `true` if `m.is_contiguous()` is `true` for all objects `m` of type `M`. Otherwise, `false`.
...
[27]{.pnum} *Returns:* `true` if `m.is_strided()` is `true` for all objects `m` of type `M`. Otherwise, `false`.
...
[31]{.pnum} *Returns:* `sr` as defined in `m.is_strided()` above.
...
```
[2]{.pnum} *Result:* A type `X` that meets the layout mapping requirements ([mdspan.layout.reqmts]), and for which both `is_same_v<typename M::layout_type,MP>` and `is_same<typename M::extents_type,E>` are `true`.
...
<b>22.7.�.3.1 `layout_left` Mapping Constructors  [mdspan.layout.left.ctors]</b>

```c++
constexpr mapping(const extents_type& e) noexcept;
```

[1]{.pnum} *Preconditions:* The size of `e` is a representable value of `size_type` ([basic.fundamental]).
...
<b>22.7.�.3.1 `layout_left` Mapping Observers  [mdspan.layout.left.obs]</b>

```c++
constexpr size_type required_span_size() const noexcept;
```

[9]{.pnum} *Returns:* `extents().`_`fwd-prod-of-extents`_`(extents_type::rank())`.
...
[15]{.pnum} *Returns:*  `extents().`_`fwd-prod-of-extents`_`(i)`.
...
<b>22.7.�.4.1 `layout_right` Mapping Constructors  [mdspan.layout.right.ctors]</b>
...
[7]{.pnum} *Preconditions:*
   ...
   * [7.1]{.pnum} if `extents_type::rank() > 0` then for all `r` in the range $[0,$ `extents_type::rank()`$)$, `other.stride(r)` equals
                  `extents().`_`rev-prod-of-extents`_`(r+1)`.

[8]{.pnum} *Effects:* Direct-non-list-initializes $extents\_$ with `other.extents()`.

<b>22.7.�.4.2 `layout_right` Mapping Observers  [mdspan.layout.right.obs]</b>
...
[15]{.pnum} *Returns:*  `extents().`_`rev-prod-of-extents`_`(i+1)`
...
[5]{.pnum} Let _`REQUIRED-SPAN-SIZE`_`(e, strides)` be:

   * [9.1]{.pnum} $1$, if `e.rank()==0` is `true`, otherwise

   * [9.2]{.pnum} $0$, if size of `e` is $0$, otherwise

   * [9.3]{.pnum} $1$ plus the sum of products of `(e.extent(r) - 1)` and `strides[r]` for all `r` in the range $[0,$ `e.rank()` $)$.

<b>22.7.�.5.1 `layout_stride` Mapping Constructors  [mdspan.layout.stride.ctors]</b>
...
[2]{.pnum} *Constraints:*
   ...
   * [2.2]{.pnum} `is_nothrow_constructible_v<size_type, const SizeType&>` is `true`, and

   * [2.3]{.pnum} `N == extents_type::rank()` is `true`.

[3]{.pnum} *Preconditions:*

   * [3.1]{.pnum} _`REQUIRED-SPAN-SIZE`_`(e,s)` is a representable value of `size_type` ([basic.fundamental]).
...
[4]{.pnum} *Effects:* Direct-non-list-initializes $extents\_$ with `e`, and for all $d$ in the range 
                      $[0,$ `rank()`$)$, direct-non-list-initializes _`strides_`_`[d]` with `s[d]`.

```c++
template<class StridedLayoutMapping>
  explicit(!is_convertible_v<typename StridedLayoutMapping::extents_type,extents_type>)
...
[6]{.pnum} *Precondition:* 
   ...
   * [6.1]{.pnum} `StridedLayoutMapping` meets the layout mapping requirements and 

   * [6.2]{.pnum} `other.required_span_size()` is a representable value of `size_type` ([basic.fundamental]).

[7]{.pnum} *Postcondition:* `*this == other` is `true`.

<b>22.7.�.5.2 `layout_stride` Mapping Observers  [mdspan.layout.stride.obs]</b>

```c++
constexpr size_type required_span_size() const noexcept;
```

[9]{.pnum} *Returns:* _`REQUIRED-SPAN-SIZE`_`(extents(),`_`strides_`_`)`.
...
<b>22.7.�.1 Accessor policy requirements [mdspan.accessor.reqmts]</b>

[1]{.pnum} A type `A` meets the accessor policy requirements if `A` meets the requirements of *Cpp17CopyConstructible*, *Cpp17CopyAssignable*, `is_nothrow_move_constructible_v<A>` is `true`, `is_nothrow_move_assignable_v<A>` is `true` and the following types and expressions are well-formed and have the specified semantics.
...
```
[10]{.pnum} *Result:* A type `O` such that:
...
[13]{.pnum} *Returns:* An object which provides access to the `i`-th element in the range of elements that starts at `p`.
...
[15]{.pnum} *Postconditions:* Let `a_sub` be `A::offset_policy(a)`, `p_sub` be `a.offset(p,i)`, and `n` have a value such that $[$`p`,`n`$)$ is the accessible range of `a`, then 
   ...
   * [15.2]{.pnum} `a_sub.access(p_sub,j)` provides acces to the same element as `a.access(p,i+j)`, for every `j` in the range $[$`0`,`n-i`$)$. 
...
[6]{.pnum} `ElementType` is required to be a complete object type that is neither an abstract class type nor an array type.
           ...
           `AccessorPolicy` shall meet the accessor policy requirements. If `is_same_v<typename AccessorPolicy::element_type, ElementType>` equals `false`, then the program is ill-formed.
...
<b>22.7.�.1 `mdspan` Constructors  [mdspan.mdspan.ctors]</b>
...
[3]{.pnum} *Effects:*
   ...
   * [3.1]{.pnum} Direct-non-list-initializes _`ptr_`_ with `move(p)`, and

   * [3.2]{.pnum} Direct-non-list-initializes _`map_`_ with `Extents(static_cast<size_type>(move(exts))...)`.
...
[6]{.pnum} *Effects:*
   ...
   * [6.1]{.pnum} Direct-non-list-initializes _`ptr_`_ with `move(p)`, and
...
[9]{.pnum} *Effects:*
   ...
   * [9.1]{.pnum} Direct-non-list-initializes _`ptr_`_ with `move(p)`, and
...
[12]{.pnum} *Effects:*
   ...
   * [12.1]{.pnum} Direct-non-list-initializes _`ptr_`_ with `move(p)`, and
...
[14]{.pnum}*Effects:*
   ...
   * [14.1]{.pnum} Direct-non-list-initializes _`ptr_`_ with `move(p)`,
...
[17]{.pnum} *Preconditions:* 
   ...
   * [17.2]{.pnum} $[$_`ptr_`_,_`map_`_`.required_span_size()`$)$ is an accesible range of _`acc_`_ for values of _`ptr_`_, _`map_`_ and _`acc_`_ after the invocation of this constructor.
...
```c++
  ...
  !is_convertible_v<const OtherAccessorPolicy&, AccessorPolicy>
```
...
[2]{.pnum} Let `I` be `static_cast<size_type>(move(indices))`.
...
constexpr size_type size() const;
```
[7]{.pnum} *Precondition:* The size of `extents()` is a representable value of `size_type` ([basic.fundamental]).

[7]{.pnum} *Returns:* `extents().`_`fwd-prod-of-extents`_`(rank())`.
...
[6]{.pnum} *Mandates:* For each rank index `r` of `src.extents()` only one of the following is `true`:
  `is_convertible_v<`$S_k$`,size_t>`, `is_convertible_v<`$S_k$`,tuple<size_t,size_t>>`, `is_convertible_v<`$S_k$`,full_extent_t>`.
...
[7]{.pnum} *Constraints:*
   ...
   * [7.3]{.pnum} `LayoutPolicy` is `layout_left`, `layout_right`, `layout_stride`,
   or an implementation-defined layout mapping policy type.
...
[8]{.pnum} *Preconditions:* For each rank index `r` of `src.extents()`,
     `0 <= `_`first`_`[r] && `_`first`_`[r] <= `_`last`_`[r] && `_`last`_`[r] <= src.extent(r)` is `true`.

[9]{.pnum} *Effects:*

   * [9.1]{.pnum} Direct-non-list-initializes `sub.`_`acc_`_ with `src.accessor()`.

   * [9.2]{.pnum} Direct-non-list-initializes `sub.`_`ptr_`_ with `src.accessor().offset(src.data(),apply(src.mapping(),`_`first`_`))`.
...
[10]{.pnum} *Postconditions:*
   ...
   * [10.2]{.pnum} Let `j` be a multidimensional index into `sub.extents()`, let `J` be `array{static_cast<size_t>(j)...}`, let `I` be `array<size_t,decltype(src)::rank()>` such that
...
<i>[Note:</i> Example of `submdspan` use:
See code below
<i>- end note]</i>
