...
[1]{.pnum} *Returns:* true if `lhs.rank()` equals `rhs.rank()` and `lhs.extents(r)` equals `rhs.extents(r)`  for every rank index `r`, otherwise `false`.
...
[15]{.pnum} *Constraints:*
   ...
   * [15.1]{.pnum} `(is_convertible_v<Integrals, size_type> && ...)` is `true`.

   * [15.2]{.pnum} `ElementType` is required to be a complete object type that is neither an abstract class type nor an array type.
...
[17]{.pnum} *Constraints:*
   ...
   * [17.1]{.pnum} `(is_convertible_v<const SizeType&, size_type> && ...)` is `true`.

   * [17.2]{.pnum} `ElementType` is required to be a complete object type that is neither an abstract class type nor an array type.

[18]{.pnum} *Remarks:* The deduced type is `mdspan<ElementType, dextents<N>>`.
...
[19]{.pnum} *Constraints:* `ElementType` is required to be a complete object type that is neither an abstract class type nor an array type.

[20]{.pnum} *Remarks:* The deduced type is `mdspan<ElementType, extents<ExtentsPack...>>`.
...
[21]{.pnum} *Constraints:*
   ...
   * [21.1]{.pnum} `ElementType` is required to be a complete object type that is neither an abstract class type nor an array type.

   * [21.2]{.pnum} `MappingType` meets the requirements of layout mapping.

[22]{.pnum} *Remarks:* The deduced type is `mdspan<ElementType, typename MappingType::extents_type, typename MappingType::layout_type>`.
...
[23]{.pnum} *Constraints:*
   ...
   * [23.1]{.pnum} `ElementType` is required to be a complete object type that is neither an abstract class type nor an array type.

   * [23.2]{.pnum} `MappingType` meets the requirements of layout mapping.

   * [23.3]{.pnum} `AccessorType` meets the requirements of accessor policy.

[24]{.pnum} *Remarks:* The deduced type is `mdspan<ElementType, typename MappingType::extents_type, typename MappingType::layout_type, AccessorType>`.
...
[2]{.pnum} *Preconditions:* _`acc_`_`.access(`_`ptr_`_`, `_`map_`_`(std::move(indices)...))` is well defined.

[3]{.pnum} *Effects:* Equivalent to: `return `_`acc_`_`.access(`_`ptr_`_`, `_`map_`_`(std::move(indices)...));`.
...
[4]{.pnum} *Constraints:*
   ...
   * [4.1]{.pnum} `is_convertible_v<const SizeType&, size_type>` is `true`, and
   
   * [4.2]{.pnum} `is_nothrow_constructible_v<size_type, const SizeType&>` is `true`, and

   * [4.3]{.pnum} `rank() == N` is `true`.

[5]{.pnum} *Effects:* Let `P...` be the parameter pack such that
            `is_same_v<make_index_sequence<sizeof...(Indices)>, index_sequence<P...>>` is `true`.
