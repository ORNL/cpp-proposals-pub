...
[1]{.pnum} An accessor policy defines types and operations by which a reference to a single object is created from an abstract data handle to a number of such objects and an index.
A range of indicies $[0, N)$ is an *accessible range* of a given data handle and an accessor, if for each `i` in the range the accessor policies `access` function produces a valid reference to an object.
...
[12]{.pnum} *Returns:* `q` such that for `b` being `A::offset_policy(a)`, and any integer `n` for which $[$`0`, `n`$)$ is an accessible range of `p` and `a`:
   ...
   * [13.1]{.pnum} $[$`0`, `n-i`$)$ is an accessible range of `q` and `b`, and

   * [13.2]{.pnum} `b.access(q, j)` provides access to the same element as `a.access(p, i + j)`, for every `j` in the range $[$`0`, `n-i`$)$.
