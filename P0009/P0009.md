...
public:
  ...
  static constexpr size_t max_dyn_extent_idx() noexcept;
...
[6]{.pnum} Let $r_max_dyn$ be the maximum value $r$ such that $E_r# is a _dynamic extent_ if any $E_r$ is a _dynamic extent_,
           otherwise let $r_max_dyn$ be `sizeof()...Extents`.
...
```c++
constexpr size_t dynamic_index_rev(size_t i) noexcept; // @_exposition only_@

* [8]{.pnum} *Returns*: The minimum value of $r$ such that `dynamic_index(r+1)` is $i+1$.
...
* [5]{.pnum} *Constraints:*
    ...
    * [5.1]{.pnum} `((is_convertible_v<SizeTypes, size_type> || is_same_v<SizeType, full_extent_t>) && ...)` is `true`, and
    
    * [5.2]{.pnum} `sizeof...(SizeTypes) > max_dyn_ext_idx() || dynamic_rank() == 0` is `true`.

* [5]{.pnum} *Preconditions:* If $E_r$ is a _static extent_ and $r$ is smaller than `sizeof()...SizeTypes)` and
             `is_same_v<decltype(std::get<r,std::tuple<SizeTypes...>>(std::tuple{exts...})),full_extent_t>` is `false`, than
             $E_r$ equals `std::get<r,std::tuple<SizeTypes...>>(std::tuple{exts...})`.

* [6]{.pnum} *Effects:*  For all $r$ in the range `[0, rank_dynamic())`, initializes `dynamic_extents_[r]`
             with `std::array<size_type,sizeof()...(SizeTypes)>{exts...}[dynamic_index_rev(r)]`.

```c++
template<class SizeType, size_t N>
constexpr extents(const array<SizeType, N>& args) noexcept;
```

* [7]{.pnum} *Constraints:* `is_convertible_v<SizeType, size_type>` is `true`.

* [5]{.pnum} *Preconditions:* If $E_r$ is a _static extent_ and $r$ is smaller than $N$, than
             $E_r$ equals `args[r]`.

* [6]{.pnum} *Effects:*  For all $r$ in the range `[0, rank_dynamic())`, initializes `dynamic_extents_[r]`
             with `args[dynamic_index_rev(r)]`.
...
```c++
constexpr static size_t max_dyn_ext_idx() noexcept;
```c++

* [5]{.pnum} *Returns:* $r_max_dyn$ if `dynamic_rank() > 0` is `true`, otherwise `rank()`.
...
* [1]{.pnum} *Constraints:*
    ...
    + [1.1]{.pnum} `((is_convertible_v<SizeTypes, size_type> || is_same_v<SizeTypes, full_extent_t>) && ...)` is `true`,
