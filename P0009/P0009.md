...
of rank equal to `sizeof...(Extents)`. In subclause 22.7, `extents` will be used synonymously with multidimensional index space.
...
<b>22.7.�.3 Constructors [mdspan.extents.ctor]</b>
...
<b>22.7.�.1 Layout mapping policy general [mdspan.layout.general]</b>

[1]{.pnum} In subclause 22.7.�.2 and subclause 22.7.�.3
   ...
   * [1.3]{.pnum} `i` and `j` are packs of (possibly const) integers which are multidimensional indices in `m.extents()` ([mdspan.terms]).
     <i>[Note:</i> The type of each element in multidimensional indicies can be a different integer type. <i>— end note]</i>
...
[2]{.pnum} In subclauses from [mdspan.layout.reqmts] to [mdspan.layout.stride] let _`is-mapping-of`_ be the variable template defined as follows
...
```
[3]{.pnum} *Result:* `typename M::extents_type::rank_type`.
...
[7]{.pnum} *Returns:* A value less then `numeric_limits<typename M::size_type>::max()`.
...
[11]{.pnum} *Returns:* If the size of the multidimensional index space `m.extents()` is $0$, then `0`,
...
[22]{.pnum} *Returns:* `true` only if `m.is_unique()` is `true` for all possible objects `m` of type `M`.
...
[24]{.pnum} *Returns:* `true` only if `m.is_contiguous()` is `true` for all possible objects `m` of type `M`.
     <i>[Note:</i> A mapping may return `false` even if the above condition is met.
...
[26]{.pnum} *Returns:* `true` only if `m.is_strided()` is `true` for all possible objects `m` of type `M`.
     <i>[Note:</i> A mapping may return `false` even if the above condition is met.
                   For certain layout mappings it may not be feasible to determine whether every instance is strided.<i>— end note]</i>


<b>22.7.�.3 Layout policy requirements [mdspan.layoutpolicy.reqmts]</b>
...
<br/>
<b>22.7.�.4 Class template `layout_left::mapping` [mdspan.layout.left]</b>

<br/>
<b>22.7.�.4.1 `layout_left::mapping` overview [mdspan.layout.left.overview]</b>

[1]{.pnum} `layout_left` provides a layout mapping where the left most extent has stride 1 and strides increase left-to-right as the product of extents.
...
class layout_left::mapping {
  public:
    ...
    // [mdspan.layout.left.ctor], layout_left Mapping Constructors
    ...
    static constexpr bool is_unique() const noexcept { return true; }
    static constexpr bool is_contiguous() const noexcept { return true; }
    static constexpr bool is_strided() const noexcept { return true; }
...
[3]{.pnum} `layout_left` meets the requirements of layout mapping policy.

[4]{.pnum} `layout_left` is a trivially copyable type.  `layout_left::mapping<E>` is a trivially copyable type for each `E`.



<b>22.7.�.4.2 `layout_left::mapping` constructors  [mdspan.layout.left.ctor]</b>
...
<b>22.7.�.4.3 `layout_left::mapping` observers  [mdspan.layout.left.obs]</b>
...
<br/>
<b>22.7.�.5 Class template `layout_right::mapping` [mdspan.layout.right]</b>

<br/>
<b>22.7.�.5.1 `layout_right::mapping` overview [mdspan.layout.right.overview]</b>

[1]{.pnum} `layout_right` provides a layout mapping where the right most extent is stride 1 and strides increase right-to-left as the product of extents.
...
class layout_right::mapping {
  public:
    ...
    // [mdspan.layout.right.ctor], layout_right Mapping Constructors
    constexpr mapping() noexcept = default;
    constexpr mapping(const mapping&) noexcept = default;
    constexpr mapping(const extents_type&) noexcept;
    template<class OtherExtents>
...
[3]{.pnum} `layout_right` meets the requirements of layout mapping policy.

[4]{.pnum} `layout_right` is a trivially copyable type.  `layout_right::mapping<E>` is a trivially copyable type for each `E`.


<b>22.7.�.5.2 `layout_right::mapping` constructors  [mdspan.layout.right.ctor]</b>
...
<b>22.7.�.5.3 `layout_right::mapping` observers  [mdspan.layout.right.obs]</b>
...
<br/>
<b>22.7.�.6 Class template `layout_stride::mapping` [mdspan.layout.stride]</b>

<br/>
<b>22.7.�.6.1 `layout_stride::mapping` overview [mdspan.layout.stride.overview]</b>
...
class layout_stride::mapping {
  ...
  private:
    static constexpr rank_type rank = extents_type::rank();

  public:
    // [mdspan.layout.stride.ctor], layout_stride Mapping Constructors
    constexpr mapping() noexcept = default;
    constexpr mapping(const mapping&) noexcept = default;
    template<class SizeType>
    constexpr mapping(const extents_type&,
                      const array<SizeType, rank>&) noexcept;
    ...
    constexpr const extents_type& extents() const noexcept { return @_extents\__@; }
    constexpr array<size_type, rank> strides() const noexcept
    { return @_strides\__@; }
  ...
  private:
    extents_type @_extents\__@{}; // @_exposition only_@
    array<size_type, rank> @_strides\__@{}; // @_exposition only_@
};
}
```
...
[3]{.pnum} `layout_stride` meets the requirements of layout mapping policy.

[4]{.pnum} `layout_stride` is a trivially copyable type.  `layout_stride::mapping<E>` is a trivially copyable type for each `E`.


<b>22.7.�.6.2 `layout_stride::mapping` exposition-only helpers  [mdspan.layout.stride.expo]</b>

[5]{.pnum} Let _`REQUIRED-SPAN-SIZE`_`(e, strides)` be:

   * [5.1]{.pnum} $1$, if `e.rank()==0` is `true`, otherwise

   * [5.2]{.pnum} $0$, if size of the multidimensional index space `e` is $0$, otherwise

   * [5.3]{.pnum} $1$ plus the sum of products of `(e.extent(r) - 1)` and `strides[r]` for all `r` in the range $[0,$ `e.rank()` $)$.

[6]{.pnum} Let _`OFFSET`_`(m)` be `m(z...)` for a pack of integers `z`, that is a multidimensional index into `m.extents()` and each element of `z` equals `0`.

<b>22.7.�.6.3 `layout_stride::mapping` constructors  [mdspan.layout.stride.ctor]</b>

```c++
template<class SizeType>
constexpr mapping(const extents_type& e, array<SizeType, rank> s) noexcept;
```
[1]{.pnum} Let $P$ be a permutation of the integers $0, ...,$ `rank-1` and let $p_i$ be the $i^{th}$ element of $P$.
...
[3]{.pnum} *Preconditions:*
   ...
   * [3.1]{.pnum} _`REQUIRED-SPAN-SIZE`_`(e, s)` is a representable value of type `size_type` ([basic.fundamental]).

   * [3.1]{.pnum}`s[i] > 0` is `true` for all `i` in the range $[0,$ `rank` $)$.

   * [3.2]{.pnum} If `rank` is greater than zero, then there exists a permutation $P$ 
     such that `s[` $p_i$ `] >= s[` $p_{i-1}$ `] * e.extent(` $p_{i-1}$ `)` is `true`
     for all $i$ in the range $[1,$ `rank` $)$. 
...
[5]{.pnum} *Constraints:*
   ...
   * [5.5]{.pnum} if `rank > 0` then `is_same_v<decltype(other.stride(0)), typename StridedLayoutMapping::size_type>` is `true`.
...
[8]{.pnum} *Remarks:* The condition in the `explicit` is:
...
<b>22.7.�.6.4 `layout_stride::mapping` mapping observers  [mdspan.layout.stride.obs]</b>
...
[2]{.pnum} *Constraints:*
   ...
   * [2.1]{.pnum} `sizeof...(Indices) == rank` is `true`, and
...
[5]{.pnum} Let $P$ be a permutation of the integers $0, ...,$ `rank-1` and let $p_i$ be the $i^{th}$ element of $P$.

[6]{.pnum}*Returns:*

   * [6.1]{.pnum} `true` if `rank` is zero.
...
[7]{.pnum} *Constraints:* `rank == OtherMapping::extents_type::rank()` is `true`, and
            `OtherMapping::is_always_strided()` is `true`.

[8]{.pnum} *Preconditions:* `OtherMapping` meets layout mapping requirements.

[9]{.pnum} *Returns:* If `x.extents() == y.extents()` is `true`, _`OFFSET`_`(y) == 0` is `true`, and 
...
<br/>
<b>22.7.�.1 Accessor Policy [mdspan.accessor.general]</b>
...
<br/>
<b>22.7.�.2 Accessor policy requirements [mdspan.accessor.reqmts]</b>
...
[2]{.pnum} A type `A` meets the accessor policy requirements if
   ...
   * [2.3]{.pnum} `is_nothrow_move_assignable_v<A>` is `true`, and 
...
[11]{.pnum} *Returns:* `p_sub` such that for `a_sub` being `A::offset_policy(a)`, and an integer `n` such that $[$`p`, `n`$)$ is the accessible range of `a`:
...
<br/>
<b>22.7.�.3 Class template `default_accessor` [mdspan.accessor.default]</b>
...
<b>22.7.�.4 `default_accessor` members [mdspan.accessor.members]</b>
...
<b>22.7.�.2 `mdspan` constructors  [mdspan.mdspan.ctor]</b>
...
<br/>
<b>22.7.�.3 `mdspan` members [mdspan.mdspan.members]</b>
