...
[9]{.pnum} *Effects:*
   ...
   * [9.2]{.pnum} If _`first`_`[r] < src.extent(r)` for all rank index `r` of `src.extents()`, then
                  direct-non-list-initializes `sub.`_`ptr_`_ with `src.accessor().offset(src.data(), apply(src.mapping(), `_`first`_`))`.
                  Otherwise, direct-non-list-initializes `sub.`_`ptr_`_ with `src.accessor().offset(src.data(), src.mapping().required_span_size())`.
                  <i>[Note:</i> The condition protects against applying invalid indicies to the source mapping in cases such as `submdspan(a, tuple{a.extent(0), a.extent(0)})`, where
                      _`first`_`[0] ==` _`last`_`[0] == src.extent(0)`. <i>- end note]</i> 
