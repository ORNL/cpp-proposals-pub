...
class layout_stride::mapping {
  ...
  public:
    ...
    constexpr mapping(const extents_type&,
                      const span<SizeType, @_rank\__@>&) noexcept;
    template<class SizeType>
    constexpr mapping(const extents_type&,
    ...
    constexpr const extents_type& extents() const noexcept { return @_extents\__@; }
    constexpr const span<size_type, @_rank\__@>& strides() const noexcept
    { return @_strides\__@; }
...
```c++
template<class SizeType>
constexpr mapping(const extents_type& e, const span<SizeType, @_rank\__@>& s) noexcept;
template<class SizeType>
constexpr mapping(const extents_type& e, const array<SizeType, @_rank\__@>& s) noexcept;
```
[1]{.pnum} Let $P$ be a permutation of the integers $0, ...,$ _`rank_`_`-1` and let $p_i$ be the $i^{th}$ element of $P$.
...
[3]{.pnum} *Preconditions:*
   ...
   * [3.3]{.pnum} If _`rank_`_ is greater than zero, then there exists a permutation $P$ 
...
[4]{.pnum} *Effects:* Direct-non-list-initializes $extents\_$ with `e`, and for all $d$ in the range 
                      $[0,$ _`rank_`_$)$, direct-non-list-initializes _`strides_`_`[d]` with `s[d]`.
...
[5]{.pnum} Let $P$ be a permutation of the integers $0, ...,$ _`rank_`_`-1` and let $p_i$ be the $i^{th}$ element of $P$.

[6]{.pnum}*Returns:*

   * [6.1]{.pnum} `true` if _`rank_`_ is zero.
   ...
   * [6.2]{.pnum} Otherwise, `true` if there is a permutation $P$ such that
     ...
     for $i$ in the range $[1,$ _`rank_`_ $)$.
