...
[2]{.pnum} A range of indicies $[0, N)$ is an *accessible range* of a given data handle and an accessor, if for each `i` in the range the accessor policies `access` function produces a valid reference to an object.
...
typename A::pointer
```
[3]{.pnum} *Result:* A type that models `semiregular`, and for which `is_nothrow_move_constructible_v<A::pointer>` is `true`, `is_nothrow_move_assignable_v<A::pointer>` is `true`, and `is_nothrow_swappable_v<A::pointer>` is `true`.
