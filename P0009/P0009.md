...
[13]{.pnum} *Returns:* `true` if for every `i` and `j` where `(i != j || ...)` is `true`, `m(i...) != m(j...)` is `true`. Otherwise `false`.
...
[15]{.pnum} *Returns:* `true` if for all $k$ in the range $[0,$ `m.required_span_size()` $)$ there exists an `i` such that `m(i...)` equals $k$. Otherwise `false`.
...
[17]{.pnum} *Returns:* `true` if for every rank index $r$ of `m.extents()` there exists an integer $s_r$
...
[22]{.pnum} *Returns:* `true` if `m.is_unique()` is `true` for all objects `m` of type `M`. Otherwise, `false`.
...
[26]{.pnum} *Returns:* `true` if `m.is_strided()` is `true` for all objects `m` of type `M`. Otherwise, `false`.
