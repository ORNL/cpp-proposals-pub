...
* [7]{.pnum} *Constraints:*
    ...
    + [7.1]{.pnum} `is_constructible_v<mapping_type, Extents>` is `true`, and

    + [7.2]{.pnum} `is_default_constructible_v<accessor_type>` is `true`.

* [8]{.pnum} *Effects:* Equivalent to: `mdspan(p, mapping_type(ext))`.
...
* [11]{.pnum} *Effects:*
    ...
    + [11.1]{.pnum} Initializes `ptr_` with `p`, and

    + [11.2]{.pnum} Initializes `map_` with `m`.
...
* [13]{.pnum}*Effects:*
    ...
    + [13.1]{.pnum} Initializes `ptr_` with `p`,

    + [13.2]{.pnum} Initializes `map_` with `m`, and

    + [13.3]{.pnum} Initializes `acc_` with `a`.

* [14]{.pnum} *Throws:* Nothing.
...
* [15]{.pnum} *Constraints:*
    ...
    + [15.1]{.pnum} `is_convertible_v<OtherLayoutPolicy::template mapping<OtherExtents>, mapping_type` is `true`;

    + [15.2]{.pnum} `is_convertible_v<OtherAccessor, Accessor>` is `true`;

    + [15.3]{.pnum} `is_convertible_v<OtherAccessor::pointer, pointer>` is `true`;

    + [15.4]{.pnum} `is_convertible_v<OtherExtents, extents_type>` is `true`; and

    + [15.5]{.pnum} For all `r` in the range `[0, rank())`,
...
* [16]{.pnum} *Preconditions:* For all `r` in the range `[0, rank())`,
       if `other.static_extent(r) == dynamic_extent || static_extent(r) == dynamic_extent` is `true`,
       then `other.extent(r) == extent(r)` is `true`.

* [17]{.pnum} *Effects:*

    + [17.1]{.pnum} Initializes `ptr_` with `other.ptr_`,

    + [17.2]{.pnum} initializes `map_` with `other.map_`, and

    + [17.3]{.pnum} initializes `acc_` with `other.acc_`.

* [18]{.pnum} *Throws:* Nothing.
...
* [19]{.pnum} *Constraints:*
    ...
    + [19.1]{.pnum} `is_assignable_v<mapping_type, OtherLayoutPolicy::template mapping<OtherExtents>>` is `true`;

    + [19.2]{.pnum} `is_assignable_v<Accessor, OtherAccessor>` is `true`;

    + [19.3]{.pnum} `is_assignable_v<pointer, OtherAccessor::pointer>` is `true`;

    + [19.4]{.pnum} `OtherExtents::rank() == rank()` is `true`; and

    + [19.5]{.pnum} For all `r` in the range `[0, rank())`, 
       if `other.static_extent(r) != dynamic_extent && static_extent(r) != dynamic_extent` is `true`,
       then `other.static_extent(r) == static_extent(r)` is `true`.

* [20]{.pnum} *Preconditions:* For all `r` in the range `[0, rank())`,
   if `other.static_extent(r) == dynamic_extent || static_extent(r) == dynamic_extent` is `true`,
   then `other.extent(r) == extent(r)` is `true`.

* [21]{.pnum} *Effects:*

    + [21.1]{.pnum} Assigns `other.ptr_` to `ptr_`,

    + [21.2]{.pnum} Assigns `other.map_` to `map_`, and

    + [21.3]{.pnum} Assigns `other.acc_` to `acc_`.

* [22]{.pnum} *Throws:* Nothing.
...
* [5]{.pnum} *Constraints:*
    ...
    + [5.1]{.pnum} `is_convertible_v<SizeType, size_type>` is `true`, and

    + [5.2]{.pnum} `rank() == N` is `true`.

* [6]{.pnum} *Effects:* Equivalent to: `return apply(*this, indices);`.
...
* [8]{.pnum} *Returns:* Product of `extent(r)` for all `r` in the range `[0, Extents::rank())`.

```c++
constexpr size_type unique_size() const;
```

* [9]{.pnum} *Returns:* The number of unique elements in the codomain.
