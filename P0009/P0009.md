...
[2]{.pnum} Let _`OFFSET`_`(m)` be:
   ...
   * [1.1]{.pnum} `m()`, if `e.rank() == 0` is `true`, otherwise
   
   * [1.2]{.pnum} $0$, if the size of the multidimensional index space `e` is $0$, otherwise

   * [1.3]{.pnum} `m(z...)` for a pack of integers `z` that is a multidimensional index into `m.extents()` and each element of `z` equals `0`.
...
[5]{.pnum} *Constraints:*
   ...
   * [5.1]{.pnum} _`layout-mapping-alike`_`<StridedLayoutMapping>` is satisfied.

   * [5.2]{.pnum} `is_constructible_v<extents_type, typename StridedLayoutMapping::extents_type>` is `true`.

   * [5.3]{.pnum} `StridedLayoutMapping::is_always_unique()` is `true`.

   * [5.4]{.pnum} `StridedLayoutMapping::is_always_strided()` is `true`.
...
[7]{.pnum} *Constraints:* 
   ...
   * [7.1]{.pnum} _`layout-mapping-alike`_`<OtherMapping>` is satisfied.

   * [7.2]{.pnum} _`rank_`_ ` == OtherMapping::extents_type::rank()` is `true`.

   * [7.3]{.pnum} `OtherMapping::is_always_strided()` is `true`.
...
```
[3]{.pnum} *Result:* A complete object type that is not an abstract class type.

```c++
typename A::pointer
```
[4]{.pnum} *Result:* A type that models `semiregular` and `swappable` ([swappable.requirements]), and for which `is_nothrow_move_constructible_v<A::pointer>` is `true`, `is_nothrow_move_assignable_v<A::pointer>` is `true`, and `is_nothrow_swappable_v<A::pointer>` is `true`.
...
```
[6]{.pnum} *Result:* A type that models `convertible_to<A::element_type>`,
...
```
[8]{.pnum} *Result:* A type `OP` such that:

   * [8.1]{.pnum} `OP` meets the accessor policy requirements.

   * [8.2]{.pnum} `constructible_from<OP, const A&>` is modeled.

   * [8.3]{.pnum} `is_same_v<typename OP::element_type, typename A::element_type>` is `true`
...
[10]{.pnum} $[$`p`, `n`$)$ is an _accessible range_ for `a` if, for each $i$ in the range $[0,$`n`$)$, `a.access(p, `$i$`)` is valid.
...
[12]{.pnum} *Returns:* `p_sub` such that for `a_sub` being `A::offset_policy(a)`, and any integer `n` for which $[$`p`, `n`$)$ is an accessible range of `a`:

   * [12.1]{.pnum} $[$`p_sub`, `n-i`$)$ is an accessible range of `a_sub`, and

   * [12.2]{.pnum} `a_sub.access(p_sub, `$j$`)` provides access to the same element as `a.access(p, i+`$j$`)`, for every $j$ in the range $[$`0`, `n-i`$)$.
