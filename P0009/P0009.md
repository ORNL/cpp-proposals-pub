...
The _size_ of a multidimensional index space is the product of $U_i-L_i$ for each dimension $i$ if its rank is greater than $0$, and $1$ otherwise.
In the following representations of a multidimensional index space, such as `extents`, will be used synonymously with the index space itself.
...
[1]{.pnum} In subclause 22.7.�.1 and subclause 22.7.�.2
   ...
   * [1.3]{.pnum} `i` and `j` are packs of (possibly const) integers which are multidimensional indices into `m.extents()` ([mdspan.terms]).
     <i>[Note:</i> The type of each element of `i` and `j` can be a different integer type. <i>— end note]</i>

   * [1.4]{.pnum} `r` is a (possibly const) rank index of `typename M::extents_type`.

   * [1.5]{.pnum} $d_r$ is a pack of (possibly const) integers for which `sizeof...(`$d_r$`) == e.rank()` is `true`,
...
[4]{.pnum} *Result:* A type `MP` which meets the layout mapping policy requirements ([mdspan.layoutpolicy.reqmts]) and for which 
                     `typename MP::template mapping<typename M::extents_type>` denotes the type `M`.
...
m(i...)
```
[6]{.pnum} *Result:* `typename M::size_type`

[7]{.pnum} *Returns:* A value less then `numeric_limits<typename M::size_type>::max - 1`.
...
[11]{.pnum} *Returns:* If the multidimensional index space `m.extents()` has size $0$, then `0`,
...
[13]{.pnum} *Returns:* `true` only if for every `i` and `j` where `(i != j || ...)` is `true`, `m(i...) != m(j...)` is `true`.
...
[15]{.pnum} *Returns:* `true` only if for all $k$ in the range $[0,$ `m.required_span_size()` $)$ there exists an `i` such that `m(i...)` equals $k$.
     <i>[Note:</i> A mapping may return `false` even if the above condition is met.
...
[17]{.pnum} *Returns:* `true` only if for every rank index $r$ of `m.extents()` there exists an integer $s_r$
       such that, for all `i` where `(i+`$d_r$`)` is a multidimensional index into `m.extents()` ([mdspan.terms]),
...
m.stride(r)
```
[18]{.pnum} *Preconditions:* `m.is_strided()` is `true`.

[19]{.pnum} *Result:* `typename M::size_type`

[20]{.pnum} *Returns:* $s_r$ as defined in `m.is_strided()` above.

```c++
M::is_always_unique()
```
[21]{.pnum} *Result:* A constant expression ([expr.const]) of type `bool`.

[22]{.pnum} *Returns:* `true` only if `m.is_unique()` is `true` for all objects `m` of type `M`.
     <i>[Note:</i> A mapping may return `false` even if the above condition is met.
...
```
[23]{.pnum} *Result:* A constant expression ([expr.const]) of type `bool`.

[24]{.pnum} *Returns:* `true` only if `m.is_contiguous()` is `true` for all objects `m` of type `M`.
     <i>[Note:</i> A mapping may return `false` even if the above condition is met.
...
M::is_always_strided()
```
[25]{.pnum} *Result:* A constant expression ([expr.const]) of type `bool`.

[26]{.pnum} *Returns:* `true` only if `m.is_strided()` is `true` for all objects `m` of type `M`.
     <i>[Note:</i> A mapping may return `false` even if the above condition is met.
                   For certain layout mappings it may not be feasible to determine whether every instance is strided.<i>— end note]</i>


<b>22.7.�.2 Layout policy requirements [mdspan.layoutpolicy.reqmts]</b>
...
[2]{.pnum} *Result:* A type `X` that meets the layout mapping requirements ([mdspan.layout.reqmts]), and for which `typename M::layout_type` denotes `MP` and `typename M::extents_type` denotes `E`.
...
[1]{.pnum} *Preconditions:* The size of the multidimensional index space `e` is a representable value of `size_type` ([basic.fundamental]).
...
[1]{.pnum} *Constraints:*
   ...
   * [1.2]{.pnum} `is_constructible_v<extents_type, static_cast<size_type>(SizeTypes)...>` is `true`,
...
constexpr size_type size() const;
```
[7]{.pnum} *Precondition:* The size of the multidimensional index space `extents()` is a representable value of `size_type` ([basic.fundamental]).
