...
- make extents and mdspan constructors accept either rank_dynamic or rank integer args (or array of that size)
...
* [5]{.pnum} *Preconditions:* If `N` is `rank()`, $r$ is smaller than `N` and $E_r$ is a _static extent_, than
             $E_r$ equals `args[r]`.
