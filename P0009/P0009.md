...
[6]{.pnum} *Constraints:* 
  * [4.1]{.pnum} `is_same_v<layout_stride, typename LayoutStrideMapping::layout_type>` is `true`,
  * [4.2]{.pnum} `is_constructible_v<extents_type,typename LayoutRightMapping::extents_type>` is `true`.
...
[14]{.pnum} *Constraints:* `x.extents().rank() == y.extents().rank()` is `true`.
...
* [4]{.pnum} *Constraints:* `is_constructible_v<extents_type,OtherExtents>` is `true`.
...
* [6]{.pnum} *Constraints:* `is_constructible_v<extents_type,OtherExtents>` is `true`.

* [7]{.pnum} *Preconditions:*

   * [7.1]{.pnum} `extents_type::rank() == 0 || other.stride(extents_type::rank()-1) == 1` is `true`, and

   * [7.2]{.pnum} for all `r` in the range $[0,$ `extents_type::rank()-2`$)$, `other.stride(r)` equals
...
* [11]{.pnum} *Preconditions:* $0\:\le$ `array{i...}[r]` $\lt$ `extents().extent(r)` for all `r` in the range $[0,$ `extents_type::rank()` $)$.
...
* [12]{.pnum} *Returns:* `1` if `r` equals `extents_type::rank()-1`, otherwise, the product of `extents().extent(k)` 
   for all `k` in the range $[$ `r+1` $,$ `extents_type::rank()` $)$.
...
* [7]{.pnum} *Constraints:*
    ...
    * [7.3]{.pnum} `is_constructible_v<extents_type, LayoutMapping::extents_type>` is `true`.
...
* [11]{.pnum} *Returns:*
    ...
    * [11.1]{.pnum} One plus the sum of `(extents().extent(r) - 1) * stride(r)` for all `r` in the range $[0,$ `extents_type::rank()` $)$ if `(extents().extent(r) > 0)` is `true` for all `r` in the range $[0,$ `extents_type::rank()` $)$.
...
* [13]{.pnum} *Preconditions:* $0\:\le$ `array{i...}[r]` $\lt$ `extents().extent(r)` for all `r` in the range $[0,$ `extents_type::rank()` $)$.
