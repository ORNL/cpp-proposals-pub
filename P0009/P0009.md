...
[2]{.pnum} A _multidimensional index_ is an element within a multidimensional
...
The multidimensional index `idx...` refers to an element within the domain of a multidimensional index space if both the following are true:
  ...
  * [2.2]{.pnum} For all $i$ in the range $[0,$rank$)$, the $i^{th}$ value of `idx` is an integer in the range $[I_b, I_e)$.
...
template<class>
constexpr size_t @_make-dynamic-extent_@() { return dynamic_extent; } // @_exposition only_@

template <class... Integrals>
explicit extents(Integrals...)
  -> extents<@_make-dynamic-extent_@<Integrals>()...>;
...
[5]{.pnum} If $E_r$ is a dynamic extent, let $D_r$ be the value of _`dynamic-extents`_`[`_`dynamic-index`_`(`_r_`)]`.
...
* [3]{.pnum} *Effects:* For each $i$ in the range $[0, dynamic_rank())$,
             initializes the $i^{th}$ element of _`dynamic-extent`_ with `other.extents(`_`dynamic-index-inv`_`(i))`.
...
Let `b` be `std::array<size_type,rank()>{exts...}`
...
* [7]{.pnum} *Effects:*  
    ...
    * [7.2]{.pnum} If `sizeof...(SizeTypes)` equals `rank()`, 
                   for each $i$ in the range $[0, rank_dynamic())$, initializes the $i^{th}$ element of _`dynamic-extent`_
                   with `std::array<size_type,rank()>{exts...}[`_`dynamic-index-inv`_`(i)]`.
...
* [10]{.pnum} *Effects:*  
    ...
    * [10.2]{.pnum} If `N` equals `rank()`, 
                   for all $d$ in the range `[0, rank_dynamic())`, initializes _`dynamic-extent`_`[d]`
...
* [4]{.pnum} *Returns:* _`dynamic-extent`_`[`_`dynamic-index`_`(r)]` if `static_extent(r) == dynamic_extent` is `true`, otherwise `static_extent(r)`.
...
`E::rank() == Rank && E::rank() == E::dynamic_rank()`  is `true`.
...
<b>22.7.�.1 Layout mapping requirements general [mdspan.layout.reqmts_gen]</b>

[1]{.pnum} In subclause 22.7.�.2,

    * [1.1]{.pnum} `MP` denotes a layout mapping policy class.

    * [1.2]{.pnum} `M` denotes a specialization of the layout mapping policys nested layout mapping template class.

    * [1.3]{.pnum} `E` denotes a specialization of `extents`.

    * [1.4]{.pnum} `e` denotes a value of type `E`.

    * [1.5]{.pnum} `m` denotes a value of type `M`.

    * [1.6]{.pnum} `i...` and `j...` are multidimensional indices in the multidimensional index space defined by `e`.

    * [1.7]{.pnum} `r` is an integral value in the range `[0,e.rank())`.

    * [1.8]{.pnum} `dr...` is an integer pack where `sizeof...(dr)` equals `e.rank()`,
...
```c++
typename MP::mapping_type<E>
```
* [2]{.pnum} *Result:* `M`
* [3]{.pnum} *Preconditions:* `M` meets the layout mapping requirements specified below.


* [4]{.pnum} A type `M` meets the *layout mapping* requirements if `M` meets the requirements of *Cpp17CopyConstructible*, *Cpp17CopyAssignable*, and *Cpp17EqualityComparable*, and the following types and expressions are well-formed and have the specified semantics.
...
```
* [3]{.pnum} *Result:* `MP`
* [4]{.pnum} *Preconditions:* `is_same_v<MP::template mapping<extents_type>, M>` is `true`.
...
m(i...)
```
* [9]{.pnum} *Preconditions:* $0\:\le$ `array{i...}[r]` $\lt$ `extents().extent(r)`
       for all `r` in the range $[0,$ `Extents::rank()` $)$.

* [10]{.pnum} *Result:* `E::size_type`

* [11]{.pnum} *Value:* A value in the range of $[0,$ `required_span_size()` $)$.
       defined by applying the layout mapping to a multidimensional index `i...`.
...
* [13]{.pnum} *Value:* If the multidimensional index space that `e` defines is empty, then zero,
...
* [19]{.pnum} *Value:* `true` if for every `r` there exists an integer `sr`
       ...
       `m(j...) - m(i...)` equals `sr`. Otherwise, `false`.
...
struct layout_left {
  ...
  private:
    Extents @_extents\__@{}; // @_exposition only_@
  };
};
}
```
...
* [3]{.pnum} *Effects:* Initializes $extents\_$ with `other.extents()`.
...
* [5]{.pnum} *Effects:* Initializes $extents\_$ with `other.extents()`.
...
* [8]{.pnum} *Effects:* Initializes $extents\_$ with `other.extents()`.
...
struct layout_stride {
  ...
  private:
    Extents @_extents\__@{}; // @_exposition only_@
    array<size_type, Extents::rank()> @_strides\__@{}; // @_exposition only_@
  };
};
}
```
...
* [6]{.pnum} *Effects:* Initializes $extents\_$ with `other.extents()`, and initializes $strides\_$ with `other.strides()`.
...
* [8]{.pnum} *Effects:* Initializes $extents\_$ with `other.extents()`, and initializes $strides\_$ such that
  $strides\_$`[r] == other.stride(r)` is `true` for all $r$ in the range $[0,$ `Extents::rank()` $)$.
...
```
* [8]{.pnum} *Result:* A type that meets the requirements of accessor policy, which can be constructed from `a` and for which
 `is_convertible_v<A, A::offset_policy>` is `true`.
...
a.offset(p, i)
```
* [9]{.pnum} *Result:* `A::offset_policy::pointer`
* [10]{.pnum} *Value:* A handle to the range of objects which that starts at the `i`-th element of the objects `p` is a handle for.
...
public:
  ...
  constexpr accessor_type accessor() const { return @_acc\__@; }
  ...
  constexpr Extents extents() const { return @_map\__@.extents(); }
  ...
  constexpr pointer data() const { return @_ptr\__@; }
  constexpr mapping_type mapping() const { return @_map\__@; }
...
* [2]{.pnum} *Effects:*
    ...
    + [2.1]{.pnum} Initializes _`ptr_`_ with `ptr`, and

    + [2.2]{.pnum} Initializes _`map_`_ with `Extents(exts...)`.
...
* [4]{.pnum} *Effects:*
    ...
    + [4.1]{.pnum} Initializes _`ptr_`_ with `p`, and

    + [4.2]{.pnum} Initializes _`map_`_ with `Extents(exts)`.
...
* [8]{.pnum} *Effects:*
    ...
    + [8.1]{.pnum} Initializes _`ptr_`_ with `p`, and
...
* [9]{.pnum}*Effects:*
    ...
    + [9.1]{.pnum} Initializes _`ptr_`_ with `p`,
...
* [12]{.pnum} *Effects:*
    ...
    + [12.1]{.pnum} Initializes _`ptr_`_ with `other.`_`ptr_`_,

    + [12.2]{.pnum} initializes _`map_`_ with `other.`_`map_`_, and

    + [12.3]{.pnum} initializes _`acc_`_ with `other.`_`acc_`_.
...
* [2]{.pnum} *Preconditions:* _`acc_`_`.access(`_`ptr_`_`, `_`map_`_`(indices...))` shall be valid.

* [3]{.pnum} *Effects:* Equivalent to: `return `_`acc_`_`.access(`_`ptr_`_`, `_`map_`_`(indices...));`.
