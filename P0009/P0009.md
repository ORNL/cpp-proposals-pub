...
<b>22.7.�.2 Exposition only helpers [mdspan.extents.helpers]</b>
...
[8]{.pnum} *Returns:* If `i+1 < rank()` is `true`, the product of `extent(k)` for all `k` in the range $[$ `i` $,$ `e.rank()` $)$, otherwise `1`.
...
[5]{.pnum} Let `exts_arr` be `array<size_type,sizeof...(SizeTypes)>{static_cast<size_type>(move(exts))...}`
...
<b>22.7.�.4 Observers of the multidimensional index space [mdspan.extents.obs]</b>
...
<br/>
<b>22.7.�.5 `extents` comparison operators [mdspan.extents.cmp]</b>
...
<br/>
<b>22.7.�.6 Template alias `dextents` [mdspan.extents.dextents]</b>
...
[1]{.pnum} In subclause 22.7.�.1 and subclause 22.7.�.2
   ...
   * [1.3]{.pnum} `E` denotes a specialization of `extents`.

   * [1.4]{.pnum} `e` denotes a (possibly const) value of type `E`.

   * [1.5]{.pnum} `i` and `j` are packs of (possibly const) integers which are multidimensional indices into `e` ([mdspan.terms]).

   * [1.6]{.pnum} `r` is a (possibly const) rank index of `E`.

   * [1.7]{.pnum} $d_r$ is a pack of (possibly const) integers for which `sizeof...(`$d_r$`) == e.rank()` is `true`,
                 the `r`-th element is equal to `1`, and all other elements are equal to `0`.

<b>22.7.�.1 Layout mapping requirements [mdspan.layout.reqmts]</b>
...
```
[4]{.pnum} *Result:* A type `MP` which meets the layout mapping policy requirements ([mdspan.layoutpolicy.reqmts]) and for which 
                     `is_same_v<typename MP::template mapping<typename M::extents_type>, M>` is `true`.

```c++
m.extents()
```
[5]{.pnum} *Result:* `const typename M::extents_type&`
...
[8]{.pnum} *Postcondition:* `m(i...)==m(static_cast<typename M::size_type>(i)...)` is `true`.

```c++
m.required_span_size()
```
[9]{.pnum} *Result:* `typename M::size_type`

[10]{.pnum} *Returns:* If the multidimensional index space that `e` defines has size $0$, then `0`,
...
[12]{.pnum} *Returns:* `true` if for every `i` and `j` where `(i != j || ...)` is `true`, `m(i...) != m(j...)` is `true`. Otherwise `false`.
...
[14]{.pnum} *Returns:* `true` if for all $k$ in the range $[0,$ `m.required_span_size()` $)$ there exists an `i` such that `m(i...)` equals $k$. Otherwise `false`.
...
[16]{.pnum} *Returns:* `true` if for every rank index `r` of `m.extents()` there exists an integer $s_r$
...
[18]{.pnum} *Returns:* `true` if `m.is_unique()` is `true` for all objects `m` of type `M`. Otherwise, `false`.

[19]{.pnum} *Remarks:* `M::is_always_unique()` is a core constant expression ([expr.const]).
...
[21]{.pnum} *Returns:* `true` if `m.is_contiguous()` is `true` for all objects `m` of type `M`. Otherwise, `false`.

[22]{.pnum} *Remarks:* `M::is_always_contiguous()` is a core constant expression ([expr.const]).
...
[24]{.pnum} *Returns:* `true` if `m.is_strided()` is `true` for all objects `m` of type `M`. Otherwise, `false`.

[25]{.pnum} *Remarks:* `M::is_always_strided()` is a core constant expression ([expr.const]).
...
[27]{.pnum} *Result:* `typename M::size_type`

[28]{.pnum} *Returns:* $s_r$ as defined in `m.is_strided()` above.


[29]{.pnum} A type `MP` meets the *layout mapping policy* requirements if the following types are well formed and have the specified semantics.

```c++
typename MP::template mapping_type<E>
```

[30]{.pnum} *Result:* A type `X` that meets the layout mapping requirements ([mdspan.layout.reqmts]), and for which both `is_same_v<typename M::layout_type,MP>` and `is_same<typename M::extents_type,E>` are `true`.
...
[5]{.pnum} *Constraints:* 
   ...
   * [5.1]{.pnum} `is_same_v<typename layout_right::template mapping<typename LayoutRightMapping::extents_type>,LayoutRightMapping>` is `true`.

   * [5.2]{.pnum} `extents_type::rank() <= 1` is `true`, and
 
   * [5.3]{.pnum} `is_constructible_v<extents_type,typename LayoutRightMapping::extents_type>` is `true`.
...
[7]{.pnum} *Constraints:* 
   ...
   * [7.1]{.pnum} `is_same_v<typename layout_stride::template mapping<typename LayoutStrideMapping::extents_type>,LayoutStrideMapping>` is `true`.

   * [7.2]{.pnum} `is_constructible_v<extents_type,typename LayoutRightMapping::extents_type>` is `true`.
...
[3]{.pnum} *Preconditions:* `static_cast<size_type>(i)` is a multidimensional index into $extents\_$ ([mdspan.terms]). 

[4]{.pnum} *Effects:* Let `P` be the parameter pack such that
...
[5]{.pnum} *Constraints:*
   ...
   * [5.1]{.pnum} `is_same_v<typename layout_left::template mapping<typename LayoutLeftMapping::extents_type>,LayoutLeftMapping>` is `true`.

   * [5.2]{.pnum} `extents_type::rank() <= 1` is `true`, and

   * [5.3]{.pnum} `is_constructible_v<extents_type,typename LayoutLeftMapping::extents_type>` is `true`.
...
[7]{.pnum} *Constraints:*
   ...
   * [7.2]{.pnum} `is_constructible_v<extents_type,typename LayoutStrideMapping::extents_type>` is `true`.
...
[2]{.pnum} *Constraints:*
   ...
   * [2.3]{.pnum} `(is_nothrow_constructlible_v<size_type, Indices> && ...)` is `true`.
...
[7]{.pnum} *Returns:*  `extents().`_`rev-prod-of-extents`_`(i+1)`
...
[5]{.pnum} Let _`REQUIRED-SPAN-SIZE`_`(e, strides)` be:
   ...
   * [5.1]{.pnum} $1$, if `e.rank()==0` is `true`, otherwise

   * [5.2]{.pnum} $0$, if size of `e` is $0$, otherwise

   * [5.3]{.pnum} $1$ plus the sum of products of `(e.extent(r) - 1)` and `strides[r]` for all `r` in the range $[0,$ `e.rank()` $)$.
...
[5]{.pnum} Let $P$ be a permutation of the integers $0, ...,$ `extents_type::rank()-1` and let $p_i$ be the $i^{th}$ element of $P$.

[6]{.pnum}*Returns:*

   * [6.1]{.pnum} `true` if `extents_type::ranks()` is zero.

   * [6.2]{.pnum} Otherwise, `true` if there is a permutation $P$ such that
...
[7]{.pnum} *Constraints:* `extents_type::rank() == OtherMapping::extents_type::rank()` is `true`, and
            `OtherMapping::is_always_strided()` is `true`.

[8]{.pnum} *Preconditions:* `OtherMapping` meets layout mapping requirements.

[9]{.pnum} *Returns:* If `x.extents() == y.extents()` is `true` and 
...
typename A::pointer
```
[4]{.pnum} *Result:* A type that meets the requirements of _Cpp17DefaultConstructible_, _Cpp17CopyConstructible_, and _Cpp17CopyAssignable_. `is_nothrow_move_constructible_v<A::pointer>` is `true` and `is_nothrow_move_assignable_v<A::pointer>` is `true`.
...
[9]{.pnum} *Returns:* An object which provides access to the `i`-th element in the range of elements that starts at `p`.


```c++
a.offset(p, i)
```
[10]{.pnum} *Result:* `A::offset_policy::pointer`

[11]{.pnum} *Postconditions:* Let `a_sub` be `A::offset_policy(a)`, `p_sub` be `a.offset(p,i)`, and `n` have a value such that $[$`p`,`n`$)$ is the accessible range of `a`, then 

   * [11.1]{.pnum} $[$`p_sub`,`n-i`$)$ is the accessible range of `a_sub`, and

   * [11.2]{.pnum} `a_sub.access(p_sub,j)` provides acces to the same element as `a.access(p,i+j)`, for every `j` in the range $[$`0`,`n-i`$)$. 
...
<b>22.7.�.3 Class template `default_accessor` members [mdspan.accessor.members]</b>
...
[4]{.pnum} `mdspan<ElementType, Extents, LayoutPolicy, AccessorPolicy>` is a trivially copyable type if 
           `AccessorPolicy`, `LayoutPolicy::mapping_type<Extents>` and `AccessorPolicy::pointer` are
           trivially copyable types.

[5]{.pnum} `ElementType` is required to be a complete object type that is neither an abstract class type nor an array type.
