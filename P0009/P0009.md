...
[4]{.pnum} *Effects:* Initializes $extents\_$ with `e`, and direct-non-list-initializes $strides\_$ with `s`.
...
* Initializes `sub.`_`acc_`_ with `src.accessor()`.
* Initializes `sub.`_`ptr_`_ with `src.accessor().offset(src.data(),apply(src.mapping(),first))`.
* Initializes `sub.`_`map_`_ in a way that above conditions are satisfied.
