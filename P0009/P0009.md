...
```
[2]{.pnum} *Result:* `typename M::extents_type::size_type`.
...
[4]{.pnum} *Preconditions:* `is_same_v<typename MP::template mapping<typename M::extents_type>, M>` is `true` and
    `MP` meets the *layout mapping policy* requirements ([mdspan.layoutpolicy.reqmts]).

```c++
m.extents()
```
[7]{.pnum} *Result:* `const typename M::extents_type&`
...
[26]{.pnum} *Remarks:* `M::is_always_unique()` is a core constant expression ([expr.const]).
...
[26]{.pnum} *Remarks:* `M::is_always_contiguous()` is a core constant expression ([expr.const]).
...
[26]{.pnum} *Remarks:* `M::is_always_strided()` is a core constant expression ([expr.const]).
...
[4]{.pnum} *Constraints:* 
   ...
   * [6.1]{.pnum} `is_same_v<typename layout_right::template mapping<typename LayoutRightMapping::extents_type>,LayoutRightMapping>` is `true`.
...
[4]{.pnum} *Constraints:*
   ...
   * [6.1]{.pnum} `is_same_v<typename layout_left::template mapping<typename LayoutLeftMapping::extents_type>,LayoutLeftMapping>` is `true`.
...
[12]{.pnum} *Constraints:* `extents_type::rank() > 0` is `true`.

[13]{.pnum} *Preconditions:* `i < extents_type::rank()` is `true`.
...
[15]{.pnum} *Constraints:* `extents_type::rank() == OtherExtents::rank()` is `true`.
...
[7]{.pnum} *Constraints:*
   ...
   * [7.6]{.pnum} `is_same_v<typename StridedLayoutMapping::extents_type,decltype(other.extents())>` is `true`.

   * [7.7]{.pnum} if `extents_type::rank()>0` then `is_same_v<decltype(other.stride(0)),typename StridedLayoutMapping::size_type>` is `true`.

[8]{.pnum} *Precondition:* `StridedLayoutMapping` meets the layout mapping requirements.
...
[10]{.pnum} *Returns:*
   ...
   * [10.2]{.pnum} `0`, if _`fwd-prod-of-extents`_`(extents(), extents_type::rank()-1)==0` is `true`, otherwise
...
[19]{.pnum} *Constraints:* `extents_type::rank() == OtherMapping::extents_type::rank()` is `true`, and
            `OtherMapping::is_always_strided()` is `true`.

[20]{.pnum} *Preconditions:* `OtherMapping` meets layout mapping requirements.
...
[5]{.pnum} In subclause 22.7.�.1,
   ...
   * [5.4]{.pnum} `n`, `i` and `j` each denote a `size_t` value.
...
[3]{.pnum} <i>[Note:</i> The type of `pointer` need not be `element_type*`. <i>— end note]</i>
...
[3]{.pnum} <i>[Note:</i> The type of `reference` need not be `element_type&`. <i>— end note]</i>
...
[13]{.pnum} *Postconditions:* Let `a_sub` be `A::offset_policy(a)`, `p_sub` be `a.offset(p,i)`, and `n` have a value such that $[$`p`,`n`$)$ is the accessible range of `a`, then 

   * [13.1]{.pnum} $[$`p_sub`,`n-i`$)$ is the accessible range of `a_sub`, and

   * [13.2]{.pnum} `a_sub.access(p_sub,j)` provides acces to the same element as `a.access(p,i+j)`. 
...
[2]{.pnum} *Effects:*
   ...
   * [2.1]{.pnum} Direct-non-list-initializes _`ptr_`_ with `std::move(p)`, and

   * [2.2]{.pnum} Direct-non-list-initializes _`map_`_ with `Extents(std::move(exts)...)`.
...
[4]{.pnum} *Effects:*
   ...
   * [4.1]{.pnum} Direct-non-list-initializes _`ptr_`_ with `std::move(p)`, and

   * [4.2]{.pnum} Direct-non-list-initializes _`map_`_ with `extents_type(exts)`.
...
[6]{.pnum} *Effects:*
   ...
   * [6.2]{.pnum} Direct-non-list-initializes _`map_`_ with `ext`.
...
[2]{.pnum} *Precondition:* $[$`p`,`m.required_span_size()`$)$ is an accesible range of _`acc_`_ for value of _`acc_`_ after the invocation of this constructor.

[8]{.pnum} *Effects:*

   * [8.1]{.pnum} Direct-non-list-initializes _`ptr_`_ with `std::move(p)`, and

   * [8.2]{.pnum} Direct-non-list-initializes _`map_`_ with `m`.

```c++
constexpr mdspan(pointer p, const mapping_type& m, const accessor_type& a);
```

[2]{.pnum} *Precondition:* $[$`p`,`m.required_span_size()`$)$ is an accesible range of `a`.

[9]{.pnum}*Effects:*

   * [9.1]{.pnum} Direct-non-list-initializes _`ptr_`_ with `std::move(p)`,

   * [9.2]{.pnum} Direct-non-list-initializes _`map_`_ with `m`, and

   * [9.3]{.pnum} Direct-non-list-initializes _`acc_`_ with `a`.
...
[12]{.pnum} *Preconditions:* 
   ...
   * [12.1]{.pnum} For all `r`, `static_extent(r) == dynamic_extent || static_extent(r) == other.extent(r)` is `true`.

   * [12.2]{.pnum} *Precondition:* $[$_`ptr_`_,_`map_`_`.required_span_size()`$)$ is an accesible range of _`acc_`_ for values of _`ptr_`_, _`map_`_ and _`acc_`_ after the invocation of this constructor.

[13]{.pnum} *Effects:*

   * [13.1]{.pnum} Direct-non-list-initializes _`ptr_`_ with `other.`_`ptr_`_,

   * [13.2]{.pnum} Direct-non-list-initializes _`map_`_ with `other.`_`map_`_, and

   * [13.3]{.pnum} Direct-non-list-initializes _`acc_`_ with `other.`_`acc_`_.

[14]{.pnum} *Remarks:* The expression inside `explicit` is equivalent to:
...
[15]{.pnum} *Constraints:* `(is_convertible_v<Integrals, size_type> && ...)` is `true`.

[16]{.pnum} *Remarks:* The deduced type is `mdspan<ElementType, dextents<sizeof...(Integrals)>>`.
...
mdspan(ElementType*, const array<SizeType, N>&)
  -> mdspan<@_see below_@>;
```
[17]{.pnum} *Remarks:* The deduced type is `mdspan<ElementType, dextents<N>>`.
...
mdspan(ElementType*, const extents<ExtentsPack...>&)
  -> mdspan<@_see below_@>;
```
[18]{.pnum} *Remarks:* The deduced type is `mdspan<ElementType, extents<ExtentsPack...>>`.
...
mdspan(ElementType*, const MappingType&)
  -> mdspan<@_see below_@>;
```
[19]{.pnum} *Remarks:* The deduced type is `mdspan<ElementType, typename MappingType::extents_type, typename MappingType::layout_type>`.
...
[20]{.pnum} *Remarks:* The deduced type is `mdspan<ElementType, typename MappingType::extents_type, typename MappingType::layout_type, AccessorType>`.
...
[6]{.pnum} *Returns:* _`fwd-prod-of-extents`_`(extents(), rank())`.
