...
[The class template `extents` stores multiple extents.
This defines a multi-dimensional index space where the number of extents stored is the <i>rank</i>.]{.add}
...
private:
  static constexpr size_t dynamic_rank(size_t) noexcept; // @<i>exposition only</i>@
