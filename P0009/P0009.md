...
[4]{.pnum} *Preconditions:* `is_same_v<typename MP::template mapping<extents_type>, M>` is `true` and 
...
[2]{.pnum} *Preconditions:* _`acc_`_`.access(`_`ptr_`_`, `_`map_`_`(static_cast<size_type>(std::move(indices))...))` is well defined.

[3]{.pnum} *Effects:* Equivalent to: `return `_`acc_`_`.access(`_`ptr_`_`, `_`map_`_`(static_cast<size_type>(std::move(indices))...));`.
