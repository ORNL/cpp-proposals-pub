...
```c++
template<class... SizeTypes>
  explicit constexpr extents(SizeTypes... ext) noexcept;
```
...
* [5]{.pnum} *Preconditions:* If $E_r$ is a _static extent_ and $r$ is smaller than `sizeof()...SizeTypes)`, than
...
* [6]{.pnum} *Effects:*  For all $r$ in the range `[0, rank_dynamic())`, initializes `dynamic_extents_[r]`
             with `std::array<size_type,sizeof()...(SizeTypes)>{dynamic...}[dynamic_index_rev(r)]`.
