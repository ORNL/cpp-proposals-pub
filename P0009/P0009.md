  ...
  - Why from all extents:
    ...
    - enables easier writing of certain types of generic code e.g.:
      ...
      auto alloc_gemm_result(mds1_t mdspan1, mds2_t mdspan2) {
         using return_t = mdspan<double,Extents<mds1_t::extents_type::static_extent<0>,mds2_t::extents_type::static_extent<1>>;
...
namespace std {
  ...
  template<class ElementType, class Extents, class LayoutPolicy,
    ...
    constexpr mdspan<ElementType, @_see below_@, @_see below_@, typename AccessorPolicy::offset_policy> submdspan(
...
* [8]{.pnum} *Returns*: If `i < rank_dynamic()` is true,
                        the minimum value of $r$ such that `dynamic_index(r+1)` is $i+1$.
...
* [5]{.pnum} *Preconditions:* If `sizeof...(SizeTypes) == rank()` is `true`,
...
* [6]{.pnum} *Effects:*  For all $r$ in the range `[0, rank_dynamic())`, initializes `dynamic_extents_[r]`
             with `std::array<size_type,rank()>{dynamic...}[dynamic_index_inv(r)]`.
...
* [7]{.pnum} *Constraints:*
   ...
   * [7.1]{.pnum} `is_convertible_v<SizeType, size_type>` is `true`, and

   * [7.2]{.pnum} `N` is `rank_dynamic()` or `rank()`.

* [5]{.pnum} *Preconditions:* If `N` is `rank()`, $r$ is smaller than `N` and $E_r$ is a _static extent_, then
             $E_r$ equals `args[r]`.
...
struct layout_left {
  ...
  public:
    ...
    template<class OtherExtents>
      ...
      requires(rank() <= 1);
    template<class OtherExtents>
      explicit constexpr mapping(const layout_stride::template mapping<OtherExtents>& other) noexcept;
...
* [3]{.pnum} *Preconditions:*
   ...
   * [3.1]{.pnum} `other.stride(0)` equals `1`, and

   * [3.2}{.pnum} for all `r` in the range $[1,$ `rank()`$)$, `other.stride(r)` equals
...
* [3]{.pnum} *Preconditions:*
   ...
   * [3.1]{.pnum} `other.stride(rank()-1)` equals `1`, and

   * [3.2}{.pnum} for all `r` in the range $[0,$ `rank()-2`$)$, `other.stride(r)` equals
