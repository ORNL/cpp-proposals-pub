...
[9]{.pnum} *Returns:* If `extents_type::rank() > 0` is `true`, the product of `extents().extent(r)` for all `r` in the range $[0,$ `extents_type::rank()` $)$. Otherwise, 1.
...
[15]{.pnum} *Returns:* `1` if `i` equals zero, otherwise, the product of `extents().extent(k)` for all `k` in the range $[0,$ `i` $)$.
...
[7]{.pnum} *Preconditions:*
   ...
   * [7.2]{.pnum} for all `r` in the range $[0,$ `extents_type::rank()-1`$)$, `other.stride(r)` equals
                  the product of `extents().extent(k)` for all `k` in the range of $[r+1,$ `extents_type::rank()`$)$.
...
[14]{.pnum} *Returns:* `1` if `i` equals `extents_type::rank()-1`, otherwise, the product of `extents().extent(k)` 
            for all `k` in the range $[$ `i+1` $,$ `extents_type::rank()` $)$.
