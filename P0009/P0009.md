...
[2]{.pnum} In subclause 24.7.�.2,
   ...
   * [2.3]{.pnum} `p` denotes an object of type `A::pointer`. <i>[Note:</i> The type of `pointer` need not be a dereferenceable type.
...
[13]{.pnum} $[$`p`, `n`$)$ is an _accessible range_ for `a` if, for each $i$ in the range $[0,$`n`$)$, `a.access(p, `$i$`)` is well defined.
