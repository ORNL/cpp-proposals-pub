...
[3]{.pnum} Let _`is-extents`_ be the exposition only class defined as follows:

```c++
template<class T>
struct @_is-extents_@ : false_type {};

template<size_t ... Args>
struct @_is-extents_@<extents<Args...> : true_type {};
```
...
concept @_layout-mapping-alike_@ = requires {
  requires @_is-extents_@<typename M::extents_type>::value;
  bool_constant<M::is_always_strided()>::value;
  bool_constant<M::is_always_contiguous()>::value;
  bool_constant<M::is_always_unique()>::value;
  same_as<decltype(M::is_always_strided()), bool>;
  same_as<decltype(M::is_always_contiguous()), bool>;
  same_as<decltype(M::is_always_unique()), bool>;
};
```
...
[2]{.pnum} In subclause 24.7.�.2,
   ...
   * [2.2]{.pnum} `a` denotes a possibly const object of type `A`.

   * [2.3]{.pnum} `p` denotes a possibly const object of type `A::pointer`. <i>[Note:</i> The type of `pointer` need not be a dereferenceable type.
                  For example, the expression `*std::declval<A::pointer>()` need not be valid. <i>- end note]</i>
...
[3]{.pnum} A type `A` meets the accessor policy requirements if

   * [3.1]{.pnum} `A` meets the requirements of *Cpp17CopyConstructible* and *Cpp17CopyAssignable*,

   * [3.2]{.pnum} `is_nothrow_move_constructible_v<A>` is `true`,

   * [3.3]{.pnum} `is_nothrow_move_assignable_v<A>` is `true`, and 

   * [3.4]{.pnum} the following types and expressions are well-formed and have the specified semantics.

```c++
typename A::element_type
```
[4]{.pnum} *Result:* A complete object type that is not an abstract class type.

```c++
typename A::pointer
```
[5]{.pnum} *Result:* A type that models `semiregular` and `swappable` ([swappable.requirements]), and for which `is_nothrow_move_constructible_v<A::pointer>` is `true`, `is_nothrow_move_assignable_v<A::pointer>` is `true`, and `is_nothrow_swappable_v<A::pointer>` is `true`.

[6]{.pnum} <i>[Note:</i> The type of `pointer` need not be `element_type*`. <i>— end note]</i>

```c++
typename A::reference
```
[7]{.pnum} *Result:* A type that models `convertible_to<A::element_type>`,
       and if `A::element_type&` models `assignable_from<A::element_type>` then `is_assignable_v<A::reference, A::element_type>` is `true`.

[8]{.pnum} <i>[Note:</i> The type of `reference` need not be `element_type&`. <i>— end note]</i>

```c++
typename A::offset_policy
```
[9]{.pnum} *Result:* A type `OP` such that:

   * [9.1]{.pnum} `OP` meets the accessor policy requirements.

   * [9.2]{.pnum} `constructible_from<OP, const A&>` is modeled.

   * [9.3]{.pnum} `is_same_v<typename OP::element_type, typename A::element_type>` is `true`

```c++
a.access(p, i)
```
[10]{.pnum} *Result:* `A::reference`

[11]{.pnum} $[$`p`, `n`$)$ is an _accessible range_ for `a` if, for each $i$ in the range $[0,$`n`$)$, `a.access(p, `$i$`)` is valid.
<i>[Note:</i> Concrete accessor policies will generally impose preconditions for their `access` function. However, they might not.
For example, one could imagine an accessor where `p` is `span<A::element_type,dynamic_extent>` and `access(p,i)` returns `i < p.size() ? p[i] : p[p.size()-1]`. 
<i>- end note]</i>

```c++
a.offset(p, i)
```
[12]{.pnum} *Result:* `A::offset_policy::pointer`

[13]{.pnum} *Returns:* `p_sub` such that for `a_sub` being `A::offset_policy(a)`, and any integer `n` for which $[$`p`, `n`$)$ is an accessible range of `a`:

   * [13.1]{.pnum} $[$`p_sub`, `n-i`$)$ is an accessible range of `a_sub`, and

   * [13.2]{.pnum} `a_sub.access(p_sub, `$j$`)` provides access to the same element as `a.access(p, i+`$j$`)`, for every $j$ in the range $[$`0`, `n-i`$)$.
...
[8]{.pnum} *Precondition:* $[$`p`, _`map_`_`.required_span_size()`$)$ is an accessible range of _`acc_`_ for the values of _`map_`_ and _`acc_`_ after the invocation of this constructor. 
...
[11]{.pnum} *Precondition:* $[$`p`, `m.required_span_size()`$)$ is an accessible range of _`acc_`_ for value of _`acc_`_ after the invocation of this constructor.
...
[13]{.pnum} *Precondition:* $[$`p`, `m.required_span_size()`$)$ is an accessible range of `a`.
...
[3]{.pnum} *Preconditions:* 
   ...
   * [3.1]{.pnum} $[$_`ptr_`_ `,` _`map_`_`.required_span_size()`$)$ is an accessible range of _`acc_`_, and

   * [3.2]{.pnum} `I` is a multidimensional index in `extents()`.
