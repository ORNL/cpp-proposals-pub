...
[2]{.pnum} In subclause 24.7.�.2,
   ...
   * [2.2]{.pnum} `a` denotes a value of type `A` or `const A`.

   * [2.3]{.pnum} `p` denotes a value of type `A::pointer` or `const A::pointer`. <i>[Note:</i> The type `A::pointer` need not be dereferenceable. <i>- end note]</i>

   * [2.4]{.pnum} `n`, `i` and `j` each denote values of type `size_t`.
