...
public:
  ...
  template<class SizeType, size_t N>
    explicit(N != rank_dynamic())
    constexpr extents(const span<SizeType, N>&) noexcept;
...
template<class SizeType, size_t N>
  explicit(N != rank_dynamic())
  constexpr extents(const span<SizeType, N>& exts) noexcept;
```
...
<b>22.7.�.4 Layout policies [mdspan.layoutpolicy.overview]</b>
...
[1]{.pnum} Each of `layout_left`, `layout_right`, and `layout_stride` meets the layout mapping policy requirements and is a trivially copyable type.
...
<br/>
<b>22.7.�.5 Class template `layout_left::mapping` [mdspan.layout.left]</b>


<br/>
<b>22.7.�.5.1 `layout_left::mapping` overview [mdspan.layout.left.overview]</b>
...
<b>22.7.�.5.2 `layout_left::mapping` constructors  [mdspan.layout.left.ctor]</b>
...
<b>22.7.�.5.3 `layout_left::mapping` observers  [mdspan.layout.left.obs]</b>
...
<br/>
<b>22.7.�.6 Class template `layout_right::mapping` [mdspan.layout.right]</b>

<br/>
<b>22.7.�.6.1 `layout_right::mapping` overview [mdspan.layout.right.overview]</b>
...
<b>22.7.�.6.2 `layout_right::mapping` constructors  [mdspan.layout.right.ctor]</b>
...
<b>22.7.�.6.3 `layout_right::mapping` observers  [mdspan.layout.right.obs]</b>
...
<br/>
<b>22.7.�.7 Class template `layout_stride::mapping` [mdspan.layout.stride]</b>

<br/>
<b>22.7.�.7.1 `layout_stride::mapping` overview [mdspan.layout.stride.overview]</b>
...
class layout_stride::mapping {
  ...
  public:
    ...
    template<class SizeType>
    constexpr mapping(const extents_type&,
                      const span<SizeType, rank>&) noexcept;
    ...
    constexpr const extents_type& extents() const noexcept { return @_extents\__@; }
    constexpr const span<size_type, rank>& strides() const noexcept
    { return @_strides\__@; }
...
<b>22.7.�.7.2 `layout_stride::mapping` exposition-only helpers  [mdspan.layout.stride.expo]</b>

[1]{.pnum} Let _`REQUIRED-SPAN-SIZE`_`(e, strides)` be:

   * [1.1]{.pnum} $1$, if `e.rank() == 0` is `true`, otherwise

   * [1.2]{.pnum} $0$, if size of the multidimensional index space `e` is $0$, otherwise
...
<b>22.7.�.7.3 `layout_stride::mapping` constructors  [mdspan.layout.stride.ctor]</b>

```c++
template<class SizeType>
constexpr mapping(const extents_type& e, const array<SizeType, rank>& s) noexcept;
```
...
[3]{.pnum} *Preconditions:*
   ...
   * [3.3]{.pnum} If `rank` is greater than zero, then there exists a permutation $P$ 
...
```c++
template<class SizeType>
constexpr mapping(const extents_type& e, const span<SizeType, rank>& s) noexcept;
```
[1]{.pnum} Let $P$ be a permutation of the integers $0, ...,$ `rank-1` and let $p_i$ be the $i^{th}$ element of $P$.
...
<b>22.7.�.7.4 `layout_stride::mapping` mapping observers  [mdspan.layout.stride.obs]</b>
...
public:
  ...
  template<class SizeType, size_t N>
    explicit(N != rank_dynamic())
    constexpr mdspan(pointer p, const span<SizeType, N>& exts);
  ...
  template<class SizeType>
    constexpr reference operator[](const span<SizeType, rank()>& indices) const;
...
template <class ElementType, class SizeType, size_t N>
mdspan(ElementType*, const span<SizeType, N>&)
  -> mdspan<ElementType, dextents<N>>;
...
[4]{.pnum} *Constraints:*
   ...
   * [4.2]{.pnum} `is_constructible_v<extents_type, const span<SizeType, N>&>` is `true`,
