...
[12]{.pnum} *Preconditions:* If `N != rank_dynamic()` is `true`,
            `exts[r]` equals $E_r$
            for each $r$ for which $E_r$ is a static extent.

[13]{.pnum} *Effects:*

  * [13.1]{.pnum} If `N` equals `dynamic_rank()`,
                  for all $d$ in the range $[0,$ `rank_dynamic()`$)$, direct-non-list-initializes _`dynamic-extent`_`[d]`
                  with `exts[d]`.

  * [13.2]{.pnum} Otherwise, for all $d$ in the range $[0,$ `rank_dynamic()`$)$, direct-non-list-initializes _`dynamic-extent`_`[d]`
                  with `exts[`_`dynamic-index-inv`_`(d)]`.
...
```
[14]{.pnum} *Constraints:* `(is_convertible_v<Integrals, size_type> && ...)` is `true`.

[15]{.pnum} *Remarks:* The deduced type is `dextents<sizeof...(Integrals)>`.
...
[7]{.pnum} *Preconditions:*
   ...
   * [7.1]{.pnum}`s[i] > 0` is `true` for all `i` in the range $[0,$ `rank` $)$.

   * [7.2]{.pnum} _`REQUIRED-SPAN-SIZE`_`(e, s)` is a representable value of type `size_type` ([basic.fundamental]).

   * [7.3]{.pnum} If `rank` is greater than zero, then there exists a permutation $P$ 
     such that `s[` $p_i$ `] >= s[` $p_{i-1}$ `] * e.extent(` $p_{i-1}$ `)` is `true`
     for all $i$ in the range $[1,$ `rank` $)$. 
     <i>[Note:</i> This condition guarantees that `is_unique()` is `true`. <i>— end note]</i>

[8]{.pnum} *Effects:* Direct-non-list-initializes $extents\_$ with `e`, and for all $d$ in the range 
                      $[0,$ `rank`$)$, direct-non-list-initializes _`strides_`_`[d]` with `s[d]`.
...
[9]{.pnum} *Constraints:*
   ...
   * [9.1]{.pnum} `is_constructible_v<extents_type, StridedLayoutMapping::extents_type>` is `true`.

   * [9.2]{.pnum} `StridedLayoutMapping::is_always_unique()` is `true`.

   * [9.3]{.pnum} `StridedLayoutMapping::is_always_strided()` is `true`.

   * [9.4]{.pnum} `is_same_v<typename StridedLayoutMapping::extents_type, decltype(other.extents())>` is `true`.

   * [9.5]{.pnum} if `rank > 0` then `is_same_v<decltype(other.stride(0)), typename StridedLayoutMapping::size_type>` is `true`.

[10]{.pnum} *Precondition:* 

   * [10.1]{.pnum} `StridedLayoutMapping` meets the layout mapping requirements, 

   * [10.2]{.pnum} `other.required_span_size()` is a representable value of type `size_type` ([basic.fundamental]), and

   * [10.3]{.pnum} _`OFFSET`_`(other) == 0` is `true`.

[11]{.pnum} *Postcondition:* `*this == other` is `true`.

[12]{.pnum} *Remarks:* The condition in the `explicit` is:
...
[11]{.pnum} *Precondition:* $[$`p`, _`map_`_`.required_span_size()`$)$ is an accesible range of _`acc_`_ for the values of _`map_`_ and _`acc_`_ after the invocation of this constructor. 
...
[14]{.pnum} *Precondition:* $[$`p`, `m.required_span_size()`$)$ is an accesible range of _`acc_`_ for value of _`acc_`_ after the invocation of this constructor.
...
[16]{.pnum} *Precondition:* $[$`p`, `m.required_span_size()`$)$ is an accesible range of `a`.
...
[18]{.pnum} *Mandates:* 
   ...
   * [18.1]{.pnum} `is_constructible_v<pointer, const OtherAccessor::pointer&>` is `true`;

   * [18.2]{.pnum} `is_constructible_v<extents_type, OtherExtents>` is `true`.

[19]{.pnum} *Constraints:*

   * [19.1]{.pnum} `is_constructible_v<mapping_type, const OtherLayoutPolicy::template mapping<OtherExtents>&>` is `true`;

   * [19.2]{.pnum} `is_constructible_v<accessor_type, const OtherAccessor&>` is `true`; and
...
[20]{.pnum} *Preconditions:* 
   ...
   * [20.2]{.pnum} $[$_`ptr_`_, _`map_`_`.required_span_size()`$)$ is an accesible range of _`acc_`_ for values of _`ptr_`_, _`map_`_ and _`acc_`_ after the invocation of this constructor.
...
[23]{.pnum} *Constraints:* `(is_convertible_v<Integrals, size_type> && ...)` is `true`.
