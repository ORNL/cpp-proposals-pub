...
[godbolt](https://godbolt.org/z/Mxa7cej1a).
...
public:
  ...
  static constexpr auto @_index-cast_@(OtherSizeType&&) noexcept; // @_exposition only_@
...
[2]{.pnum} *Mandates:*
   ...
   * [2.1]{.pnum} `SizeType` is a signed or unsigned integer type, and

   * [2.2]{.pnum} each element of `Extents` is either equal to `dynamic_extent`, or is representable as a value of type `SizeType`.

[3]{.pnum} Each specialization of `extents` models `regular` and is trivially copyable.

[4]{.pnum} Let $E_r$ be the $r^{th}$ element of `Extents`.
...
[5]{.pnum} The $r^{th}$ interval of the multidimensional index space represented by an `extents` object is $[0, D_r)$.
...
```c++
  template<class OtherSizeType>
  static constexpr auto @_index-cast_@(OtherSizeType&& i) noexcept; // @_exposition only_@
```

[9]{.pnum} *Effects:*

  * [9.1]{.pnum} If `OtherSizeType` is an integral type other than `bool`, then equivalent to `return i;`, otherwise

  * [9.2]{.pnum} equivalent to `return static_cast<size_type>(i);`.
    <i>[Note:</i> This function will always return an integral type other than `bool`. Since this function's call sites are 
    constrained on convertibility of `OtherSizeType` to `size_type`, integer-class types can use the `static_cast` branch 
    without loss of precision.<i>— end note]</i>
...
[5]{.pnum} *Constraints:*
   ...
   * [5.3]{.pnum} `sizeof...(OtherSizeTypes) == rank_dynamic() || sizeof...(OtherSizeTypes) == rank()` is `true`.
...
[6]{.pnum} Let `exts_arr` be `array<size_type, sizeof...(OtherSizeTypes)>{static_cast<size_type>(std::move(exts))...}`

[7]{.pnum} *Preconditions:*

   * [7.1]{.pnum} If `sizeof...(OtherSizeTypes) != rank_dynamic()` is `true`, `exts_arr[r]` equals 
...
[10]{.pnum} *Preconditions*:
   ...
   * [10.1]{.pnum} If `N != rank_dynamic()` is `true`, `exts[r]` equals $E_r$ for each $r$ for which $E_r$ is a static extent, and
...
[13]{.pnum} *Remarks:* The deduced type is `dextents<size_t, sizeof...(Integrals)>`.
...
[1]{.pnum} *Returns:* `true` if `lhs.rank()` equals `rhs.rank()` and if `lhs.extent(r)` equals `rhs.extent(r)` for every rank index `r` of `rhs`, otherwise `false`.
...
[8]{.pnum} *Returns:* A nonnegative integer less than `numeric_limits<typename M::size_type>::max()` and less than or equal to `numeric_limits<size_t>::max()`.
...
[2]{.pnum} *Effects:* Direct-non-list-initializes _`extents_`_ with `e`.
...
[4]{.pnum} *Preconditions*: `other.required_span_size()` is representable as a value of type `size_type` ([basic.fundamental]).

[5]{.pnum} *Effects:* Direct-non-list-initializes _`extents_`_ with `other.extents()`.
...
[6]{.pnum} *Constraints:* 
   ...
   * [6.1]{.pnum} `extents_type::rank() <= 1` is `true`, and
 
   * [6.2]{.pnum} `is_constructible_v<extents_type, OtherExtents>` is `true`.

[7]{.pnum} *Preconditions*: `other.required_span_size()` is representable as a value of type `size_type` ([basic.fundamental]).

[8]{.pnum} *Effects:* Direct-non-list-initializes _`extents_`_ with `other.extents()`.
...
[9]{.pnum} *Constraints:* `is_constructible_v<extents_type, OtherExtents>` is `true`.

[10]{.pnum} *Preconditions:*

   * [10.1]{.pnum} If `extents_type::rank() > 0` is `true`, then for all $r$ in the range $[0,$ `extents_type::rank()`$)$, `other.stride(`$r$`)` equals
                  `extents().`_`fwd-prod-of-extents`_`(`$r$`)`, and

   * [10.2]{.pnum} `other.required_span_size()` is representable as a value of type `size_type` ([basic.fundamental]).

[11]{.pnum} *Effects:* Direct-non-list-initializes _`extents_`_ with `other.extents()`.
...
[3]{.pnum} *Preconditions:* `extents_type::`_`index-cast`_`(i)` is a multidimensional index in _`extents_`_ ([mdspan.terms]).
...
[10]{.pnum} *Preconditions:*
   ...
   * [10.1]{.pnum} If `extents_type::rank() > 0` is `true`, then for all $r$ in the range $[0,$ `extents_type::rank()`$)$, `other.stride(`$r$`)` equals `extents().`_`rev-prod-of-extents`_`(`$r$`)`.
...
class layout_stride::mapping {
  ...
  public:
    ...
    constexpr const extents_type& extents() const noexcept { return @_extents\__@; }
    constexpr array<size_type, @_rank\__@> strides() const noexcept
    { return @_strides\__@; }
...
[1]{.pnum} *Constraints:*
   ...
   * [1.1]{.pnum} `is_convertible_v<const OtherSizeType&, size_type>` is `true`,

   * [1.2]{.pnum} `is_nothrow_constructible_v<size_type, const OtherSizeType&>` is `true`.
...
[3]{.pnum} *Effects:* Direct-non-list-initializes _`extents_`_ with `e`, and for all $d$ in the range 
...
[6]{.pnum} *Effects:* Direct-non-list-initializes _`extents_`_ with `other.extents()`, and for all $d$ in the range 
...
[7]{.pnum} *Result:* A type `OP` such that:
   ...
   * [7.2]{.pnum} `constructible_from<OP, const A&>` is modeled, and
...
[4]{.pnum} `AccessorPolicy` shall meet the accessor policy requirements [mdspan.accessor.reqmts].

[5]{.pnum} Each specialization `MDS` of `mdspan` models `copyable` and

   * [5.1]{.pnum} `is_nothrow_move_constructible_v<MDS>` is `true`,

   * [5.2]{.pnum} `is_nothrow_move_assignable_v<MDS>` is `true`, and
   
   * [5.3]{.pnum} `is_nothrow_swappable_v<MDS>` is `true`.

[6]{.pnum} A specialization of `mdspan` is a trivially copyable type if its `accessor_type`, `mapping_type`, and `pointer` are
...
[7]{.pnum} *Constraints:*
   ...
   * [7.1]{.pnum} `is_convertible_v<const OtherSizeType&, size_type>` is `true`,

   * [7.2]{.pnum} `(is_nothrow_constructible<size_type, const OtherSizeType&> && ...)` is `true`,
...
[15]{.pnum} *Effects:*
   ...
   * [15.3]{.pnum} Value-initializes _`acc_`_.
...
[4]{.pnum} *Effects:* Equivalent to: `return ` _`acc_`_`.access(`_`ptr_`_`, `_`map_`_`(static_cast<size_type>(std::move(indices))...));`
...
[5]{.pnum} *Constraints:*
   ...
   * [5.1]{.pnum} `is_convertible_v<const OtherSizeType&, size_type>` is `true`, and
   
   * [5.2]{.pnum} `is_nothrow_constructible_v<size_type, const OtherSizeType&>` is `true`.
...
[7]{.pnum} *Precondition:* The size of the multidimensional index space `extents()` is representable as a value of type `size_t` ([basic.fundamental]).
