...
```c++
constexpr rank_type @_dynamic-index_@(rank_type i) noexcept; // @_exposition only_@
```
...
```c++
constexpr rank_type @_dynamic-index-inv_@(rank_type i) noexcept; // @_exposition only_@
```
...
[8]{.pnum} `LayoutPolicy` shall meet the layout mapping policy requirements.

[9]{.pnum} `AccessorPolicy` shall meet the accessor policy requirements. If `is_same_v<typename AccessorPolicy::element_type, ElementType>` equals `false`, then the program is ill-formed.
