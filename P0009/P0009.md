...
[1]{.pnum} In subclause 22.7.�.1 and subclause 22.7.�.2
    ...
    * [1.1]{.pnum} `M` denotes a layout mapping template class.
    ...
    * [1.6]{.pnum} `i...` and `j...` are multidimensional indices which refer to an element in the
                   multidimensional index space defined by `e`.
...
* [19]{.pnum} *Value:* `true` if for every `r` there exists an integer `sr`
       such that, for all `i...` where `(i+dr)...` references an element in the multidimensional index space represented by `e`,
...
<b>22.7.�.2 Layout mapping policy requirements [mdspan.layout.reqmts]</b>
...
```
* [2]{.pnum} *Result:* `M`
* [3]{.pnum} *Preconditions:* `is_same_v<typename M::layout_type,MP>` is `true`
             and `is_same<typename M::extents_type,E>` is `true`.
