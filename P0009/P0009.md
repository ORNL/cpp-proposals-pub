...
[11]{.pnum} *Constraints:*
   ...
   * [11.3]{.pnum} `N==rank_dynamic() || N==rank()` is `true`.

[12]{.pnum} *Effects:* Equivalent to: `extents(span{exts})`.

```c++
template <class... Integrals>
explicit extents(Integrals...) -> @_see below_@;
```
[13]{.pnum} *Constraints:* `(is_convertible_v<Integrals, size_type> && ...)` is `true`.

[14]{.pnum} *Remarks:* The deduced type is `dextents<sizeof...(Integrals)>`.

<br/>

<b>24.7.�.3 Observers of the multidimensional index space [mdspan.extents.obs]</b>
...
<br/>
<b>24.7.�.4 `extents` comparison operators [mdspan.extents.cmp]</b>
...
<br/>
<br/>
<b>24.7.� Layout mapping policy [mdspan.layout]</b>

<b>24.7.�.1 Layout mapping policy general [mdspan.layout.general]</b>
...
<b>24.7.�.2 Layout mapping requirements [mdspan.layout.reqmts]</b>
...
[8]{.pnum} *Returns:* A value less then `numeric_limits<typename M::size_type>::max()`.
...
<b>24.7.�.3 Layout policy requirements [mdspan.layoutpolicy.reqmts]</b>
...
<b>24.7.�.4 Layout policies [mdspan.layoutpolicy.overview]</b>
...
<br/>
<b>24.7.�.5 Class template `layout_left::mapping` [mdspan.layout.left]</b>


<br/>
<b>24.7.�.5.1 `layout_left::mapping` overview [mdspan.layout.left.overview]</b>
...
<b>24.7.�.5.2 `layout_left::mapping` constructors  [mdspan.layout.left.ctor]</b>
...
<b>24.7.�.5.3 `layout_left::mapping` observers  [mdspan.layout.left.obs]</b>
...
<br/>
<b>24.7.�.6 Class template `layout_right::mapping` [mdspan.layout.right]</b>

<br/>
<b>24.7.�.6.1 `layout_right::mapping` overview [mdspan.layout.right.overview]</b>
...
<b>24.7.�.6.2 `layout_right::mapping` constructors  [mdspan.layout.right.ctor]</b>
...
<b>24.7.�.6.3 `layout_right::mapping` observers  [mdspan.layout.right.obs]</b>
...
<br/>
<b>24.7.�.7 Class template `layout_stride::mapping` [mdspan.layout.stride]</b>

<br/>
<b>24.7.�.7.1 `layout_stride::mapping` overview [mdspan.layout.stride.overview]</b>
...
<b>24.7.�.7.2 `layout_stride::mapping` exposition-only helpers  [mdspan.layout.stride.expo]</b>
...
<b>24.7.�.7.3 `layout_stride::mapping` constructors  [mdspan.layout.stride.ctor]</b>
...
[6]{.pnum} *Constraints:*
   ...
   * [6.1]{.pnum} `is_convertible_v<const SizeType&, size_type>` is `true`,

   * [6.2]{.pnum} `is_nothrow_constructible_v<size_type, const SizeType&>` is `true`.

[7]{.pnum} *Effects:* Equivalent to: `mapping(e, span{s});`.
...
[8]{.pnum} *Constraints:*
   ...
   * [8.1]{.pnum} `is_constructible_v<extents_type, StridedLayoutMapping::extents_type>` is `true`.

   * [8.2]{.pnum} `StridedLayoutMapping::is_always_unique()` is `true`.

   * [8.3]{.pnum} `StridedLayoutMapping::is_always_strided()` is `true`.

   * [8.4]{.pnum} `is_same_v<typename StridedLayoutMapping::extents_type, decltype(other.extents())>` is `true`.

   * [8.5]{.pnum} if `rank > 0` then `is_same_v<decltype(other.stride(0)), typename StridedLayoutMapping::size_type>` is `true`.

[9]{.pnum} *Precondition:* 

   * [9.1]{.pnum} `StridedLayoutMapping` meets the layout mapping requirements, 

   * [9.2]{.pnum} `other.required_span_size()` is a representable value of type `size_type` ([basic.fundamental]), and

   * [9.3]{.pnum} _`OFFSET`_`(other) == 0` is `true`.

[10]{.pnum} *Postcondition:* `*this == other` is `true`.

[11]{.pnum} *Remarks:* The condition in the `explicit` is:
...
<b>24.7.�.7.4 `layout_stride::mapping` mapping observers  [mdspan.layout.stride.obs]</b>
...
<br/>
<b>24.7.�.1 Accessor Policy [mdspan.accessor.general]</b>
...
<br/>
<b>24.7.�.2 Accessor policy requirements [mdspan.accessor.reqmts]</b>
...
<b>24.7.�.4 `default_accessor` members [mdspan.accessor.members]</b>
...
<br/>
<b>24.7.�.1 `mdspan` overview [mdspan.mdspan.overview]</b>
...
<b>24.7.�.2 `mdspan` constructors  [mdspan.mdspan.ctor]</b>
...
[7]{.pnum} *Constraints:*
   ...
   * [7.2]{.pnum} `is_constructible_v<extents_type, const span<SizeType, N>&>` is `true`,

   * [7.3]{.pnum} `is_constructible_v<mapping_type, extents_type>` is `true`, and

   * [7.4]{.pnum} `is_default_constructible_v<accessor_type>` is `true`.

[8]{.pnum} *Effects:* Equivalent to: `mdspan(p, span{exts});`.
...
[9]{.pnum} *Constraints:*
   ...
   * [9.1]{.pnum} `is_constructible_v<mapping_type, const extents_type&>` is `true`, and

   * [9.2]{.pnum} `is_default_constructible_v<accessor_type>` is `true`.

[10]{.pnum} *Precondition:* $[$`p`, _`map_`_`.required_span_size()`$)$ is an accesible range of _`acc_`_ for the values of _`map_`_ and _`acc_`_ after the invocation of this constructor. 

[11]{.pnum} *Effects:*

   * [11.1]{.pnum} Direct-non-list-initializes _`ptr_`_ with `std::move(p)`, and

   * [11.2]{.pnum} Direct-non-list-initializes _`map_`_ with `ext`.

```c++
constexpr mdspan(pointer p, const mapping_type& m);
```

[12]{.pnum} *Constraints:* `is_default_constructible_v<accessor_type>` is `true`.

[13]{.pnum} *Precondition:* $[$`p`, `m.required_span_size()`$)$ is an accesible range of _`acc_`_ for value of _`acc_`_ after the invocation of this constructor.

[14]{.pnum} *Effects:*

   * [14.1]{.pnum} Direct-non-list-initializes _`ptr_`_ with `std::move(p)`, and

   * [14.2]{.pnum} Direct-non-list-initializes _`map_`_ with `m`.

```c++
constexpr mdspan(pointer p, const mapping_type& m, const accessor_type& a);
```

[15]{.pnum} *Precondition:* $[$`p`, `m.required_span_size()`$)$ is an accesible range of `a`.

[16]{.pnum}*Effects:*

   * [16.1]{.pnum} Direct-non-list-initializes _`ptr_`_ with `std::move(p)`,

   * [16.2]{.pnum} Direct-non-list-initializes _`map_`_ with `m`, and

   * [16.3]{.pnum} Direct-non-list-initializes _`acc_`_ with `a`.
...
[17]{.pnum} *Mandates:* 
   ...
   * [17.1]{.pnum} `is_constructible_v<pointer, const OtherAccessor::pointer&>` is `true`;
...
[19]{.pnum} *Preconditions:* 
   ...
   * [19.1]{.pnum} For each rank index `r` of `extents_type`, `static_extent(r) == dynamic_extent || static_extent(r) == other.extent(r)` is `true`.

   * [19.2]{.pnum} $[$_`ptr_`_, _`map_`_`.required_span_size()`$)$ is an accesible range of _`acc_`_ for values of _`ptr_`_, _`map_`_ and _`acc_`_ after the invocation of this constructor.

[20]{.pnum} *Effects:*

   * [20.1]{.pnum} Direct-non-list-initializes _`ptr_`_ with `other.`_`ptr_`_,

   * [20.2]{.pnum} Direct-non-list-initializes _`map_`_ with `other.`_`map_`_, and

   * [20.3]{.pnum} Direct-non-list-initializes _`acc_`_ with `other.`_`acc_`_.

[21]{.pnum} *Remarks:* The expression inside `explicit` is:
```c++
...
[22]{.pnum} *Constraints:* `(is_convertible_v<Integrals, size_type> && ...)` is `true`.
...
<br/>
<b>24.7.�.3 `mdspan` members [mdspan.mdspan.members]</b>
...
[7]{.pnum} *Constraints:*
   ...
   * [7.1]{.pnum} `is_convertible_v<const SizeType&, size_type>` is `true`, and
   
   * [7.2]{.pnum} `is_nothrow_constructible_v<size_type, const SizeType&>` is `true`.

[8]{.pnum} *Effects:* Equivalent to: `return operator[](span{indices});`.

```c++
constexpr size_type size() const;
```
[9]{.pnum} *Precondition:* The size of the multidimensional index space `extents()` is a representable value of type `size_type` ([basic.fundamental]).

[10]{.pnum} *Returns:* `extents().`_`fwd-prod-of-extents`_`(rank())`.
