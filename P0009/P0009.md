...
Let `exts_arr` be `std::array<size_type,sizeof...(SizeTypes)>{static_cast<size_type>(exts)...}`
...
[8]{.pnum} *Constraints:*
   ...
   * [8.2]{.pnum} `is_nothrow_constructible_v<size_type, SizeType>` is `true`, and
...
```
[1]{.pnum} *Returns:* Product of `e.extent(k)` for all `k` in the range $[0, $`i`$)$. 

```c++
template<size_t ... Extents>
constexpr size_t @_rev-prod-of-extents_@(extents<Extents>, size_t) noexcept; // @_exposition only_@
```
[1]{.pnum} *Returns:* Product of `e.extent(k)` for all `k` in the range $[$`i`$, $`e.rank()`$)$. 
...
[4]{.pnum} If `E` is not a specialization of `extents`, then the program is ill-formed.
...
[7]{.pnum} *Preconditions:*
   ...
   * [7.1]{.pnum} if `extents_type::rank() > 0` then `other.stride(0)` equals `1`, and

   * [7.2]{.pnum} for all `r` in the range $[1,$ `extents_type::rank()`$)$, `other.stride(r)` equals
                  _fwd-prod-of-extents_`(extents(),r)`.
...
[9]{.pnum} *Returns:* If `extents_type::rank() > 0` is `true`, _fwd-prod-of-extents_`(extents(),extents_type::rank())`. Otherwise, 1.
...
[13]{.pnum} *Constraints:* `extents().rank() > 0` is `true`.

[14]{.pnum} *Preconditions:* `i < extents().rank()` is `true`.

[15]{.pnum} *Returns:* `1` if `i` equals zero, otherwise, _fwd-prod-of-extents_`(extents(), i)`.
...
[4]{.pnum} *Constraints:*
  * [4.1]{.pnum} `is_same_v<layout_right,typename LayoutLeftMapping::layout_type>` is `true`.
  * [4.2]{.pnum} `extents_type::rank() <= 1` is `true`, and
...
[7]{.pnum} *Preconditions:*
   ...
   * [7.1]{.pnum} if `extents_type::rank() > 0` then `other.stride(extents_type::rank()-1) == 1` is `true`, and

   * [7.2]{.pnum} for all `r` in the range $[0,$ `extents_type::rank()-1`$)$, `other.stride(r)` equals
                  _rev-prod-of-extents_`(extents(),r+1)`.
...
[14]{.pnum} *Returns:* `1` if `i` equals `extents_type::rank()-1`, otherwise, _rev-prod-of-extents_`(extents(), i+1)`.
...
[5]{.pnum} *Constraints:* `is_constructible_v<extents_type,OtherExtents>` is `true`.

[6]{.pnum} *Effects:* Direct-non-list-initializes $extents\_$ with `other.extents()`, and initializes $strides\_$ with `other.strides()`.
...
[7]{.pnum} *Constraints:*
    ...
    * [7.1]{.pnum} `LayoutMapping` meets the layout mapping requirements.

    * [7.2]{.pnum} `is_constructible_v<extents_type, LayoutMapping::extents_type>` is `true`.

    * [7.3]{.pnum} `other.is_always_unique()` is `true`.

    * [7.4]{.pnum} `other.is_always_strided()` is `true`.


[8]{.pnum} *Effects:* Direct-list-initializes $extents\_$ with `other.extents()`, and initializes $strides\_$ such that
           $strides\_$`[r] == other.stride(r)` is `true` for all $r$ in the range $[0,$ `extents_type::rank()` $)$.

[9]{.pnum} *Remarks:* The expression inside `explicit` is equivalent to:
           `!std::is_convertible_v<typename LayoutMapping::extents_type,extents_type>`.
...
[10]{.pnum} *Returns:*
    ...
    * [10.1]{.pnum} One plus the sum of `(extents().extent(r) - 1) * stride(r)` for all `r` in the range $[0,$ `extents_type::rank()` $)$ if `(extents().extent(r) > 0)` is `true` for all `r` in the range $[0,$ `extents_type::rank()` $)$.

    * [10.2]{.pnum} Otherwise, `0`.
...
[17]{.pnum} *Constraints:* `x.extents().rank() == y.extents().rank()` is `true`.

[18]{.pnum} *Effects:* Equivalent to: `return x.extents() == y.extents() && x.strides() == y.strides();`.
...
[20]{.pnum} *Effects:* Equivalent to: `return x.extents() == y.extents() && x.strides() == y.strides();`.
...
```
    ...
    [12]{.pnum} *Value:* A handle to the range of objects which that starts at the `i`-th element of the objects `p` is a handle for.
