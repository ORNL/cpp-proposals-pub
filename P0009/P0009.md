...
[3]{.pnum} A type `A` meets the accessor policy requirements if
   ...
   * [3.1]{.pnum} `A` models `semiregular`,

   * [3.2]{.pnum} `is_nothrow_move_constructible_v<A>` is `true`,

   * [3.3]{.pnum} `is_nothrow_move_assignable_v<A>` is `true`,
   
   * [3.4]{.pnum} `is_nothrow_swappable_v<A>` is `true`, and 

   * [3.5]{.pnum} the following types and expressions are well-formed and have the specified semantics.
...
typename A::pointer
```
[5]{.pnum} *Result:* A type that models `semiregular`, and for which `is_nothrow_move_constructible_v<A::pointer>` is `true`, `is_nothrow_move_assignable_v<A::pointer>` is `true`, and `is_nothrow_swappable_v<A::pointer>` is `true`.
...
```
[7]{.pnum} *Result:* A type that models `common_reference_with<A::reference&&, A::element_type&>`.
...
[11]{.pnum} *Remarks:* The expression is equality preserving.

[12]{.pnum} <i>[Note:</i> Concrete accessor policies can impose preconditions for their `access` function. However, they might not.
For example, one could imagine an accessor where `p` is `span<A::element_type,dynamic_extent>` and `access(p,i)` returns `p[i % p.size()]`. 
<i>- end note]</i>

```c++
a.offset(p, i)
```
[13]{.pnum} *Result:* `A::offset_policy::pointer`

[14]{.pnum} *Returns:* `q` such that for `b` being `A::offset_policy(a)`, and any integer `n` for which $[$`0`, `n`$)$ is an accessible range of `p` and `a`:

   * [14.1]{.pnum} $[$`0`, `n-i`$)$ is an accessible range of `q` and `b`, and

   * [14.2]{.pnum} `b.access(q, j)` provides access to the same element as `a.access(p, i + j)`, for every `j` in the range $[$`0`, `n-i`$)$.

[15]{.pnum} *Remarks:* The expression is equality preserving.
...
[4]{.pnum} Each specialization `MDS` of `mdspan` models `semiregular` and
