...
- Wording changes per LWG review:
  - Change individual `Src*` and `Dst*` template arguments to `Src` and `Dst`, constraining that those are specializations of `std::mdspan`
  ...
  - Change `mdspan` parameter types to const references for consistency with `submdspan`
...
[1]{.pnum} *Constraints:*
  ...
  * [1.1]{.pnum} `Src` and `Dst` are specializations of `mdspan`,

  * [1.2]{.pnum} `std::is_assignable_v<typename Dst::reference, typename Src::reference>` is `true`, and

  * [1.3]{.pnum} `is_constructible_v<typename Src::extents_type, typename Dst::extents_type>` is `true`.
...
[5]{.pnum} *Constraints:*
  ...
  * [5.1]{.pnum} `Dst` is a specializations of `mdspan`, and

  * [5.2]{.pnum} `std::is_assignable_v<typename Dst::reference, const T&>` is `true`.
