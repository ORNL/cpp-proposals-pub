...
## P3242R3: 2026-06 Post-Brno Mailing{.unnumbered}

- Wording changes per LWG review:
  - Change individual `Src*` and `Dst*` template arguments to `SrcMDSpan` and `DstMDSpan`, constraining that those are specializations of `std::mdspan`
  - Add default for `T` template parameter in `fill`
  - Make first `copy` precondition hardened
  - Fix use of "unique" in multidimensional wording
  - Simplify effects wording for both `copy` and `fill`

## P3242R2: 2026-04 Post-Croydon Mailing{.unnumbered}

- Forwarded to LWG
- Add non-normative note to make constraint on `copy` clear
- Add `<mdspan>` header to wording
- Add feature test macro
- Add `constexpr` to serial `copy` and `fill` (pending LEWG approval)

## P3242R1: 2025-03 Post-Hagenberg Mailing{.unnumbered}

- Add discussion of applying `copy` to `mdspan`s of differing sizes

## P3242R0: 2024-04 Post-Tokyo Mailing{.unnumbered}
...
A common question about `copy` to `mdspan` is whether it should support `mdspan`s with differing extent (and presumably applying the operation only to the overlapping part). It's the authors' firm opinion that `copy` should **not** support this. Users wishing to copy an `mdspan` of a differing size should instead make use of `submdspan` [@P2630R4] as this provides explicit control of which subregions of the `mdspan` the user actually desires to copy.
...
> In [version.syn]{.sref}, add:

```c++
#define __cpp_lib_mdspan_copy YYYYMML // also in <mdspan>
```

> Adjust the placeholder value as needed so as to denote this proposal’s date of adoption.

> Add the following to [mdspan.syn]{.sref}

```c++
// [mdspan.copy]
template<class SrcMDSpan, class DstMDSpan>
constexpr void copy(SrcMDSpan src, DstMDSpan dst);

template<class ExecutionPolicy, class SrcMDSpan, class DstMDSpan>
void copy(ExecutionPolicy&& policy, SrcMDSpan src, DstMDSpan dst);

template<class DstMDSpan, class T = typename DstMDSpan::value_type>
constexpr void fill(DstMDSpan dst, const T& value);

template<class ExecutionPolicy, class DstMDSpan, class T = typename DstMDSpan::value_type>
void fill(ExecutionPolicy&& policy, DstMDSpan dst, const T& value);
```

> Add the following to [views.multidim]{.sref}:

## 23.7.3.?? Multidmensional Copy Algorithms{.unnumbered}
...
[1]{.pnum} *Constraints:*
  ...
  * [1.1]{.pnum} `SrcMDSpan` and `DstMDSpan` are specializations of `std::mdspan`,

  * [1.2]{.pnum} `std::is_assignable_v<typename SrcMDSpan::reference, typename DstMDSpan::reference>` is `true`, and

  * [1.3]{.pnum} `is_constructible_v<typename SrcMDSpan::extents_type, typename DstMDSpan::extents_type>`.
...
[2]{.pnum} *Hardened preconditions:* `src.extents()` equals `dst.extents()`.

[3]{.pnum} *Preconditions:*

  * [3.1]{.pnum} `dst.is_unique()` is `true`, and

  * [3.2]{.pnum} for each multidimensional index `i` in `src.extents()`, there is no multidimensional index `j` in `dst.extents()` such that `src[i]` and `dst[j]` refer to the same element.

[4]{.pnum} *Effects:* assigns each element of src to the corresponding element of `dst`.
...
[5]{.pnum} *Constraints:*
  ...
  * [5.1]{.pnum} `DstMDSpan` is a specializations of `std::mdspan`, and

  * [5.2]{.pnum} `std::is_assignable_v<typename DstMDSpan::reference, const T&>`.

[6]{.pnum} *Effects:* assigns `value` to each element of `dst`.
