...
[4]{.pnum} *Preconditions:*
  ...
  * [4.2]{.pnum} _`multiplyable`_`(A,x,y)` is `true`.
...
[6]{.pnum} These functions perform a Rank-2 update of a symmetric matrix taking into account the `Triangle` parameter [linalg.general].

[7]{.pnum} *Effects:* Computes $A'$ such that
...
[8]{.pnum} These functions perform a rank-2 update of a Hermitian matrix taking into account the `Triangle` parameter [linalg.general].
...
[2]{.pnum} *Mandates:* _`possibly-multiplyable`_`<InMat1, InMat2, OutMat>()` is `true`.

[3]{.pnum} *Preconditions:* _`multiplyable`_`(A, B, C)` is `true`.
...
[9]{.pnum} *Remarks:* `C` and `E` may alias.

#### Symmetric, Hermitian and triangular matrix-matrix product [linalg.algs.blas3.xmm]

<i>[Note:</i> These functions correspond to the BLAS function `xSYMM`, `xHEMM`, and `xTRMM`. The symmetric, Hermitian, or triangular matrix is given by the argument following the `Triangle` parameter.
<i>-- end note]</i>

[1]{.pnum} The following elements apply
to all functions in [linalg.algs.blas3.xmm].

[2]{.pnum} *Mandates:* 

   * [2.1]{.pnum} _`possibly-multiplyable`_`<decltype<A>, decltype<B>, decltype<C>>()` is `true`, and
 
   * [2.2]{.pnum} _`possibly-addable`_`<decltype<E>, decltype<E>, decltype<C>>()` is `true` for those overloads that take an `E` parameter.

[3]{.pnum} *Preconditions:*

   * [3.1]{.pnum} _`multiplyable`_`(A, B, C)` is `true`, and
...
[5]{.pnum} These functions perform an overwriting matrix-matrix multiply, taking into account the `Triangle` and `DiagonalStorage` parameter where applicable [linalg.general]. `A` is the symmetric or Hermitian matrix respectively.

[6]{.pnum} *Constraints:* If `InMat1` has `layout_blas_packed` layout, then the
      layout's `Triangle` template argument has the same type as
      the function's `Triangle` template argument.

[7]{.pnum} *Mandates:* _`compatible-static-extents`_`<InMat1,InMat1>(0,1)`

[8]{.pnum} *Preconditions:* `A.extent(0) == A.extent(1)` is `true.

[9]{.pnum} *Effects:* Computes $C = A B$.
...
[10]{.pnum} These functions perform an overwriting matrix-matrix multiply, taking into account the `Triangle` and `DiagonalStorage` parameter where applicable [linalg.general]. `B` is the symmetric or Hermitian matrix respectively.

[11]{.pnum} *Constraints:* If `InMat2` has `layout_blas_packed` layout, then the
      layout's `Triangle` template argument has the same type as
      the function's `Triangle` template argument.

[12]{.pnum} *Mandates:* _`compatible-static-extents`_`<InMat2,InMat2>(0,1)`

[13]{.pnum} *Preconditions:* `B.extent(0) == B.extent(1)` is `true.

[14]{.pnum} *Effects:* Computes $C = A B$.
...
[15]{.pnum} These functions perform an overwriting matrix-matrix multiply, taking into account the `Triangle` and `DiagonalStorage` parameter where applicable [linalg.general]. `A` is the symmetric or Hermitian matrix respectively.

[16]{.pnum} *Constraints:* If `InMat1` has `layout_blas_packed` layout, then the
      layout's `Triangle` template argument has the same type as
      the function's `Triangle` template argument.

[17]{.pnum} *Mandates:* _`compatible-static-extents`_`<InMat1,InMat1>(0,1)`

[18]{.pnum} *Preconditions:* `A.extent(0) == A.extent(1)` is `true.

[19]{.pnum} *Effects:* Computes $C = E + A B$.

[20]{.pnum} *Remarks:* `E` may alias `C`.
...
[21]{.pnum} These functions perform an overwriting matrix-matrix multiply, taking into account the `Triangle` and `DiagonalStorage` parameter where applicable [linalg.general]. `B` is the symmetric or Hermitian matrix respectively.

[22]{.pnum} *Constraints:* If `InMat2` has `layout_blas_packed` layout, then the
      layout's `Triangle` template argument has the same type as
      the function's `Triangle` template argument.

[23]{.pnum} *Mandates:* _`compatible-static-extents`_`<InMat2,InMat2>(0,1)`

[24]{.pnum} *Preconditions:* `B.extent(0) == B.extent(1)` is `true.

[25]{.pnum} *Effects:* Computes $C = E + A B$.

[26]{.pnum} *Remarks:* `E` may alias `C`.
