...
# Revision history
  ...
  * Revision 13 - running revision for LWG review
    ...
    * Move exposition-only helpers _`transpose-extents`_ and
        ...
        in terms of _`transpose-extents`_ (via `decltype`),
...
Scalar vector_two_norm(ExecutionPolicy&& exec,
                       ...
                       Scalar init);
template<@_in-vector_@ InVec>
auto vector_two_norm(InVec v) -> /* @_see-below_@ */;
template<class ExecutionPolicy,
         @_in-vector_@ InVec>
...
Scalar vector_abs_sum(ExecutionPolicy&& exec,
                      ...
                      Scalar init);
template<@_in-vector_@ InVec>
auto vector_abs_sum(InVec v) -> /* @_see-below_@ */;
template<class ExecutionPolicy,
         @_in-vector_@ InVec>
...
#### Exposition-only helpers [linalg.transp.helpers]
...
constexpr auto
@_transpose-extents_@(const InputExtents& in); // @_exposition only_@
```
...
[4]{.pnum} *Returns:* An `extents` object `out` such that

  * [4.1]{.pnum} `out.static_extent(1)` equals
      `in.static_extent(0)`,

  * [4.2]{.pnum} `out.static_extent(0)` equals
      `in.static_extent(1)`,

  * [4.3]{.pnum} `out.extent(1)` equals
      `in.extent(0)`, and

  * [4.4]{.pnum} `out.extent(0)` equals
      `in.extent(1)`.
...
[3]{.pnum} *Mandates:*
  ...
  * [3.1]{.pnum} `Extents` is a specialization of `extents`, and
...
[5]{.pnum} *Constraints:* `is_always_strided()` is `true`.
...
[7]{.pnum} *Constraints:*
  ...
  * [7.1]{.pnum} _`nested-mapping-type`_ and
    `decltype(y.`_`nested-mapping_`_`)`
    are equality comparable, and

  * [7.2]{.pnum} `x.rank()` equals `y.rank()`.
