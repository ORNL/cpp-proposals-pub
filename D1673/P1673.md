...
template<@_in-vector_@ InVec,
         class T>
T vector_two_norm(InVec v,
                  T init);
template<class ExecutionPolicy,
         @_in-vector_@ InVec,
         class T>
T vector_two_norm(ExecutionPolicy&& exec,
                  InVec v,
                  T init);
template<@_in-vector_@ InVec>
...
[1]{.pnum} For the effects of all functions in this clause,
...
[2]{.pnum} Some of the functions in this clause distinguish between
...
* [1.1]{.pnum} the `value_type` type alias
    ...
    of any function in **[linalg]**;

* [1.2]{.pnum} the `Scalar` template parameter (if any)
    of any function or class in **[linalg]**;

* [1.3]{.pnum} the `T` template parameter of any function
    in **[linalg]** with a `T init` parameter; and
...
* [2.4]{.pnum} The *triangular solve algorithms* in **[linalg.algs]**
    ...
    It is a constraint on the algorithm that
    any addend,
    any subtrahend,
...
* [2.6]{.pnum} If the wording for the function
    additionally includes

    * _`abs-if-needed`_ `(x)` for some expression `x`
      (**[linalg.scaled.abs]**),

    * _`conj-if-needed`_ `(x)` for some expression `x`
      (**[linalg.scaled.conj]**),

    * _`real-if-needed`_ `(x)` for some expression `x`
      (**[linalg.scaled.real]**),

    * _`imag-if-needed`_ `(x)` for some expression `x`
      (**[linalg.scaled.imag]**), or

    * `sqrt(x)` for some expression `x`,

    then it is a constraint on the algorithm that
    use of the expression
    is well formed.

* [2.7]{.pnum} If the function has an output `mdspan`,
    then all addends,
...
* [2.8]{.pnum} The function may reorder
...
* [2.3]{.pnum} use approximations (that might not be exact
...
[5]{.pnum} *Remarks:* `z` may refer to the same object as `x` or `y`.
...
[2]{.pnum} *Remarks:* `y` and `z` may refer to the same object.
...
[5]{.pnum} *Complexity:*
For all algorithms in this Clause with `mdspan` parameters,
...
[2]{.pnum} *Remarks:* `C` and `E` may refer to the same object.
