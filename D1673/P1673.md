...
### Math helper functions [linalg.type\_preserving\_math]

#### Exposition-only function object _`abs-if-needed`_ [linalg.type\_presercinb\_math.abs]
...
#### Exposition-only function object _`conj-if-needed`_ [linalg.type\_preserving\_math.conj]
...
#### Exposition-only function object _`real-if-needed`_ [linalg.type\_preserving\_math.real]
...
#### Exposition-only function object _`imag-if-needed`_ [linalg.type\_preserving\_math.imag]
...
[7]{.pnum} *Remarks:* The expression inside `noexcept` is equivalent to:
```c++
    noexcept(scaling_factor() * NestedAccessor::element_type(` _`nested-accessor`_`.access(p, i)))
```
...
```c++
constexpr const ScalingFactor& scaling_factor() const;
```

[9]{.pnum} *Returns:* _`scaling-factor`_
