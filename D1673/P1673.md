...
# Revision history
  ...
  * Revision 13 - running revision for LWG review
    ...
    * Add `triangle_type` and `storage_order_type` public type aliases
    ...
    * Make sure that Complexity clauses use a math multiplication symbol
        instead of code-font `*`.
        (The latter would cause unwarranted overflow issues,
        especially for BLAS 3 functions.)
...
Our proposal also has the following distinctive characteristics.
  ...
  * It uses `mdspan` (adopted into C++23),
  ...
  * Unlike the BLAS, our proposal can be expanded to support "batched"
      operations (see P2901) with almost no interface differences.
      This will support machine learning and other applications
      that need to do many small matrix or vector operations at once.
...
by using `submdspan` (P2630, adopted into the C++26 draft).
(`submdspan` was separated from the rest of P0009
as a way to avoid delaying the adoption of P0009 into C++23.
...
* how we support for user-defined complex number types
    ...
    with `std::abs`, `std::conj`, `std::real`, and `std::imag`;

* how we support division for triangular solves,
    for value types with noncommutative multiplication; and

* how we address consistency between `layout_blas_packed`
    having a `Triangle` template parameter,
    and functions also taking a `Triangle` parameter.
...
(Ranges was a feature added to the C++ Standard Library in C++20.)
...
* None of the algorithms we propose share names
    with any existing ranges algorithms.
...
In addition, using `mdspan` has made it easier for us
to propose an efficient "batched" interface
in our separate proposal
<a href="https://wg21.link/p2901">P2901</a>,
with almost no interface differences.
...
We have submitted a separate proposal,
<a href="https://wg21.link/p2901">P2901</a>,
that adds "batched" versions
of linear algebra functions to this proposal.
"Batched" linear algebra functions solve many independent problems
all at once, in a single function call.
For discussion, please see also Section 6.2 of our background paper
<a href="https://wg21.link/p1417r0">P1417R0</a>.
...
This proposal depends on `mdspan`,
a feature proposed by P0009 and follow-on papers,
and voted into C++23.
The `mdspan` class template views the elements of a multidimensional array.
The rank (number of dimensions) is fixed at compile time.
Users may specify some dimensions at run time and others at compile time;
the type of the `mdspan` expresses this.
`mdspan` also has two customization points:
...
* the <a href="https://github.com/kokkos/stdBLAS/">reference implementation</a>
    written and maintained by the authors and others, and

* an implementation by NVIDIA, which was released as part of their HPC SDK,
    and which uses NVIDIA libraries such as cuBLAS
    to accelerate many of the algorithms.

This proposal depends on `mdspan` (adopted into C++23),
`submdspan` (P2630, adopted into the current C++ draft), and
(indirectly) on the padded `mdspan` layouts in P2642.
The
<a href="https://github.com/kokkos/mdspan/">reference implementation of `mdspan`,</a>
...
* M. Hoemmen, K. Liegeois, and C. Trott,
    "Extending Linear Algebra Support to Batched Operations,"
    <a href="https://wg21.link/p2901r0">P2901R0</a>, May 2023.
...
* C. Trott, D. S. Hollman, D. Lebrun-Grandie, M. Hoemmen, D. Sunderland,
    H. C. Edwards, B. A. Lelbach, M. Bianco, B. Sander, A. Iliopoulos,
    J. Michopoulos, and N. Liber,
    "`mdspan`," <a href="https://wg21.link/p0009r18">P0009R18</a>,
    Jul. 2022.

* C. Trott, D. S. Hollman, M. Hoemmen, D. Sunderland, and D. Lebrun-Grandie,
    "`mdarray`: An Owning Multidimensional Array Analog of `mdspan`",
    <a href="https://wg21.link/p1684r5">P1684R5</a>,
    May 2023.

* C. Trott, D. Lebrun-Grandie, M. Hoemmen, and N. Liber,
    "`submdspan`," <a href="https://wg21.link/p2630r4">P2630R4</a>,
    Jul. 2023.
...
policy which upon access produces scaled elements.
...
[3]{.pnum} *Returns:*
`mdspan<typename SA::element_type, Extents, Layout, SA>(x.data_handle(), x.mapping(), SA(alpha, x.accessor()))`
...
is an `mdspan` accessor policy which upon access produces conjugate elements.
...
[4]{.pnum} *Returns:* With `ReturnMapping` being the type `typename ReturnLayout::template mapping<ReturnExtents>`: 
  ...
  * [4.1]{.pnum}
      `R(a.data_handle(), ReturnMapping(`_`transpose-extents`_`(a.mapping().extents())), a.accessor())`
      if `Layout` is `layout_left`, `layout_right`, or
      a specialization of `layout_blas_packed`;

  * [4.2]{.pnum} otherwise,
      `R(a.data_handle(), ReturnMapping(`_`transpose-extents`_`(a.mapping().extents()), array{a.mapping().stride(1), a.mapping().stride(0)}), a.accessor())`
      if `Layout` is `layout_stride`;
...
[1]{.pnum} These functions compute the Givens plane rotation
...
<i>[Note:</i>
These functions correspond to the LAPACK function
`xLARTG`. <i>-- end note]</i>
...
[3]{.pnum} *Effects:* Applies the plane rotation
...
functions `xDOT`, `xDOTU`, and `xDOTC`.
<i>-- end note]</i>
...
[4]{.pnum} These functions compute a non-conjugated dot product with an explicitly specified result type.

[5]{.pnum} *Returns:* Let `N` be `v1.extent(0)`.

  * [5.1]{.pnum} `init` if `N` is zero;
...
[7]{.pnum} These functions compute a non-conjugated dot product with a default result type.

[8]{.pnum} *Effects:* Let `T` be `decltype(declval<typename InVec1::value_type>() * declval<typename InVec2::value_type>())`.
Then,

  * [8.1]{.pnum} the two-parameter overload is equivalent to
      `return dot(v1, v2, T{});`, and

  * [8.2]{.pnum} the three-parameter overload is equivalent to
...
[9]{.pnum} These functions compute a conjugated dot product with an explicit specified result type.

[10]{.pnum} *Effects:*

  * [10.1]{.pnum} The three-parameter overload is equivalent to
      `return dot(conjugated(v1), v2, init);`, and

  * [10.2]{.pnum} the four-parameter overload is equivalent to
...
[11]{.pnum} These functions compute a conjugated dot product with a default result type.

[12]{.pnum} *Effects:* Let `T` be `decltype(`_`conj-if-needed`_`(declval<typename InVec1::value_type>()) * declval<typename InVec2::value_type>())`.
Then,

  * [12.1]{.pnum} the two-parameter overload is equivalent to
      `return dotc(v1, v2, T{});`, and

  * [12.2]{.pnum} the three-parameter overload is equivalent to
...
[2]{.pnum} *Effects:* Returns a value `result` such that
  ...
  * [2.2]{.pnum} let `s2init` be `init.scaling_factor * init.scaling_factor * init.scaled_sum_of_squares`, then `result.scaling_factor * result.scaling_factor * result.scaled_sum_of_squares`
      equals the sum of `s2init` and the squares of _`abs-if-needed`_`(x[i])`
      for all `i` in the domain of `v`.
...
[2]{.pnum} *Returns:* The square root of the sum of the square of `init` and the squares of the absolute values of the elements of `v`.
...
<i>[Note:</i>
A possible implementation of this function
...
[1]{.pnum} Let `T` be `decltype(` _`abs-if-needed`_`(` _`real-if-needed`_ `(declval<typename InVec::value_type>())) + ` _`abs-if-needed`_`(` _`imag-if-needed`_ `(declval<typename InVec::value_type>())))`.
...
<i>[Note:</i> These functions exist in the BLAS standard but are not part of the reference implementation.
<i>-- end note]</i>
...
`decltype(` _`abs-if-needed`_`(declval<typename InMat::value_type>()) * ` _`abs-if-needed`_`(declval<typename InMat::value_type>()))`. Then,
...
[4]{.pnum} *Complexity:* $O($ `x.extent(0)` $\cdot$ `A.extent(1)` $)$
...
[5]{.pnum} These functions perform an overwriting matrix-vector product.
...
[7]{.pnum} These functions performs an updating matrix-vector product.
...
[5]{.pnum} These functions perform an overwriting symmetric matrix-vector product,
taking into account the `Triangle` parameter
that applies to the symmetric matrix `A` [linalg.general].
...
[7]{.pnum} These functions perform an updating symmetric matrix-vector product,
taking into account the `Triangle` parameter
that applies to the symmetric matrix `A` [linalg.general].
...
[5]{.pnum} These functions perform an overwriting Hermitian matrix-vector product,
taking into account the `Triangle` parameter
that applies to the Hermitian matrix `A` [linalg.general].
...
[7]{.pnum} These functions perform an updating Hermitian matrix-vector product,
taking into account the `Triangle` parameter
that applies to the Hermitian matrix `A` [linalg.general].
...
[6]{.pnum} *Complexity:* $O($ `x.extent(0)` $\cdot$ `A.extent(1)` $)$
...
[9]{.pnum} *Complexity:* $O($ `y.extent(0)` $\cdot$ `A.extent(1)` $)$
...
[13]{.pnum} *Complexity:* $O($ `x.extent(0)` $\cdot$ `A.extent(1)` $)$
...
[6]{.pnum} *Complexity:* $O($ `A.extent(1)` $\cdot$ `b.extent(0)` $)$
...
[11]{.pnum} *Complexity:* $O($ `A.extent(1)` $\cdot$ `b.extent(0)` $)$
...
[5]{.pnum} *Complexity:* $O($ `x.extent(0)` $\cdot$ `y.extent(0)` $)$
...
[7]{.pnum} *Effects:*
  ...
  * [7.2]{.pnum} otherwise, equivalent to
...
[4]{.pnum} *Complexity:* $O($ `x.extent(0)` $\cdot$ `x.extent(0)` $)$
...
[6]{.pnum} *Effects:* Computes a matrix $A'$ such that $A' = A + x x^T$
and assigns each element of $A'$ to the corresponding element of $A$.
...
$A' = A + \alpha x x^T$, where the scalar $\alpha$ is `alpha`,
and assigns each element of $A'$ to the corresponding element of $A$.
...
$A' = A + \alpha x x^H$, where the scalar $\alpha$ is `alpha`,
and assigns each element of $A'$ to the corresponding element of $A$.
...
[2]{.pnum} *Mandates:*
  ...
  * [2.3]{.pnum} _`possibly-multipliable`_`<decltype(A), decltype(x), decltype(y)>()` is `true`.
...
[4]{.pnum} *Complexity:* $O($ `x.extent(0)` $\cdot$ `y.extent(0)` $)$
...
[4]{.pnum} *Complexity:* $O($ `A.extent(0)` $\cdot$ `A.extent(1)` $\cdot$ `B.extent(1)` $)$
...
[4]{.pnum} *Effects:* Computes a matrix $C'$ such that $C' = A C$ and
assigns each element of $C'$ to the corresponding element of $C$.

[5]{.pnum} *Complexity:* $O($ `A.extent(0)` $\cdot$ `A.extent(1)` $\cdot$ `C.extent(0)` $)$
...
[8]{.pnum} *Effects:* Computes a matrix $C'$ such that $C' = C A$ and
assigns each element of $C'$ to the corresponding element of $C$.

[9]{.pnum} *Complexity:* $O($ `A.extent(0)` $\cdot$ `A.extent(1)` $\cdot$ `C.extent(0)` $)$
...
[4]{.pnum} *Complexity:* $O($ `A.extent(0)` $\cdot$ `A.extent(1)` $\cdot$ `C.extent(0)` $)$
...
[3]{.pnum} *Preconditions:*
  ...
  * [3.1]{.pnum} _`addable`_`(A, B, C)` is `true`, and
...
[5]{.pnum} *Effects:* Computes a matrix $C'$ such that $C' = C + A B^T + B A^T$,
and assigns each element of $C'$ to the corresponding element of $C$. 
...
[6]{.pnum} *Effects:* Computes a matrix $C'$ such that $C' = C + A B^T + B A^T$,
and assigns each element of $C'$ to the corresponding element of $C$. 
...
[5]{.pnum} *Complexity:* $O($ `A.extent(0)` $\cdot$ `X.extent(1)` $\cdot$ `X.extent(1)` $)$
...
[12]{.pnum} *Complexity:* $O($ `B.extent(0)` $\cdot$ `B.extent(1)` $\cdot$ `A.extent(1)` $)$
...
[5]{.pnum} *Complexity:* $O($ `A.extent(0)` $\cdot$ `A.extent(1)` $\cdot$ `B.extent(1)` $)$
...
[12]{.pnum} *Complexity:* $O($ `A.extent(0)` $\cdot$ `A.extent(1)` $\cdot$ `B.extent(1)` $)$
