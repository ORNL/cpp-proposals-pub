...
>
> At the end of *[numerics]* (after subsection 28.8 *[numbers]*), add all the material that follows add the .
...
class scaled_accessor {
public:
  using element_type = add_const_t<decltype(declval<ScalingFactor>() * declval<Accessor::element_type>())>;
...
template<class ScalingFactor, class ValueType>
concept @_scalable_@ = // exposition only
requires {
  { declval<ScalingFactor>() * declval<ValueType>() };
};
```

[2]{.pnum} *Mandates:*

  * [2.1]{.pnum} `is_copy_constructible_v<element_type>` is `true`.

  * [2.2]{.pnum} `ScalingFactor` models `semiregular`.

  * [2.3]{.pnum} `Accessor` meets the accessor policy requirements **[mdspan.accessor.reqmts]**.

  * [2.4]{.pnum} _`scalable`_`<ScalingFactor, typename Accessor::element_type>` is `true`. 
...
[6]{.pnum} *Returns:* `scaling_factor() * Accessor::element_type(` _`nested-accessor`_`.access(p, i))`
...
[8]{.pnum} *Returns:* _`scaling-factor`_

```c++
constexpr const Accessor& nested_accessor() const;
```

[9]{.pnum} *Returns:* _`nested-accessor`_
...
public:
  using element_type = decltype(@_conj-if-needed_@(declval<Accessor::element_type>()));
  using reference = element_type;
  using data_handle_type = typename Accessor::data_handle_type;
  using offset_policy = conjugate_accessor<Accessor::offset_policy>;

  constexpr conjugate_accessor(const Accessor& acc);
  constexpr conjugate_accessor() = default;
  constexpr conjugate_accessor(const scaled_accessor&) = default;
  template<class OtherScalingFactor, class OtherNestedAccessor>
    constexpr conjugate_accessor(const conjugate_accessor<OtherNestedAccessor>& other);
...
[2]{.pnum} *Mandates:*
  ...
  * [2.2]{.pnum} `Accessor` meets the accessor policy requirements **[mdspan.accessor.reqmts]**.

  * [2.3]{.pnum} _`scalable`_`<ScalingFactor, typename Accessor::element_type>` is `true`. 
...
_`nested-accessor`_ with `acc`.

```c++
template<class OtherScalingFactor, class OtherNestedAccessor>
  constexpr conjugate_accessor(const conjugate_accessor<OtherScalingFactor, OtherNestedAccessor>& other);
```

[4]{.pnum} *Constraints:*

  * [4.1]{.pnum} `is_constructible_v<Accessor, const OtherNestedAccessor&>` is `true`.

  * [4.2]{.pnum} `is_constructible_v<ScalingFactor, const OtherScalingFactor&>` is `true`.

[5]{.pnum} *Effects:* direct non-list-initializes _`nested-accessor`_ with `other.nested_accessor()`.
...
[6]{.pnum} *Returns:*
_`conj-if-needed`_`(`_`nested-accessor`_`.access(p, i))`
...
[1]{.pnum} Throughout [linalg.algs.blas1], [linalg.algs.blas2], and [linalg.algs.blas3],
 where the template parameters are not constrained, the names of template parameters are used to express type
requirements.
