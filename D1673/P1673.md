...
# Revision history
  ...
  * Revision 12 to be submitted 2023/03/15
    ...
    * Change `symmetric_matrix_{left,right}_product` to `symmetric_matrix_product`, and
      change `hermitian_matrix_{left,right}_product` to `hermitian_matrix_product`.
      Distinguish the left and right cases
      by order of the `A, t` and `B` parameters.
...
* [1.1]{.pnum} for any input or output `mdspan` parameter(s) of any function in **[linalg]**,
    the parameter(s)' `value_type` type alias;
...
[3]{.pnum} All of the following elements presume that the algorithm's
...
* [3.5]{.pnum} The *triangular solve algorithms* in **[linalg.algs]**
...
[6]{.pnum} The following elements apply to all overloads of
`triangular_matrix_left_product`.
...
[10]{.pnum} The following elements apply to all overloads of
`triangular_matrix_right_product`.
