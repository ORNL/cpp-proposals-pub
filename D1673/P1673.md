...
[1]{.pnum} *Mandates:* _`possibly-multiplyable`_`<InOutMat, InVec2, InVec1>()` is `true`.

[2]{.pnum} *Preconditions:* _`multiplyable`_`(A,y,x)` is `true`.
...
[2]{.pnum} *Mandates:*
  ...
  * [2.1]{.pnum} _`compatible-static-extents`_`<InOutMat, InOutMat>(0,1)` is `true`, and

  * [2.2]{.pnum} _`compatible-static-extents`_`<InOutMat, InVec>(0,0)` is `true`.
