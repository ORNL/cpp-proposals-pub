...
# Revision history
  ...
  * Revision 12 to be submitted 2023/03/15
    ...
    * Fix two design issues with `idx_abs_max`.
      ...
      1. For the complex case, make it behave like the BLAS's ICAMAX or IZAMAX, by using `abs(` _`real-if-needed`_ `(z[k])) + abs(` _`imag-if-needed`_ `(z[k]))` as the element absolute value definition for complex numbers, instead of `abs(z[k])`.
...
2. P1673 users have good reasons
    to define their own complex number types,
    but users cannot overload `conj`
    in the `std` namespace for these types.

### `real` and `imag` do not have the desired interface

The BLAS depends on being able to get
the real and imaginary parts of a complex number.
For example, its complex version of
the sum of absolute values of the elements of a vector
is the sum of absolute values
of the real and imaginary components
(the functions `SCASUM` and `DZASUM`).

The Standard currently offers `real` and `imag`
as the way to get the real and imaginary parts,
respectively, of a number.
However, there are two issues with these functions.

1. For arguments of integral type,
    `real` and `imag` always return `double`.
    Thus, they do not preserve the type of their argument.

2. P1673 users have good reasons
    to define their own complex number types,
    but users cannot overload `real` or `imag`
    in the `std` namespace for these types.
...
These assume that any type without ADL-findable
`real` resp. `imag` is a non-complex type.
...
<!--
This function exists because of `vector_two_norm` and `vector_abs_sum`.
...
[5]{.pnum} *Remarks:* `z` may refer to the same vector as `x` or `y`.
...
[1]{.pnum} *Effects:* Let `T` be `decltype(abs(declval<InVec::value_type>()) * abs(declval<InVec::value_type>()))`.
...
[1]{.pnum} *Mandates:*
  ...
  * [1.1]{.pnum} `decltype(init + abs(declval<InVec::value_type>()))`
      is convertible to `T`,
      if `InVec::value_type`
      is an arithmetic type.

  * [1.2]{.pnum} Otherwise,
      `decltype(init + abs(` _`real-if-needed`_ `(declval<InVec::value_type>())) + abs(` _`imag-if-needed`_ `(declval<InVec::value_type>())))`
      is convertible to `T`.
...
[2]{.pnum} *Returns:* Let `N` be `v.extent(0)`.
  ...
  * [2.3]{.pnum} Otherwise,
      *GENERALIZED_SUM*(`plus<>()`, `init`,
      `abs(` _`real-if-needed`_ `(v[0])) + abs(` _`imag-if-needed`_ `(v[0]))`, ...,
      `abs(` _`real-if-needed`_ `(v[N-1])) + abs(` _`imag-if-needed`_ `(v[N-1]))`).
...
[1]{.pnum} *Mandates:*
  ...
  * [1.2]{.pnum} Otherwise,
      `decltype(abs(` _`real-if-needed`_ `(declval<InVec::value_type>())) + abs(` _`imag-if-needed`_ `(declval<InVec::value_type>())))`
...
[2]{.pnum} *Returns:*
  ...
  * [2.3]{.pnum} Otherwise, the index (in the domain of `v`) of the first element `v_e` of `v` for which `abs(` _`real-if-needed`_ `(v_e)) + abs(` _`imag-if-needed`_ `(v_e))` has the largest value.
...
`decltype(abs(declval<InMat::value_type>()) * abs(declval<InMat::value_type>()))`,
Then, the one-parameter overload is equivalent to
...
[1]{.pnum} *Mandates:* `abs(declval<InMat::value_type>())`
is convertible to `T`.
...
[1]{.pnum} *Effects:* Let `T` be
`decltype(abs(declval<InMat::value_type>())`.
Then, the one-parameter overload is equivalent to
