...
// [linalg.helpers], exposition-only concepts and traits
...
*alias* each other,
if there exist packs of integers `i` and `j` which are multidimensional indicies into `e`, such that `x[i...]` and `y[j...]` refer to the same element.
...
model the requirements of `semiregular`.
...
In particular, if for all input and output arguments the `value_type` is a floating-point type, implementers are free to leverage approximations, use arithmetic operations not explicitly listed above, and compute floating point sums in any way that improves their accuracty.
<i>-- end note]</i>
...
* [1.1]{.pnum} `E` if `value_type` is an unsigned integer; otherwise
...
of a parameter constraint by _`in-vector`_, _`in-matrix`_, or _`in-object`_,
...
#### Checks for algorithm preconditions [linalg.helpers.precond]
...
[2]{.pnum} *Mandates:*
  ...
  * [2.4]{.pnum} `ScalingFactor` models `semiregular`,

  * [2.5]{.pnum} `NestedAccessor` meets the accessor policy requirements [mdspan.accessor.reqmts], and
...
[3]{.pnum} *Constraints:* `is_constructible_v<NestedAccessor, const OtherNestedAccessor&>` is `true`, and
...
```c++
constexpr reference access(data_handle_type p, size_t i) const noexcept(@_see below_@;
```

[6]{.pnum} *Returns:* `scaling_factor() * NestedAccessor::element_type(` _`nested-accessor`_`.access(p, i))`

[7]{.pnum} *Remarks:* The exception specification is equivalent to:
```c++
    noexcept(scaling_factor() * NestedAccessor::element_type(@_nested-accessor_@.access(p, i)))
```

```c++
constexpr offset_policy::data_handle_type
offset(data_handle_type p, size_t i) const noexcept(noexcept(@_nested-accessor_@.offset(p, i)));
```

[8]{.pnum} *Returns:* _`nested-accessor`_`.offset(p, i)`
...
public:
  ...
  constexpr const Accessor& nested_accessor() const noexcept { return @_nested-accessor_@; }
...
[7]{.pnum} *Remarks:* The exception specification is equivalent to:
```c++
noexcept(@_conj-if-needed_@(NestedAccessor::element_type(@_nested-accessor_@.access(p, i)))
```
...
[linalg.algs.blas3.symm.ovleft] and
[linalg.algs.blas3.symm.upleft].
...
[linalg.algs.blas3.symm.ovright] and
[linalg.algs.blas3.symm.upright].
...
##### Overwriting symmetric matrix-matrix left product [linalg.algs.blas3.symm.ovleft]
...
##### Overwriting symmetric matrix-matrix right product [linalg.algs.blas3.symm.ovright]
...
##### Updating symmetric matrix-matrix left product [linalg.algs.blas3.symm.upleft]
...
##### Updating symmetric matrix-matrix right product [linalg.algs.blas3.symm.upright]
...
[6]{.pnum} The following elements apply to all the functions in
[linalg.algs.blas3.hemm.ovleft] and
[linalg.algs.blas3.hemm.upleft].
...
[9]{.pnum} *Complexity:*
All algorithms in [linalg.algs.blas3.hemm.ovleft] and [linalg.algs.blas3.hemm.ovright] with `mdspan` parameters
perform a count of `mdspan` array accesses
and arithmetic operations that is linear in
`A.extent(0)` times `B.extent(1)` times `A.extent(1)`.

[10]{.pnum} The following elements apply to all the functions in
[linalg.algs.blas3.hemm.ovright] and
[linalg.algs.blas3.hemm.upright].
...
[13]{.pnum} *Complexity:*
All algorithms in [linalg.algs.blas3.hemm.ovright] and
[linalg.algs.blas3.hemm.upright] with `mdspan` parameters
perform a count of `mdspan` array accesses
and arithmetic operations that is linear in
`B.extent(0)` times `A.extent(1)` times `B.extent(1)`.

##### Overwriting Hermitian matrix-matrix left product [linalg.algs.blas3.hemm.ovleft]
...
##### Overwriting Hermitian matrix-matrix right product [linalg.algs.blas3.hemm.ovright]
...
##### Updating Hermitian matrix-matrix left product [linalg.algs.blas3.hemm.upleft]
...
##### Updating Hermitian matrix-matrix right product [linalg.algs.blas3.hemm.upright]
...
[6]{.pnum} The following elements apply to all the functions in
[linalg.algs.blas3.trmm.ovleft] and
[linalg.algs.blas3.trmm.upleft].
...
[9]{.pnum} *Complexity:*
For all algorithms in [linalg.algs.blas3.trmm.ovleft] and
[linalg.algs.blas3.trmm.upleft] with `mdspan` parameters,
...
[10]{.pnum} The following elements apply to all the functions in
[linalg.algs.blas3.trmm.ovright] and
[linalg.algs.blas3.trmm.upright].
...
[13]{.pnum} *Complexity:*
For all algorithms in [linalg.algs.blas3.trmm.ovright] and
[linalg.algs.blas3.trmm.upright] with `mdspan` parameters,
...
##### Overwriting triangular matrix-matrix left product [linalg.algs.blas3.trmm.ovleft]
...
##### Overwriting triangular matrix-matrix right product [linalg.algs.blas3.trmm.ovright]
...
##### Updating triangular matrix-matrix left product [linalg.algs.blas3.trmm.upleft]
...
##### Updating triangular matrix-matrix right product [linalg.algs.blas3.trmm.upright]
