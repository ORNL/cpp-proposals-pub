...
that this is a performance optimization,
to avoid the expense of computing $n$ square roots
(one for each vector element).
It also results in tighter error bounds.
For example, adding complex numbers
...
require using the "true" 1-norm $|A_{ij}|$,
instead of $|\Re(A_{ij})| + |\Im(A_{ij})|$
