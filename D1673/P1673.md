...
[1]{.pnum} *Mandates:*  _`abs-if-needed`_ `(declval<typename InVec::value_type>())` is convertible to `Scalar`.
...
[1]{.pnum} *Mandates:*
`init + ` _`abs-if-needed`_ `(declval<typename InVec::value_type>()) * ` _`abs-if-needed`_ `(declval<typename InVec::value_type>())`
...
[1]{.pnum} *Effects:* Let `T` be `decltype(` _`abs-if-needed`_ `(declval<typename InVec::value_type>()) * ` _`abs-if-needed`_ `(declval<typename InVec::value_type>()))`.
...
[1]{.pnum} *Mandates:*
  ...
  * [1.1]{.pnum} `decltype(init + ` _`abs-if-needed`_ `(declval<typename InVec::value_type>()))`
      is convertible to `Scalar`,
      if `InVec::value_type`
      is an arithmetic type.

  * [1.2]{.pnum} Otherwise,
      `decltype(init + ` _`abs-if-needed`_ `(` _`real-if-needed`_ `(declval<typename InVec::value_type>())) + ` _`abs-if-needed`_ `(` _`imag-if-needed`_ `(declval<typename InVec::value_type>())))`
      is convertible to `Scalar`.
...
[3]{.pnum} *Remarks:* If `InVec::value_type` is a floating-point type
    or a specialization of `complex`, if `Scalar` is a floating-point type,
    and if `Scalar` has higher precision than `InVec::value_type`,
    then intermediate terms in the sum use `Scalar`'s precision or greater.
...
[1]{.pnum} *Mandates:* `decltype(` _`abs-if-needed`_ `(` _`real-if-needed`_ `(declval<typename InVec::value_type>())) + ` _`abs-if-needed`_ `(` _`imag-if-needed`_ `(declval<typename InVec::value_type>())))`
...
[1]{.pnum} *Mandates:*
`init + ` _`abs-if-needed`_ `(declval<typename InMat::value_type>()) * ` _`abs-if-needed`_ `(declval<typename InMat::value_type>())`
is convertible to `Scalar`.
...
[1]{.pnum} *Mandates:* _`abs-if-needed`_ `(declval<typename InMat::value_type>())`
is convertible to `Scalar`.
...
[2]{.pnum} *Constraints:* if `InMat` has `layout_blas_packed` layout,
then the layout's `Triangle` template argument
has the same type as the function's `Triangle` template argument.

[3]{.pnum} *Mandates:*

  * [3.1]{.pnum} _`possibly-multiplyable`_`<decltype(A), decltype(x), decltype(y)>()` is `true` for those overlaods that take a `x` parameter, and
...
[4]{.pnum} *Preconditions:*
  ...
  * [4.1]{.pnum} _`multiplyable`_`(A,x,y)` is `true` for those overloads that take a `x` parameter, and

  * [4.2]{.pnum} _`addable`_`(x,y,z)` is `true` for those overloads that take a `z` parameter.
...
[1]{.pnum} *Mandates:* _`possibly-multiplyable`_`<InMat,InOutVec,InOutVec>()` is `true`.

[2]{.pnum} *Preconditions:* _`multiplyable`_`(A,y,y)` is `true`.
