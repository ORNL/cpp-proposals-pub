...
`SASUM`, `DASUM`, `CSASUM`, and `DZASUM`.<i>-- end note]</i>
...
[2]{.pnum} *Returns:*
  ...
  * [2.2]{.pnum} Otherwise, the index (in the domain of `v`) of the first element `v_e` of `v` for which `abs(` _`real-if-needed`_ `(v_e)) + abs(` _`imag-if-needed`_ `(v_e))` has the largest value.
