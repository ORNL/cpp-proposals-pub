...
[2]{.pnum} *Mandates:* For all `r` in 0, 1, ..., `x.rank()` - 1,
...
[3]{.pnum} *Preconditions:*
  ...
  * [3.1]{.pnum} `x.extents()` equals `z.extents()`, and

  * [3.2]{.pnum} `y.extents()` equals `z.extents()`.
...
[1]{.pnum} The following mandates and preconditions apply to all functions in [linalg.algs.blas1.dot].
...
[5]{.pnum} Nonconjugated dot product with default result type
...
[6]{.pnum} *Effects:* Let `T` be `decltype(declval<InVec1::value_type>() * declval<InVec2::value_type>())`.
...
[2]{.pnum} *Mandates:*
  ...
  * [2.1]{.pnum} _`possibly-multiplyable`_`<decltype(A), decltype(x), decltype(y)>()` is `true`, and
