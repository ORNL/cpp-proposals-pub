...
### Math helper functions [linalg.math\_helpers]

#### Exposition-only function object _`abs-if-needed`_ [linalg.math\_helpers.abs]
...
#### Exposition-only function object _`conj-if-needed`_ [linalg.math\_helpers.conj]
...
#### Exposition-only function object _`real-if-needed`_ [linalg.math\_helpers.real]
...
#### Exposition-only function object _`imag-if-needed`_ [linalg.math\_helpers.imag]
...
public:
  ...
  constexpr const NestedAccessor& nested_accessor() const;

private:
  ScalingFactor @_scaling-factor_@; // exposition only
  NestedAccessor @_nested-accessor_@; // exposition only
};
...
[2]{.pnum} *Mandates:*
  ...
  * [2.3]{.pnum} `NestedAccessor` meets the accessor policy requirements **[mdspan.accessor.reqmts]**.

  * [2.4]{.pnum} _`scalable`_`<ScalingFactor, typename NestedAccessor::element_type>` is `true`. 
...
[3]{.pnum} *Constraints:*
  ...
  * [3.1]{.pnum} `is_constructible_v<NestedAccessor, const OtherNestedAccessor&>` is `true`, and

  * [3.2]{.pnum} `is_constructible_v<ScalingFactor, const OtherScalingFactor&>` is `true`.
...
public:
  ...
  using offset_policy = conjugate_accessor<NestedAccessor::offset_policy>;

  constexpr conjugate_accessor() = default;
  constexpr conjugate_accessor(const scaled_accessor&) = default;
  constexpr conjugate_accessor(const NestedAccessor& acc);
  template<class OtherScalingFactor, class OtherNestedAccessor>
...
[2]{.pnum} *Mandates:*
  ...
  * [2.2]{.pnum} `NestedAccessor` meets the accessor policy requirements **[mdspan.accessor.reqmts]**.
...
[6]{.pnum} *Returns:*
_`conj-if-needed`_`(NestedAccessor::element_type(`_`nested-accessor`_`.access(p, i)))`
...
[1]{.pnum} *Returns:*
  ...
  * [2.1]{.pnum} `mdspan(a.data_handle(), a.mapping(), a.accessor().nested_accessor())` if `Accessor` is a specialization of `conjugate_accessor`; otherwise

  * [2.2]{.pnum} `mdspan(a.data_handle(), a.mapping(), conjugate_accessor(a.accessor()))`.
