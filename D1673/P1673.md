...
[2]{.pnum} *Constraints:* If `InMat` has `layout_blas_packed` layout, then the
      layout's `Triangle` template argument has the same type as
      the function's `Triangle` template argument.

[3]{.pnum} *Mandates:*

  * [3.1]{.pnum} _`compatible-static-extents`_`<decltype(A), decltype(A)>(0, 1)` is `true`, 

  * [3.2]{.pnum} _`possibly-multiplyable`_`<decltype(A), decltype(x), decltype(y)>()` is `true`, and

  * [3.3]{.pnum} _`possibly-addable`_`<decltype(x),decltype(y),decltype(z)>()` is `true`
                 for those overloads that take a `z` parameter.
...
[4]{.pnum} *Preconditions:*
  ...
  * [4.2]{.pnum} _`multiplyable`_`(A,x,y)` is `true`, and

  * [4.3]{.pnum} _`addable`_`(x,y,z)` is `true` for those overloads that take a `z` parameter.

[5]{.pnum} *Complexity:* $O($ `x.extent(0) * A.extent(1)` $)$
...
[6]{.pnum} This function performs an overwriting symmetric matrix-vector product accessing the elements specified by `t` [linalg.general].

[7]{.pnum} *Effects:* Computes $y = A x$.
...
[8]{.pnum} This function performs an updating symmetric matrix-vector product accessing the elements specified by `t` [linalg.general].

[9]{.pnum} *Effects:* Computes $z = y + A x$.

[10]{.pnum} *Remarks:* `y` and `z` may alias.
...
[6]{.pnum} This function performs an overwriting hermitian matrix-vector product accessing the elements specified by `t` [linalg.general].
...
[8]{.pnum} This function performs an updating hermitian matrix-vector product accessing the elements specified by `t` [linalg.general].
...
[3]{.pnum} *Mandates:*
  ...
  * [3.2]{.pnum} _`compatible-static-extents`_`<decltype(A), decltype(y)>(0, 0)` is `true`, 

  * [3.3]{.pnum} _`compatible-static-extents`_`<decltype(A), decltype(x)>(0, 0)` is `true` for those overloads that take an `x` parameter, and 
  
  * [3.4]{.pnum} _`compatible-static-extents`_`<decltype(A), decltype(z)>(0, 0)` is `true` for those overloads that take a `z` parameter.
...
[4]{.pnum} *Preconditions:*
  ...
  * [4.2]{.pnum} `A.extent(0)` equals `y.extent(0)`,

  * [4.3]{.pnum} `A.extent(0)` equals `x.extent(0)` for those overloads that take an `x` parameter, and

  * [4.4]{.pnum} `A.extent(0)` equals `z.extent(0)` for those overloads that take a `z` parameter.
...
[5]{.pnum} This function performs an overwriting triangular matrix-vector product taking into account the `Triangle` and `DiagonalStorage` parameters [linalg.general].

[6]{.pnum} *Effects:* Computes $y = A x$.

[7]{.pnum} *Complexity:* $O($ `x.extent(0) * A.extent(1)` $)$
...
[8]{.pnum} This function performs an in-place triangular matrix-vector product taking into account the `Triangle` and `DiagonalStorage` parameters [linalg.general].
...
[9]{.pnum} *Effects:* Computes $y = A y$.

[10]{.pnum} *Complexity:* $O($ `y.extent(0)` $*$ `A.extent(1)` $)$
...
[11]{.pnum} This function performs an updating triangular matrix-vector product taking into account the `Triangle` and `DiagonalStorage` parameters [linalg.general].


[12]{.pnum} *Effects:* Computes $z = y + A x$.

[13]{.pnum} *Remarks:* `y` and `z` may alias.

[14]{.pnum} *Complexity:* $O($ `x.extent(0)` $*$ `A.extent(1)` $)$
...
[3]{.pnum} *Mandates:*
  ...
  * [3.2]{.pnum} _`compatible-static-extents`_`<decltype(A), decltype(b)>(0, 0)` is `true`, and

  * [3.3]{.pnum} _`compatible-static-extents`_`<decltype(A), decltype(x)>(0, 0)` is `true` for those overloads that take an `x` parameter.

[4]{.pnum} *Preconditions:*

  * [4.1]{.pnum} `A.extent(0)` equals `A.extent(1)`,

  * [4.2]{.pnum} `A.extent(0)` equals `b.extent(0)`, and

  * [4.3]{.pnum} `A.extent(0)` equals `x.extent(0)` for those overloads that take an `x` parameter.
...
[5]{.pnum} This function performs a triangular solve taking into account the `Triangle` and `DiagonalStorage` parameters [linalg.general].

[6]{.pnum} *Effects:* Computes $x'$ such that $b = A x'$,
...
[10]{.pnum} This function performs an in-place triangular solve taking into account the `Triangle` and `DiagonalStorage` parameters [linalg.general].
...
[2]{.pnum} *Mandates:* _`possibly-multiplyable`_`<InOutMat, InVec2, InVec1>()` is `true`.

[3]{.pnum} *Preconditions:* _`multiplyable`_`(A,y,x)` is `true`.

[4]{.pnum} *Effects:* Computes $A = x y^T$.
...
`matrix_rank_1_update(x, conjugated(y), A);` for the overloads without an `ExecutionPolicy` argument, and
`matrix_rank_1_update(exec, x, conjugated(y), A);` otherwise.

#### Symmetric Rank-1 (outer product) update of a matrix [linalg.algs.blas2.symrank1]
...
[1]{.pnum} The following elements apply to all functions in [linalg.algs.blas2.symrank1].

[2]{.pnum} *Constraints:* If `InOutMat` has `layout_blas_packed` layout, then the
      layout's `Triangle` template argument has the same type as
      the function's `Triangle` template argument.

[3]{.pnum} *Mandates:*

  * [3.1]{.pnum} _`compatible-static-extents`_`<InOutMat, InOutMat>(0,1)` is `true`, and

  * [3.2]{.pnum} _`compatible-static-extents`_`<InOutMat, InVec>(0,0)` is `true`.

[4]{.pnum} *Preconditions:*

  * [4.1]{.pnum} `A.extent(0)` equals `A.extent(1)`, and

  * [4.2]{.pnum} `A.extent(0)` equals `x.extent(0)`.

[5]{.pnum} *Complexity:* $O($ `x.extent(0)` $*$ `x.extent(0)` $)$
...
[6]{.pnum} These functions perform a symmetric rank-1 update taking into account the `Triangle` parameter [linalg.general].

[7]{.pnum} *Effects:* Computes $A'$ such that $A' = A + x x^T$ and assigns each element of $A'$ to the corresponding element of $A$.
...
void symmetric_matrix_rank_1_update(
  ...
  Triangle t);
```
[8]{.pnum} These function performs a symmetric rank-1 update taking into account the `Triangle` parameter [linalg.general].

[9]{.pnum} *Effects:* Computes $A'$ such that
...
[10]{.pnum} These functions perform a Rank-1 update of a Hermitian matrix taking into account the `Triangle` parameter [linalg.general].

[11]{.pnum} *Effects:* Computes $A'$ such that
...
[12]{.pnum} These functions perform a Rank-1 update of a Hermitian matrix taking into account the `Triangle` parameter [linalg.general].

[13]{.pnum} *Effects:* Computes $A'$ such that
...
These functions perform a Rank-2 update of a symmetric matrix taking into account the `Triangle` parameter [linalg.general].

<i>[Note:</i> These functions correspond to the BLAS functions
`xSYR2` and `xSPR2`. <i>-- end note]</i>

[15]{.pnum} *Constraints:* If `InOutMat` has `layout_blas_packed` layout, then the
      layout's `Triangle` template argument has the same type as
      the function's `Triangle` template argument.

[16]{.pnum} *Mandates:*

  * [16.1]{.pnum} _`compatible-static-extents`_`<InOutMat, InOutMat>(0,1)` is `true`, and

  * [16.2]{.pnum} _`compatible-static-extents`_`<InOutMat, InVec>(0,0)` is `true`.

[17]{.pnum} *Preconditions:*

  * [17.1]{.pnum} `A.extent(0)` equals `A.extent(1)`; and

  * [17.2]{.pnum} `A.extent(0)` equals `x.extent(0)`.
