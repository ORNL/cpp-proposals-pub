...
[8]{.pnum} This function performs a symmetric rank-1 update taking into account the `Triangle` parameter [linalg.general].
...
[9]{.pnum} *Constraints:* If `InOutMat` has `layout_blas_packed` layout, then the
      layout's `Triangle` template argument has the same type as
      the function's `Triangle` template argument.

[10]{.pnum} *Mandates:*

  * [10.1]{.pnum} _`compatible-static-extents`_`<InOutMat, InOutMat>(0,1)` is `true`, and

  * [10.2]{.pnum} _`compatible-static-extents`_`<InOutMat, InVec>(0,0)` is `true`.

[11]{.pnum} *Preconditions:*

  * [11.1]{.pnum} `A.extent(0)` equals `A.extent(1)`, and

  * [11.2]{.pnum} `A.extent(0)` equals `x.extent(0)`.

[12]{.pnum} *Effects:*

  * [12.1]{.pnum} Overloads without an `alpha` parameter
      compute $A = A + x x^T$.

  * [12.2]{.pnum} Overloads with an `alpha` parameter
      compute $A = A + \alpha x x^T$,
      where the scalar $\alpha$ is `alpha`.

[13]{.pnum} *Complexity:* $O($ `x.extent(0)` $*$ `x.extent(0)` $)$
...
[14]{.pnum} These functions perform a Rank-1 update of a Hermitian matrix taking into account the `Triangle` parameter [linalg.general].
...
[18]{.pnum} *Effects:*
  ...
  * [18.1]{.pnum} Overloads without an `alpha` parameter
      compute $A = A + x x^H$.

  * [18.2]{.pnum} Overloads with an `alpha` parameter
      compute $A = A + \alpha x x^H$,
      where the scalar $\alpha$ is `alpha`.

[19]{.pnum} *Complexity:* $O($ `x.extent(0)` $*$ `x.extent(0)` $)$
