...
[7]{.pnum} `Layout` shall meet the `mdspan` layout mapping policy
requirements.
...
[1]{.pnum} *Mandates:*
  ...
  * [1.1]{.pnum} `T` meets both the *Cpp17MoveConstructible* and
    *Cpp17LessThanComparable* requirements, and

  * [1.2]{.pnum} `abs(x[0])` is convertible to `T`.
...
[1]{.pnum} *Mandates:* `init +
abs(declval<InVec::value_type>())*abs(declval<InVec::value_type>())` is
convertible to `T`.
...
[1]{.pnum} *Mandates:* For `i` in the domain of `v`, let `abs_value_type` be
`decltype(v[i])`.  `abs_value_type` meets the *Cpp17LessThanComparable*
requirements.
...
[1]{.pnum} *Mandates:* `init +
abs(declval<InMat::value_type>())*abs(declval<InMat::value_type>())` is
convertible to `T`.
...
[1]{.pnum} *Mandates:* `abs(A[0,0])` is convertible to `T`.
...
<i>[Note:</i> One can derive what these algorithms do
