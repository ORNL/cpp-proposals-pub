...
Section <a href="#implementation-experience"><span class="secno">14</span></a>
...
givens_rotation_setup(const Real a,
                      const Real b);
template<class Real>
givens_rotation_setup_result<complex<Real>>
givens_rotation_setup(const complex<Real>& a,
...
where
  ...
  * [2.2]{.pnum} `ReturnLayout` is:
    ...
    * [2.2.7]{.pnum} else if `Layout` is `layout_transpose<NestedLayout>`
        for some `NestedLayout`, then `NestedLayout`;
...
`return vector_norm2(v, T{});`,
and the two-parameter overload is equivalent to
`return vector_norm2(exec, v, T{});`.
...
avoid ambiguity with `ExecutionPolicy`. 
-->
