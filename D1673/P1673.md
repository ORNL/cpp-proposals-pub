...
[2]{.pnum} *Mandates:* _`conjugatable`_`<ReferenceValue>`.

[3]{.pnum} `Reference` models `copy_constructible`.

```c++
using value_type = /* see below */;
```

[4]{.pnum} `value_type` names the type
...
[5]{.pnum} *Effects:* Initializes _`base-type`_ with `reference`.

```c++
constexpr static auto to_value(Reference value_type);
```

[6]{.pnum} *Preconditions:* This function may be invoked arbitrarily many times.
...
[3]{.pnum} If an algorithm in *[linalg.algs]* accesses the elements
