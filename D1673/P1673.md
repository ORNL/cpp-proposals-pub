...
constexpr bool @_possibly-multipliable_@() {
...
[3]{.pnum} *Mandates:*
  ...
  * [3.2]{.pnum} _`possibly-multipliable`_`<decltype(A), decltype(x), decltype(y)>()` is `true`, and
...
[4]{.pnum} *Preconditions:*
  ...
  * [4.2]{.pnum} _`multipliable`_`(A,x,y)` is `true`, and
...
[6]{.pnum} *Effects:* Computes a vector $x'$ such that $b = A x'$,
...
[3]{.pnum} *Mandates:*
  ...
  * [3.1]{.pnum} _`compatible-static-extents`_`<decltype(A), decltype(A)>(0,1)` is `true`, and

  * [3.2]{.pnum} _`compatible-static-extents`_`<decltype(A), decltype(x)>(0,0)` is `true`.
...
[7]{.pnum} *Effects:* Computes a matrix $A'$ such that $A' = A + x x^T$ and assigns each element of $A'$ to the corresponding element of $A$.
...
[9]{.pnum} *Effects:* Computes a matrix $A'$ such that
...
[11]{.pnum} *Effects:* Computes a matrix $A'$ such that
...
[4]{.pnum} *Preconditions:*
  ...
  * [4.2]{.pnum} _`multipliable`_`(A,x,y)` is `true`.
...
[1]{.pnum} The following elements apply
to all functions in [linalg.algs.blas3.xmm] in addition to function specific alements.
...
[5]{.pnum} These functions perform a matrix-matrix multiply, taking into account the `Triangle` and `DiagonalStorage` parameter where applicable [linalg.general].
...
[10]{.pnum} These functions perform a matrix-matrix multiply, taking into account the `Triangle` and `DiagonalStorage` parameter where applicable [linalg.general].
...
[15]{.pnum} These functions perform a potentially overwriting matrix-matrix multiply-add, taking into account the `Triangle` and `DiagonalStorage` parameter where applicable [linalg.general].
`A` is the symmetric, Hermitian or triangular matrix respectively.
...
[21]{.pnum} These functions perform a potentially overwriting matrix-matrix multiply-add, taking into account the `Triangle` and `DiagonalStorage` parameter where applicable [linalg.general].
`B` is the symmetric, Hermitian or triangular matrix respectively.
...
[3]{.pnum} *Mandates:* 
   ...
   * [3.1]{.pnum} _`possibly-multipliable`_`<InMat, InOutMat, InOutMat>()` is `true`, and

   * [3.2]{.pnum} _`compatible-static-extents`_`<InMat,InMat>(0,1)`

[4]{.pnum} *Preconditions:* 

   * [4.1]{.pnum} _`multipliable`_`(A, C, C)` is `true`, and

   * [4.2]{.pnum} `A.extent(0) == A.extent(1)` is `true`.

[5]{.pnum} *Effects:* Computes a matrix $C'$ such that $C' = A C$ and assigns each element of $C'$ to the corresponding element of `C`.

[6]{.pnum} *Complexity:* $O($ `A.extent(0) * A.extent(1) * C.extent(0)` $)$
...
[7]{.pnum} *Constraints:* If `InMat` has `layout_blas_packed` layout, then the
      layout's `Triangle` template argument has the same type as
      the function's `Triangle` template argument.

[8]{.pnum} *Mandates:*
 
   * [8.1]{.pnum} _`possibly-multipliable`_`<InOutMat, InMat, InOutMat>()` is `true`, and

   * [8.2]{.pnum} _`compatible-static-extents`_`<InMat, InMat>(0,1)`

[9]{.pnum} *Preconditions:* 

   * [9.1]{.pnum} _`multipliable`_`(C, A, C)` is `true`, and

   * [9.2]{.pnum} `A.extent(0) == A.extent(1)` is `true`.

[10]{.pnum} *Effects:* Computes a matrix $C'$ such that $C' = C A$ and assigns each element of $C'$ to the corresponding element of `C`.

[11]{.pnum} *Complexity:* $O($ `A.extent(0) * A.extent(1) * C.extent(0)` $)$
...
[3]{.pnum} *Mandates:*
  ...
  * [3.2]{.pnum} _`compatible-static-extents`_`<decltype(C), decltype(C)>(0, 1)` is `true`, and 
...
[3]{.pnum} *Mandates:*
  ...
  * [3.1]{.pnum} _`possibly-addable`_`<decltype(A), decltype(B), decltype(C)>()` is `true`, and
...
[4]{.pnum} *Preconditions:*
  ...
  * [4.1]{.pnum} _`addable`_`(A, B, C)` is `true`, 

  * [4.2]{.pnum} `A.extent(0)` equals `A.extent(1)`.
...
[7]{.pnum} *Effects:* Computes a matrix $C'$ such that $C' = C + A B^T + B A^T$, and assigns each element of $C'$ to the corresponding element of $C$. 
...
[1]{.pnum} These functions perform multiple matrix solves, taking into account the `Triangle` and `DiagonalStorage` parameter where applicable [linalg.general].
`A` is the triangular matrix.

[2]{.pnum} *Constraints:* If `InMat1` has `layout_blas_packed` layout, then the
      layout's `Triangle` template argument has the same type as
      the function's `Triangle` template argument.

[3]{.pnum} *Mandates:*

  * [3.1]{.pnum} _`possibly-multipliable`_`<InMat1, OutMat, InMat2>` is `true`.

  * [3.2]{.pnum} _`compatible-static-extents`_`<InMat1, InMat1>(0,1)` is `true`,

[4]{.pnum} *Preconditions:*

  * [4.1]{.pnum} _`multipliable`_`(A,X,B)` is `true`, and
...
[6]{.pnum} *Complexity:* $O($ `A.extent(0) * X.extent(1) * X.extent(1)` $)$
...
triangular_matrix_matrix_left_solve(A, t, d, B, X,
  [](const auto& A_ij, const auto& X_ij) { return A_ij/X_ij; })
```
...
[8]{.pnum} *Effects:* Equivalent to:
```c++
triangular_matrix_matrix_left_solve(std::forward<ExecutionPolicy>(exec), A, t, d, B, X,
  [](const auto& A_ij, const auto& X_ij) { return A_ij/X_ij; })
```
...
[9]{.pnum} These functions perform multiple matrix solves, taking into account the `Triangle` and `DiagonalStorage` parameter where applicable [linalg.general].
`A` is the triangular matrix.

[10]{.pnum} *Constraints:* If `InMat1` has `layout_blas_packed` layout, then the
      layout's `Triangle` template argument has the same type as
      the function's `Triangle` template argument.

[11]{.pnum} *Mandates:*

  * [11.1]{.pnum} _`possibly-multipliable`_`<OutMat, InMat1, InMat2>` is `true`.

  * [11.2]{.pnum} _`compatible-static-extents`_`<InMat1, InMat1>(0,1)` is `true`,

[12]{.pnum} *Preconditions:*

  * [12.1]{.pnum} _`multipliable`_`(X,A,B)` is `true`, and

  * [12.2]{.pnum} `A.extent(0) == A.extent(1)` is `true`.
...
[14]{.pnum} *Complexity:* $O($ `B.extent(0) * B.extent(1) * A.extent(1)` $)$
...
```c++
triangular_matrix_matrix_right_solve(std::forward<ExecutionPolicy>(exec), A, t, d, B, X,
  [](const auto& A_ij, const auto& X_ij) { return A_ij/X_ij; })
```

#### Solve multiple triangular linear systems in-place [linalg.alg.blas3.inplacetrsm]
...
[1]{.pnum} These functions perform multiple in-place matrix solves, taking into account the `Triangle` and `DiagonalStorage` parameter where applicable [linalg.general].
`A` is the triangular matrix.
...
[3]{.pnum} *Mandates:*
  ...
  * [3.1]{.pnum} _`possibly-multipliable`_`<InMat, InOutMat, InOutMat>` is `true`.

  * [3.2]{.pnum} _`compatible-static-extents`_`<InMat, InMat>(0,1)` is `true`,

[4]{.pnum} *Preconditions:*

  * [4.1]{.pnum} _`multipliable`_`(A,B,B)` is `true`, and
...
[6]{.pnum} *Complexity:* $O($ `A.extent(0) * A.extent(1) * B.extent(1)` $)$
...
triangular_matrix_matrix_left_solve(A, t, d, B,
  [](const auto& A_ij, const auto& B_ij) { return A_ij/B_ij; })
```
...
[8]{.pnum} *Effects:* Equivalent to:
```c++
triangular_matrix_matrix_left_solve(std::forward<ExecutionPolicy>(exec), A, t, d, B,
  [](const auto& A_ij, const auto& B_ij) { return A_ij/B_ij; })
```
...
[9]{.pnum} These functions perform multiple in-place matrix solves, taking into account the `Triangle` and `DiagonalStorage` parameter where applicable [linalg.general].
`A` is the triangular matrix.
...
[10]{.pnum} *Constraints:* If `InMat` has `layout_blas_packed` layout, then the
      layout's `Triangle` template argument has the same type as
      the function's `Triangle` template argument.

[11]{.pnum} *Mandates:*

  * [11.1]{.pnum} _`possibly-multipliable`_`<InOutMat, InMat, InOutMat>` is `true`.

  * [11.2]{.pnum} _`compatible-static-extents`_`<InMat, InMat>(0,1)` is `true`,

[12]{.pnum} *Preconditions:*

  * [12.1]{.pnum} _`multipliable`_`(B,A,B)` is `true`, and
...
[14]{.pnum} *Complexity:* $O($ `A.extent(0) * A.extent(1) * B.extent(1)` $)$
...
[16]{.pnum} *Effects:* Equivalent to:
```c++
triangular_matrix_matrix_right_solve(std::forward<ExecutionPolicy>(exec), A, t, d, B,
  [](const auto& A_ij, const auto& B_ij) { return A_ij/B_ij; })
```
