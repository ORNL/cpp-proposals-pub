...
<i>[Note:</i> These functions correspond to the BLAS function `xSYMM` and `xHEMM`. The symmetric or Hermitian matrix is given by the argument following the `Triangle` parameter.
<i>-- end note]</i>
...
[5]{.pnum} These functions perform an overwriting matrix-matrix multiply, taking into account the `Triangle` parameter [linalg.general]. `A` is the symmetric or Hermitian matrix respectively.

[6]{.pnum} *Mandates:* _`compatible-static-extents`_`<InMat1,InMat1>(0,1)`
...
[9]{.pnum} These functions perform an overwriting matrix-matrix multiply, taking into account the `Triangle` parameter [linalg.general]. `B` is the symmetric or Hermitian matrix respectively.

[10]{.pnum} *Mandates:* _`compatible-static-extents`_`<InMat2,InMat2>(0,1)`
...
[13]{.pnum} These functions perform an updating matrix-matrix multiply, taking into account the `Triangle` parameter [linalg.general]. `A` is the symmetric or Hermitian matrix respectively.

[14]{.pnum} *Mandates:* _`compatible-static-extents`_`<InMat1,InMat1>(0,1)`

[15]{.pnum} *Preconditions:* `A.extent(0) == A.extent(1)` is `true.

[16]{.pnum} *Effects:* Computes $C = E + A B$.

[17]{.pnum} *Remarks:* `E` may alias `C`.
...
[18]{.pnum} These functions perform an updating matrix-matrix multiply, taking into account the `Triangle` parameter [linalg.general]. `B` is the symmetric or Hermitian matrix respectively.

[19]{.pnum} *Mandates:* _`compatible-static-extents`_`<InMat2,InMat2>(0,1)`

[20]{.pnum} *Preconditions:* `B.extent(0) == B.extent(1)` is `true.

[21]{.pnum} *Effects:* Computes $C = E + A B$.

[22]{.pnum} *Remarks:* `E` may alias `C`.
