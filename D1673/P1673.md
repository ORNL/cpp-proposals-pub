...
[6]{.pnum} *Preconditions:*
  ...
  * [6.1]{.pnum} The size of the multidimensional index space `e`
...
[9]{.pnum} *Preconditions:* `other.required_span_size()` is representable as a value
...
[12]{.pnum} *Constraints*:
  ...
  * [12.1]{.pnum} `sizeof...(Indices)` equals `extents_type::rank()` is `true`,

  * [12.2]{.pnum} `(is_convertible_v<Indices, index_Type> && ...)` is `true`, and

  * [12.3]{.pnum} `(is_nothrow_constructible_v<index_type, Indices>)` is `true`.

[13]{.pnum} *Preconditions:* `extents_type::`_`index-cast`_`(k)` is
    a multidimensional index in _`extents_`_ ([mdspan.overview]).

[14]{.pnum} *Returns:* Let `N` equal `extent(0)`,
...
[17]{.pnum} *Constraints:* `extents_type::rank() == OtherExtents::rank()` is `true`.

[18]{.pnum} *Effects:* Equivalent to: `return x.extents() == y.extents();`
