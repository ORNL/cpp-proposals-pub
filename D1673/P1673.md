...
`vector_abs_sum` and `idx_abs_max`
...
Computing the magnitude is equivalent to two-argument `hypot`.
...
and `idx_abs_max` (vector infinity-norm) functions
...
and `idx_abs_max` do not include the word "norm."
...
// [linalg.scaled.abs], exposition-only function object @_abs-if-needed_@

/* see-below */ @_abs-if-needed_@;

// [linalg.scaled.conj], exposition-only function object @_conj-if-needed_@

/* see-below */ @_conj-if-needed_@;

// [linalg.scaled.real], exposition-only function object @_real-if-needed_@

/* see-below */ @_real-if-needed_@;

// [linalg.scaled.imag], exposition-only function object @_imag-if-needed_@

/* see-below */ @_imag-if-needed_@;
...
// [linalg.scaled.scaled_accessor], class template scaled_accessor
...
// [linalg.conj.conjugate_accessor], class template conjugate_accessor
...
template<class Real>
givens_rotation_setup_result<Real>
givens_rotation_setup(Real a, Real b) noexcept;
template<class Real>
givens_rotation_setup_result<complex<Real>>
givens_rotation_setup(complex<Real> a, complex<Real> b) noexcept;
...
Scalar vector_two_norm(ExecutionPolicy&& exec,
                       ...
                       Scalar init);
template<@_in-vector_@ InVec>
auto vector_two_norm(InVec v) -> /* see-below */;
template<class ExecutionPolicy,
         @_in-vector_@ InVec>
...
the following apply:
  ...
  * [1.4]{.pnum} When $R$ is the same name as a function parameter
      ...
      <!-- FIXME (mfh 2023/02/16) or with `out` in its constraint? -->
      the intent is that the result of the computation
      is written to the elements of the function parameter `R`.

[2]{.pnum} Some of the functions in [linalg] distinguish between
the "upper triangle," "lower triangle," and "diagonal" of a matrix.

  * [2.1]{.pnum} The *diagonal* is the set of all elements of `A` accessed by
      `A[i,i]` for 0 ≤ `i` < min(`A.extent(0)`, `A.extent(1)`).

  * [2.2]{.pnum} The *upper triangle* of a matrix `A` is the set of all elements of
      `A` accessed by `A[i,j]` with `i` ≤ `j`.  It includes the diagonal.

  * [2.3]{.pnum} The *lower triangle* of `A` is the set of all elements of `A`
      accessed by `A[i,j]` with `i` ≥ `j`.  It includes the diagonal.

[3]{.pnum} For any function `F` that takes
a `Triangle t` parameter,
`t` applies to accesses to
the previous parameter to the function.
Let `M` be such an access-modified function parameter.
`F` will only access the triangle of `M` specified by `t`.
For accesses `M[i, j]` in the domain of `M`
but outside the triangle specified by `t`,
`F` will interpret the value `M[i, j]`

  * as equal to _`conj-if-needed`_`(M[j, i])`
      if the name of `F` starts with `hermitian`,

  * as equal to `M[j, i]`
      if the name of `F` starts with `symmetric`, or

  * as equal to zero
      if the name of `F` starts with `triangular`.

[4]{.pnum} For any function `F` that takes
a `DiagonalStorage d` parameter,
`d` applies to accesses to
the previous-of-the-previous parameter to the function.
Let `M` be such an access-modified function parameter.
...
and for any pack of integers `i`
which is a multidimensional index in `e`,
`x[i...]` and `y[i...]` refer to the same element.
<i>[Note:</i> EDITORIAL:
This definition may more appropriately belong in
*[mdspan.mdspan.overview]* paragraph 6.
However, it's currently only used in this proposal.
<i>-- end note]</i>
...
we refer to the following types as *linear algebra value types*:
...
[2]{.pnum} Linear algebra value types
must meet the requirements of `semiregular`.
...
[1]{.pnum} [linalg.reqs.alg] lists the minimum requirements for all algorithms and classes in [linalg].

[2]{.pnum} All of the following elements presume that the algorithm's
asymptotic complexity requirements, if any, are satisfied.

* [2.1]{.pnum} The function may perform read-only access
    on any input or output `mdspan` arbitrarily many times.

* [2.2]{.pnum} The function may make
    arbitrarily many objects
    of any linear algebra value type,
    value-initializing or direct-initializing them
    with any existing object of that type.

* [2.3]{.pnum} The function may assign
    arbitrarily many times to any reference
    resulting from a valid output `mdspan` access.

* [2.4]{.pnum} The *triangular solve algorithms* in **[linalg.algs]**
...
* [2.5]{.pnum} Otherwise, the function
...
* [2.6]{.pnum} If the function has an output `mdspan`,
    then all addends,
...
* [2.7]{.pnum} The function may reorder
...
`layout_blas_packed` (*[linalg.layouts.packed]*) layout.
...
an explicit diagonal is to be assumed (*[linalg.general]*).
...
### Exposition-only function object _`abs-if-needed`_ [linalg.scaled.abs]
...
### Exposition-only function object _`real-if-needed`_ [linalg.scaled.real]
...
is expression-equivalent to:
  ...
  * [1.2]{.pnum} else, `real(E)`,
      if that expression is valid
      with overload resolution performed in a context
	    that includes the declaration
	    `template<class T> T real(const T&) = delete;`
	    and does not include a declaration of
	    `linalg::`_`real-if-needed`_;
	    if the function selected by overload resolution
...
### Exposition-only function object _`imag-if-needed`_ [linalg.scaled.imag]
...
is expression-equivalent to:
  ...
  * [1.2]{.pnum} else, `imag(E)`, if that expression is valid
      with overload resolution performed in a context
	    that includes the declaration
	    `template<class T> T imag(const T&) = delete;`
	    and does not include a declaration of
	    `linalg::`_`imag-if-needed`_;
	    if the function selected by overload resolution
...
_`scaled-scalar`_ **[linalg.scaled.scaled_accessor]** or
_`conjugated-scalar`_ **[linalg.scaled.conjugate_accessord]**.

[2]{.pnum} The exposition-only class template _`proxy-reference`_
is part of the implementation of 
_`scaled-scalar`_ **[linalg.scaled.scaled_accessor]** and
_`conjugated-scalar`_ **[linalg.scaled.conjugate_accessord]**.
...
[3]{.pnum} *Returns:* _`abs-if-needed`_ `(value_type(static_cast<const this_type&>(x)))`

```c++
constexpr friend auto real(const derived_type& x);
```

[4]{.pnum} *Returns:* _`real-if-needed`_ `(value_type(static_cast<const this_type&>(x)))`

```c++
constexpr friend auto imag(const derived_type& x);
```

[5]{.pnum} *Returns:* _`imag-if-needed`_ `(value_type(static_cast<const this_type&>(x)))`

```c++
constexpr friend auto conj(const derived_type& x);
```

[6]{.pnum} *Returns:* _`conj-if-needed`_ `(value_type(static_cast<const this_type&>(x)))`
...
_`scaled-scalar`_ is part of the implementation of `scaled_accessor`
**[linalg.scaled.scaled_accessor]**.
...
### Class template `scaled_accessor` [linalg.scaled.scaled_accessor]
...
class scaled_accessor {
public:
  using element_type = add_const_t<typename Accessor::element_type>;
  using reference = decltype(declval<ScalingFactor>() * declval<element_type>());
  using data_handle_type = Accessor::data_handle_type;
  using offset_policy = scaled_accessor<ScalingFactor, Accessor::offset_policy>;

  constexpr scaled_accessor() = default;
  constexpr scaled_accessor(const scaled_accessor&) = default;
  ...
  constexpr const Accessor& nested_accessor() const;
...
[2]{.pnum} *Mandates:*
  ...
  * [2.4]{.pnum} _`scalable`_`<ScalingFactor, element_type>` is `true`. 
...
[3]{.pnum} *Constraints:*
  ...
  * [3.1]{.pnum} `is_constructible_v<`_`nested-accessor`_`, const OtherNestedAccessor&>` is `true`, and

  * [3.2]{.pnum} `is_constructible_v<ScalingFactor, OtherScalingFactor>` is `true`.

[3]{.pnum} *Effects:*

  * [3.1]{.pnum} Direct non-list-initializes _`scaling-factor`_ with `other.scaling_factor()`, and

  * [3.2]{.pnum} direct non-list-initializes _`nested-accessor`_ with `other.nested_accessor()`.
...
[4]{.pnum} *Effects:*
  ...
  * [4.1]{.pnum} Direct non-list-initializes _`scaling-factor`_ with `s`, and

  * [4.2]{.pnum} direct non-list-initializes _`nested-accessor`_ with `a`.

```c++
constexpr reference access(data_handle_type p, size_t i) const noexcept;
```

[5]{.pnum} *Returns:* `scaling_factor() * Accessor::element_type(` _`nested-accessor`_`.access(p, i))`
...
[6]{.pnum} *Returns:* _`nested-accessor`_`.offset(p, i)`

```c++
constexpr ScalingFactor scaling_factor() const;
```

[7]{.pnum} *Returns:* _`scaling-factor`_

```c++
constexpr Accessor nested_accessor() const;
```

[8]{.pnum} *Returns:* _`nested-accessor`_
...
```
[2]{.pnum} *Returns:*
`mdspan{x.data(), x.mapping(), scaled_accessor{alpha, x.accessor()}}`
...
_`conjugated-scalar`_ is part of the implementation of `conjugate_accessor`
**[linalg.conj.conjugate_accessor]**.
...
[5]{.pnum} *Returns:*
_`conj-if-needed`_`(ReferenceValue(reference))`

### Class template `conjugate_accessor` [linalg.conj.conjugate_accessor]

[1]{.pnum} The class template `conjugate_accessor`
...
public:
  ...
  constexpr conjugate_accessor(Accessor a);
...
<i>[Note:</i> `conjugate_accessor` provides read-only access. <i>-- end note]</i>
...
* [6.2]{.pnum} otherwise, `conjugate_accessor<typename Accessor::offset_policy>`.
...
[8]{.pnum} *Returns:*
`reference(`_`nested-accessor`_`.access(p, i))`
...
[9]{.pnum} *Returns:*
_`nested-accessor`_`.offset(p, i)`
...
where
  ...
  * [1.1]{.pnum} `ReturnElementType` is
    ...
    * [1.1.1]{.pnum} if `Accessor` is
        `conjugate_accessor<NestedAccessor>` for some `NestedAccessor`,
    ...
    * [1.1.3]{.pnum} else `conjugate_accessor<Accessor>::element_type`;
  ...
  * [1.2]{.pnum} and `ReturnAccessor` is:
    ...
    * [1.2.3]{.pnum} else `conjugate_accessor<Accessor>`.

[2]{.pnum} *Returns:*

  * [2.1]{.pnum} `R(a.data(), a.mapping(), a.nested_accessor())`
      if `Accessor` is `conjugate_accessor<NestedAccessor>`.

  * [2.2]{.pnum} Otherwise,
      `R(a.data(), a.mapping(), a.accessor())`
      if `remove_cv_t<ElementType>` is an arithmetic type.

  * [2.3]{.pnum} Otherwise,
      `R(a.data(), a.mapping(), ReturnAccessor(a.accessor()))`.
...
that swaps the rightmost two indices, extents, and strides
...
[3]{.pnum} *Constraints:* `InputExtents::rank()` equals 2.

[4]{.pnum} *Mandates:* `InputExtents` is a specialization of `extents`.
...
[10]{.pnum} *Returns:*
_`transpose-extents`_ `(` _`nested_mapping`_ `.extents())`
...
[11]{.pnum} *Returns:*
_`nested-mapping`_ `.required_span_size()`
...
[12]{.pnum} *Returns:*
_`nested-mapping`_ `(last_2_indices_reversed)`,
...
[14]{.pnum} *Returns:*
_`nested-mapping-type`_ `::is_always_unique()`
...
[15]{.pnum} *Returns:*
_`nested-mapping-type`_ `::is_always_contiguous()`
...
[16]{.pnum} *Returns:*
_`nested-mapping-type`_ `::is_always_strided()`
...
[18]{.pnum} *Returns:*
_`nested-mapping`_ `.is_contiguous()`
...
[21]{.pnum} *Returns:*
  ...
  * [21.1]{.pnum} _`nested-mapping`_ `.stride(rank() - 2)`
      if `r` equals `rank()` - 1.

  * [21.2]{.pnum} Otherwise, _`nested-mapping`_ `.stride(rank() - 1)`
      if `r` equals `rank()` - 2.

  * [21.3]{.pnum} Otherwise, _`nested-mapping`_ `.stride(r)`.
...
[22]{.pnum} *Constraints:*
  ...
  * [22.1]{.pnum} _`nested-mapping-type`_ and
    `decltype(m.` _`nested-mapping`_ `)`
    are equality comparable, and
...
[2]{.pnum} Let `ReturnExtents` be
_`transpose-extents-t`_`<Extents>`.
Let `R` be
`mdspan<ElementType, ReturnExtents, ReturnLayout, Accessor>`,
where `ReturnLayout` is:

  * [2.1]{.pnum} `layout_right`
    if `Layout` is `layout_left`.

  * [2.2]{.pnum} Otherwise, `layout_left`
    if `Layout` is `layout_right`.

  * [2.3]{.pnum} Otherwise, `layout_stride`
    if `Layout` is `layout_stride`.

  * [2.4]{.pnum} Otherwise, `layout_right_padded<padding_stride>`
    if `Layout` is `layout_left_padded<padding_stride>`
    for some `padding_stride`.

  * [2.5]{.pnum} Otherwise, `layout_left_padded<padding_stride>`
  ...
  * [2.7]{.pnum} Otherwise, `NestedLayout`
    ...
    as long as there are no intervening layout changes -->

  * [2.8]{.pnum} Otherwise, `layout_transpose<Layout>`.
...
[3]{.pnum} *Returns:*
  ...
  * [3.2]{.pnum} Otherwise,
      `R(a.data(), ReturnMapping(` _`transpose-extents`_ `(a.mapping().extents()), a.mapping().stride(1)), a.accessor())`
      if `Layout` is a specialization of `layout_left_padded`.

  * [3.3]{.pnum} Otherwise,
      `R(a.data(), ReturnMapping(` _`transpose-extents`_ `(a.mapping().extents()), a.mapping().stride(0)), a.accessor())`
...
### Helpers for algorithm mandates
...
requires(@_is-mdspan_@<MDS1>::value && @_is-mdspan_@<MDS2>::value) 
constexpr bool @_compatible-static-extents_@(size_t r1, size_t r2) {
  return MDS1::static_extent(r1) == dynamic_extent ||
         MDS2::static_extent(r2) == dynamic_extent || 
         MDS1::static_extent(r1) == MDS::static_extent(r2));
}
```

```c++
template<@_in-vector_@ In1, @_in-vector_@ In2, @_in-vector_@ Out>
constexpr bool @_possibly-addable_@() {
...
constexpr bool @_possibly-addable_@() {
  return @_compatible-static-extents_@<Out, In1>(0, 0) &&
         ...
         @_compatible-static-extents_@<On1, In2>(0, 0) &&
         @_compatible-static-extents_@<On1, In2>(1, 1);
}
```

```c++
template<@_in-matrix_@ InMat, @_in-vector_@ InVec, @_in-vector_@ OutVec>
constexpr bool @_possibly-multiplyable_@() {
...
```c++
template<@_in-vector_@ InVec, @_in-matrix_@ InVec, @_in-vector_@ OutVec>
constexpr bool @_possibly-multiplyable_@() {
...
### Checks for algorithm preconditions
...
```c++
constexpr bool @_addable_@(
    const @_in-vector_@ auto& in1, const @_in-vector_@ auto& in2, const @_in-vector_@ auto& out) {
...
```c++
constexpr bool @_addable_@(
    const @_in-matrix_@& in1, const @_in-matrix_@ auto& in2, const @_in-matrix_@ auto& out) {
  return out.extent(0) == in1.extent(0) &&
...
```c++
constexpr bool @_multiplyable_@(
    const @_in-matrix_@& in_mat, const @_in-vector_@ in_vec, const @_in-vector_@& out_vec) {
...
```c++
constexpr bool @_multiplyable_@(
    const @_in-vector_@& in_vec, const @_in-matrix_@ in_mat, const @_in-vector_@& out_vec) {
...
```c++
constexpr bool @_multiplyable_@(
    const @_in-matrix_@& in_mat1, const @_in-matrix_@ in_mat2, const @_in-matrix_@& out_mat) {
...
[1]{.pnum} Throughout [linalg.algs], where the template parameters are not
...
requirements.
  ...
  * [1.1]{.pnum} Algorithms that have a template parameter
      ...
      **[algorithms.parallel.defns]**.
      <i>[Note:</i> EDITORIAL:
      Should we hoist this wording so that it applies
      to both this proposal and
      the existing parallel algorithms? <i>-- end note]</i>
...
and `c*c + ` _`abs-if-needed`_ `(s) * ` _`abs-if-needed`_ `(s)` equals one.
...
where `c` and `s` form the plane (Givens) rotation
...
[1]{.pnum} *Preconditions:* `x.extents()` equals `y.extents()`.
...
[1]{.pnum} *Preconditions:*
  ...
  * [1.1]{.pnum} `x.extents()` equals `z.extents()`, and

  * [1.2]{.pnum} `y.extents()` equals `z.extents()`.
...
[2]{.pnum} *Preconditions:* `v1.extents()` equals `v2.extents()`.
...
[4]{.pnum} *Returns:* Let `N` be `v1.extent(0)`.

  * [4.1]{.pnum} `init` if `N` is zero.

  * [4.2]{.pnum} Otherwise,
      *GENERALIZED_SUM*(`plus<>()`, `init`, `v1[0]*v2[0]`, ..., `v1[N-1]*v2[N-1]`).

[5]{.pnum} *Remarks:*
If `InVec1::value_type`, `InVec2::value_type`, and `T`
are all floating-point types or specializations of `complex`,
and if `T` has higher precision
...
[7]{.pnum} *Effects:* Let `T` be `decltype(declval<InVec1::value_type>() * declval<InVec2::value_type>())`.
Then, the two-parameter overload is equivalent to
`return dot(v1, v2, T{});`,
and the three-parameter overload is equivalent to
`return dot(exec, v1, v2, T{});`.
...
equivalent to `return dotc(v1, v2, T{});`, and the three-parameter overload
is equivalent to `return dotc(exec, v1, v2, T{});`.
...
[1]{.pnum} *Mandates:*
  ...
  * [1.2]{.pnum} _`abs-if-needed`_ `(declval<InVec::value_type>())` is convertible to `T`.

[2]{.pnum} *Effects:* Returns two values:

  * [2.1]{.pnum} `scaling_factor`: the maximum of `init.scaling_factor`
      and _`abs-if-needed`_ `(x[i])` for all `i` in the domain of `v`; and

  * [2.2]{.pnum} `scaled_sum_of_squares`: a value such that
...
[3]{.pnum} *Remarks:* If `InVec::value_type` is either a floating-point type
...
[1]{.pnum} *Mandates:*
`init + ` _`abs-if-needed`_ `(declval<InVec::value_type>()) * ` _`abs-if-needed`_ `(declval<InVec::value_type>())`
is convertible to `T`.

[2]{.pnum} *Returns:* The square root of the sum of squares
of (`init` and the absolute values of the elements of `v`).
...
[3]{.pnum} *Remarks:* If `InVec::value_type` is a floating-point type
  ...
  * [3.2]{.pnum} any guarantees regarding overflow and underflow
      of `vector_two_norm` are implementation-defined.

<i>[Note:</i> A suggested implementation of this function
...
[1]{.pnum} *Effects:* Let `T` be `decltype(` _`abs-if-needed`_ `(declval<InVec::value_type>()) * ` _`abs-if-needed`_ `(declval<InVec::value_type>()))`.
...
`return vector_two_norm(exec, v, T{});`.
...
[1]{.pnum} *Mandates:*
  ...
  * [1.1]{.pnum} `decltype(init + ` _`abs-if-needed`_ `(declval<InVec::value_type>()))`
      is convertible to `T`,
      if `InVec::value_type`
      is an arithmetic type.

  * [1.2]{.pnum} Otherwise,
      `decltype(init + ` _`abs-if-needed`_ `(` _`real-if-needed`_ `(declval<InVec::value_type>())) + ` _`abs-if-needed`_ `(` _`imag-if-needed`_ `(declval<InVec::value_type>())))`
      is convertible to `T`.
...
and the two-parameter overload is equivalent to
`return vector_abs_sum(exec, v, T{});`.
...
[1]{.pnum} *Mandates:*
  ...
  * [1.1]{.pnum} `InVec::value_type` is *Cpp17LessThanComparable*,
      if `InVec::value_type`
      is an arithmetic type.

  * [1.2]{.pnum} Otherwise,
      `decltype(` _`abs-if-needed`_ `(` _`real-if-needed`_ `(declval<InVec::value_type>())) + ` _`abs-if-needed`_ `(` _`imag-if-needed`_ `(declval<InVec::value_type>())))`
      is *Cpp17LessThanComparable*.

[2]{.pnum} *Returns:*

  * [2.1]{.pnum} `numeric_limits<typename InVec::size_type>::max()`
      if `v` has zero elements.

  * [2.2]{.pnum} Otherwise, the index (in the domain of `v`) of the first element of `v` having largest absolute value, if `InVec::value_type` is an arithmetic type.

  * [2.3]{.pnum} Otherwise, the index (in the domain of `v`) of the first element `v_e` of `v` for which _`abs-if-needed`_ `(` _`real-if-needed`_ `(v_e)) + ` _`abs-if-needed`_ `(` _`imag-if-needed`_ `(v_e))` has the largest value.
...
[1]{.pnum} *Mandates:*
`init + ` _`abs-if-needed`_ `(declval<InMat::value_type>()) * ` _`abs-if-needed`_ `(declval<InMat::value_type>())`
is convertible to `T`.
...
`decltype(` _`abs-if-needed`_ `(declval<InMat::value_type>()) * ` _`abs-if-needed`_ `(declval<InMat::value_type>()))`,
Then, the one-parameter overload is equivalent to
`return matrix_frob_norm(A, T{});`,
and the two-parameter overload is equivalent to
`return matrix_frob_norm(exec, A, T{});`.
...
[1]{.pnum} *Mandates:* _`abs-if-needed`_ `(declval<InMat::value_type>())`
is convertible to `T`.
...
[3]{.pnum} *Remarks:* If `InMat::value_type`
...
[1]{.pnum} *Effects:* Let `T` be
`decltype(` _`abs-if-needed`_ `(declval<InMat::value_type>())`.
Then, the one-parameter overload is equivalent to
`return matrix_one_norm(A, T{});`,
and the two-parameter overload is equivalent to
`return matrix_one_norm(exec, A, T{});`.
...
and the two-parameter overload is equivalent to
`return matrix_inf_norm(exec, A, T{});`.

### BLAS 2 functions [linalg.algs.blas2]

#### General matrix-vector product [linalg.algs.blas2.gemv]

##### Common [linalg.algs.blas2.gemv.common]

<i>[Note:</i> These functions correspond to the BLAS function
`xGEMV`. <i>-- end note]</i>

[1]{.pnum} The following elements apply to all functions in this section.

[3]{.pnum} *Mandates:*

  * [3.1]{.pnum} _`possibly-multiplyable`_`<decltype(A), decltype(x), decltype(y)>()` is `true`, and

  * [3.2]{.pnum} _`possibly-addable`_`<decltype(x),decltype(y),decltype(z)>()` is `true`
                 for those overloads that take a `z` parameter.

[3]{.pnum} *Preconditions:*

  * [3.1]{.pnum} _`multiplyable`_`(A,x,y)` is `true`, and

  * [3.2]{.pnum} _`addable`_`(x,y,z)` is `true` for those overloads that take a `z` parameter.


[4]{.pnum} *Complexity:*
All algorithms in [linalg.algs.blas2.gemv] with `mdspan` parameters
perform a count of `mdspan` array accesses
and arithmetic operations that is linear in
`x.extent(0)` times `A.extent(1)`.
...
[2]{.pnum} *Remarks:* `y` may alias `z`.

#### Symmetric matrix-vector product [linalg.algs.blas2.symv]

##### Common [linalg.algs.blas2.symv.common]
...
[2]{.pnum} *Preconditions:*
  ...
  * [2.4]{.pnum} `y.extent(0)` equals `z.extent(0)`
      (for those overloads that take a `z` parameter).
...
[5]{.pnum} *Complexity:*
All algorithms in [linalg.algs.blas2.symv] with `mdspan` parameters
perform a count of `mdspan` array accesses
and arithmetic operations that is linear in
`x.extent(0)` times `A.extent(1)`.
...
[2]{.pnum} *Remarks:* `y` and `z` may alias.

#### Hermitian matrix-vector product [linalg.algs.blas2.hemv]

##### Common [linalg.algs.blas2.hemv.common]
...
[5]{.pnum} *Complexity:*
All algorithms in [linalg.algs.blas2.hemv] with `mdspan` parameters
perform a count of `mdspan` array accesses
and arithmetic operations that is linear in
`x.extent(0)` times `A.extent(1)`.
...
##### Common [linalg.algs.blas2.trmv.common]
...
[2]{.pnum} *Preconditions:*
  ...
  * [2.3]{.pnum} `A.extent(1)` equals `x.extent(0)`
...
[5]{.pnum} *Complexity:*
For all algorithms in [linalg.algs.blas2.trmv] with `mdspan` parameters,

  * [5.1]{.pnum} those overloads
      that take an `x` parameter
      perform a count of `mdspan` array accesses
      and arithmetic operations that is linear in
      `x.extent(0)` times `A.extent(1)`,

  * [5.2]{.pnum} and those overloads
      that do not take an `x` parameter
      perform a count of `mdspan` array accesses
      and arithmetic operations that is linear in
      `y.extent(0)` times `A.extent(1)`.
...
##### Common [linalg.algs.blas2.trsv.common]
...
[5]{.pnum} *Complexity:*
For all algorithms in [linalg.algs.blas2.trsv] with `mdspan` parameters,
...
[4]{.pnum} *Complexity:*
All algorithms in [linalg.algs.blas2.rank1] with `mdspan` parameters
perform a count of `mdspan` array accesses
and arithmetic operations that is linear in
...
[5]{.pnum} *Complexity:*
All algorithms in [linalg.algs.blas2.rank1.syr] with `mdspan` parameters
perform a count of `mdspan` array accesses
and arithmetic operations that is linear in
...
[5]{.pnum} *Complexity:*
All algorithms in [linalg.algs.blas2.rank1.her] with `mdspan` parameters
perform a count of `mdspan` array accesses
and arithmetic operations that is linear in
`x.extent(0)` times `x.extent(0)`.

#### Rank-2 matrix updates [linalg.algs.blas2.rank2]
...
[5]{.pnum} *Complexity:*
All algorithms in [linalg.algs.blas2.rank2.syr2] with `mdspan` parameters
perform a count of `mdspan` array accesses
and arithmetic operations that is linear in
`x.extent(0)` times `y.extent(0)`.
...
[5]{.pnum} *Complexity:*
All algorithms in [linalg.algs.blas2.rank2.her2] with `mdspan` parameters
perform a count of `mdspan` array accesses
and arithmetic operations that is linear in
`x.extent(0)` times `y.extent(0)`.
...
##### Common [linalg.algs.blas3.gemm.common]
...
[2]{.pnum} *Preconditions:*
  ...
  * [2.1]{.pnum} `C.extent(0)` equals `E.extent(0)`
      (for those overloads that take an `E` parameter),

  * [2.2]{.pnum} `C.extent(1)` equals `E.extent(1)`
      (for those overloads that take an `E` parameter),
...
[4]{.pnum} *Complexity:*
All algorithms in [linalg.algs.blas3.gemm] with `mdspan` parameters
perform a count of `mdspan` array accesses
and arithmetic operations that is linear in
...
[2]{.pnum} *Remarks:* `C` and `E` may alias.

#### Symmetric matrix-matrix product [linalg.algs.blas3.symm]

##### Common [linalg.algs.blas3.symm.common]
...
these functions interpret the input matrix --
not the output matrix -- as symmetric.
-->
...
[2]{.pnum} *Preconditions:*
  ...
  * [2.2]{.pnum} `C.extent(0)` equals `E.extent(0)` (for those overloads that take an `E` parameter), and

  * [2.3]{.pnum} `C.extent(1)` equals `E.extent(1)` (for those overloads that take an `E` parameter).
...
[4]{.pnum} *Mandates:*
  ...
  * [4.2]{.pnum} For all `r` in 0, 1, ..., `C.rank()` - 1,
      if neither `C.static_extent(r)` nor `E.static_extent(r)`
      equals `dynamic_extent`, then `C.static_extent(r)` equals
      `E.static_extent(r)` (for those overloads that take an `E` parameter).

[5]{.pnum} *Remarks:*
`C` and `E` (for those overloads that take an `E` parameter)
may alias.

[6]{.pnum} The following elements apply to all the functions in
[linalg.algs.blas3.symm.ov-left] and
[linalg.algs.blas3.symm.up-left].
...
[9]{.pnum} *Complexity:*
All algorithms in [linalg.algs.blas3.symm] with `mdspan` parameters
perform a count of `mdspan` array accesses
and arithmetic operations that is linear in
`A.extent(0)` times `B.extent(1)` times `A.extent(1)`.

[10]{.pnum} The following elements apply to all the functions in
[linalg.algs.blas3.symm.ov-right] and
[linalg.algs.blas3.symm.up-right].
...
##### Common [linalg.algs.blas3.hemm.common]
...
these functions interpret the input matrix --
not the output matrix -- as Hermitian.
-->
...
[6]{.pnum} The following elements apply to all the functions in
[linalg.algs.blas3.hemm.ov-left] and
[linalg.algs.blas3.hemm.up-left].
...
[9]{.pnum} *Complexity:*
All algorithms in [linalg.algs.blas3.hemm.ov-left] and [linalg.algs.blas3.hemm.ov-right] with `mdspan` parameters
perform a count of `mdspan` array accesses
and arithmetic operations that is linear in
`A.extent(0)` times `B.extent(1)` times `A.extent(1)`.

[10]{.pnum} The following elements apply to all the functions in
[linalg.algs.blas3.hemm.ov-right] and
[linalg.algs.blas3.hemm.up-right].
...
[13]{.pnum} *Complexity:*
All algorithms in [linalg.algs.blas3.hemm.ov-right] and
[linalg.algs.blas3.hemm.up-right] with `mdspan` parameters
perform a count of `mdspan` array accesses
and arithmetic operations that is linear in
`B.extent(0)` times `A.extent(1)` times `B.extent(1)`.
...
##### Common [linalg.algs.blas3.trmm.common]
...
[6]{.pnum} The following elements apply to all the functions in
[linalg.algs.blas3.trmm.ov-left] and
[linalg.algs.blas3.trmm.up-left].

[7]{.pnum} *Preconditions:*

  * [7.1]{.pnum} `A.extent(1)` equals `B.extent(0)` (for those overloads that take a `B` parameter),

  * [7.2]{.pnum} `A.extent(0)` equals `C.extent(0)`, and

  * [7.3]{.pnum} `B.extent(1)` equals `C.extent(1)` (for those overloads that take a `B` parameter).
...
[8]{.pnum} *Mandates:*
  ...
  * [8.1]{.pnum} if neither `A.static_extent(1)`
      nor `B.static_extent(0)` equals `dynamic_extent`,
      then `A.static_extent(1)` equals `B.static_extent(0)`
      (for those overloads that take a `B` parameter);
...
[9]{.pnum} *Complexity:*
For all algorithms in [linalg.algs.blas3.trmm.ov-left] and
[linalg.algs.blas3.trmm.up-left] with `mdspan` parameters,

  * [9.1]{.pnum} those overloads
      that take a `B` parameter
      perform a count of `mdspan` array accesses
      and arithmetic operations that is linear in
      `A.extent(0)` times `B.extent(1)` times `A.extent(1)`,

  * [9.2]{.pnum} and those overloads
      that do not take a `B` parameter
      perform a count of `mdspan` array accesses
      and arithmetic operations that is linear in
      `A.extent(0)` times `C.extent(1)` times `A.extent(1)`.

[10]{.pnum} The following elements apply to all the functions in
[linalg.algs.blas3.trmm.ov-right] and
[linalg.algs.blas3.trmm.up-right].

[11]{.pnum} *Preconditions:*

  * [11.1]{.pnum} `B.extent(1)` equals `A.extent(0)` (for those overloads that take a `B` parameter),

  * [11.2]{.pnum} `B.extent(0)` equals `C.extent(0)` (for those overloads that take a `B` parameter), and
...
[12]{.pnum} *Mandates:*
  ...
  * [12.2]{.pnum} if neither `B.static_extent(0)`
      ...
      (for those overloads that take a `B` parameter); and
...
[13]{.pnum} *Complexity:*
For all algorithms in [linalg.algs.blas3.trmm.ov-right] and
[linalg.algs.blas3.trmm.up-right] with `mdspan` parameters,

  * [13.1]{.pnum} those overloads
      that take a `B` parameter
      perform a count of `mdspan` array accesses
      and arithmetic operations that is linear in
      `B.extent(0)` times `A.extent(1)` times `B.extent(1)`,

  * [13.1]{.pnum} and those overloads
      that do not take a `B` parameter
      perform a count of `mdspan` array accesses
      and arithmetic operations that is linear in
      `C.extent(0)` times `A.extent(1)` times `C.extent(1)`.
...
##### Complexity [linalg.alg.blas3.rank-k.complexity]

<!--
Users can achieve the effect
...
[1]{.pnum} *Complexity:*
All algorithms in [linalg.alg.blas3.rank-k] with `mdspan` parameters
perform a count of `mdspan` array accesses
and arithmetic operations that is linear in
`A.extent(0)` times `A.extent(1)` times `A.extent(0)`.
...
[4]{.pnum} *Effects:*
Overloads that take an `alpha` parameter
compute $C = C + \alpha A A^T$,
where the scalar $\alpha$ is `alpha`.
Overloads that do not take an `alpha` parameter
compute $C = C + A A^T$.
...
##### Complexity [linalg.alg.blas3.rank2k.complexity]
<!--
Users can achieve the effect
of the `TRANS` argument of these BLAS functions,
by applying `transposed` or `conjugate_transposed`
...
[1]{.pnum} *Complexity:*
All algorithms in [linalg.alg.blas3.rank2k] with `mdspan` parameters
perform a count of `mdspan` array accesses
and arithmetic operations that is linear in
...
##### Common [linalg.alg.blas3.trsm.common]
...
[2]{.pnum} *Preconditions:*
  ...
  * [2.1]{.pnum} For all `r` in 0, 1, ..., `B.rank()` - 1,
      `X.extent(r)` equals `B.extent(r)` (for those overloads that take an `X` parameter); and
...
[3]{.pnum} *Constraints:*
  ...
  * [3.1]{.pnum} if `r,j` is in the domain of `X` and `B`,
      then the expression
      `X[r,j] = B[r,j]` is well formed (for those overloads that take an `X` parameter); and
      <!-- FIXME (mfh 2023/02/17) this appears left over from R10 working -->
...
[4]{.pnum} *Mandates:*
  ...
  * [4.1]{.pnum} For all `r` in 0, 1, ..., `X.rank()` - 1, if neither
      ...
      `dynamic_extent`, then `X.static_extent(r)` equals
      `B.static_extent(r)` (for those overloads that take an `X` parameter).
...
[1]{.pnum} *Complexity:*
All algorithms in [linalg.alg.blas3.trsm.left] with `mdspan` parameters
perform a count of `mdspan` array accesses
and arithmetic operations that is linear in
`A.extent(0)` times `A.extent(1)` times `B.extent(1)`.
...
[3]{.pnum} *Preconditions:* `A.extent(1)` equals `B.extent(0)`.
...
[6]{.pnum} In-place left solve of multiple triangular systems
...
[7]{.pnum} *Preconditions:* `A.extent(1)` equals `B.extent(0)`.
...
[1]{.pnum} *Complexity:*
All algorithms in [linalg.alg.blas3.trsm.right] with `mdspan` parameters
perform a count of `mdspan` array accesses
and arithmetic operations that is linear in
...
[3]{.pnum} *Preconditions:* `A.extent(1)` equals `B.extent(1)`.
...
[6]{.pnum} In-place right solve of multiple triangular systems
...
[7]{.pnum} *Preconditions:* `A.extent(1)` equals `B.extent(1)`.
