...
`OutputExtents` such that
  ...
  * [2.1]{.pnum} `InputExtents::static_extent(1)`
      equals
      `OutputExtents::static_extent(0)` and

  * [2.2]{.pnum} `InputExtents::static_extent(0)`
      equals
      `OutputExtents::static_extent(1)`.

[3]{.pnum} *Mandates:*

  * [3.1]{.pnum} `InputExtents` is a specialization of `extents`, and

  * [3.2]{.pnum} `InputExtents::rank()` equals 2.
...
[5]{.pnum} *Returns:* An `extents` object `out` such that

  * [5.1]{.pnum} `out.extent(1)` equals
      `in.extent(0)`, and

  * [5.2]{.pnum} `out.extent(0)` equals
      `in.extent(1)`.
...
public:
  ...
  public:
    ...
    constexpr size_type required_span_size() const
      noexcept(noexcept(@_nested-mapping__@.required_span_size()))
      { return @_nested-mapping__@.required_span_size(); }

    template<class Index0, class Index1>
      constexpr size_type operator()(Index0 ind0, Index1 ind1) const
        noexcept(noexcept(@_nested-mapping__@(ind1, ind0)))
      { return @_nested-mapping__@(ind1, ind0); }
    ...
    constexpr bool is_unique() const
      noexcept(noexcept(@_nested-mapping__@.is_unique()))
      { return @_nested-mapping__@.is_unique(); }
    constexpr bool is_exhaustive() const
      noexcept(noexcept(@_nested-mapping__@.is_exhaustive()))
      { return @_nested-mapping__@.is_exhaustive(); }
    constexpr bool is_strided() const
      noexcept(noexcept(@_nested-mapping__@.is_strided()))
      { return @_nested-mapping__@.is_strided(); }

    constexpr index_type stride(size_t r) const
      noexcept(noexcept(@_nested-mapping__@.stride(r)));

    template<class OtherExtents>
      friend constexpr bool
        operator==(const mapping& x, const mapping<OtherExtents>& y) noexcept(noexcept(x == y));
  };

  private:
    @_nested-mapping-type_@ @_nested-mapping__@; // @_exposition only_@
    extents_type @_extents__@;
};
```
...
[8]{.pnum} *Mandates:*
  ...
  * [8.1]{.pnum} `Extents` is a specialization of `extents`, and

  * [8.2]{.pnum} `Extents::rank()` equals 2.
...
[9]{.pnum} *Effects:*
  ...
  * [9.1]{.pnum} Initializes _`nested-mapping_`_ with `map`, and

  * [9.1]{.pnum} initializes _`extents_`_ with _`transpose-extents`_ `(map.extents())`.
...
[10]{.pnum} *Constraints:* `is_always_strided()` is `true`.

[11]{.pnum} *Returns:* _`nested-mapping_`_ `.stride(r == 0 ? 1 : 0)`
...
[12]{.pnum} *Constraints:*
  ...
  * [12.1]{.pnum} _`nested-mapping-type`_ and
    `decltype(y.` _`nested-mapping_`_ `)`
    are equality comparable, and

  * [12.2]{.pnum} `x.rank()` equals `y.rank()`.
...
where `ReturnLayout` is:
  ...
  * [2.6]{.pnum} Otherwise, `layout_transpose<Layout>`.
