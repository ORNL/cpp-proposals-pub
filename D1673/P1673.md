...
public:
  ...
  constexpr typename offset_policy::data_handle_type
    offset(data_handle_type p, size_t i) const
      noexcept(noexcept(@_nested-accessor\__@.offset(p, i)));

  constexpr const Accessor& nested_accessor() const noexcept { return @_nested-accessor\__@; }

private:
  NestedAccessor @_nested-accessor\__@{}; // exposition only
};
```
...
```c++
constexpr reference access(data_handle_type p, size_t i) const
  noexcept(noexcept(reference(@_nested-accessor\__@.access(p, i))));
```
...
[7]{.pnum} *Remarks:* The exception specification is equivalent to:
```c++
noexcept(@_conj-if-needed_@(NestedAccessor::element_type(@_nested-accessor\__@.access(p, i)))
```
