...
## P1684R5: 2023-05 Mailing

- discuss constructor overload set options
- remove constructors taking container and integral packs
- swap constructor arguments, extents/mapping should come before container
...
class mdarray {
  ...
  // also in mdspan as data_handle_type:
  using pointer = /* ... */;
  // only in mdarray:
  using const_pointer = /* ... */;
...
# Constructor Overload Set Considerations

As discussed before, there are numerous options of how to design the constructor overlaod set for `mdarray`.
Compared to other containers and container adaptors the situation is more complicated, due to the combinatorical nature of having the layout mapping arguments with the common set of container arguments (values, allocators, ranges etc.).

A prior version of this paper had well over 40 constructor overloads.
The current version still has on the order of 20, but we believe that it is the best compromise between complexity and usability.
We put the emphasis on easy usability in most common cases, while preserving clarity of behavior.
...
## Current Argument combinations

This set is almost the same as in the previous revision of the paper, however we removed an overload taking a container and integrals, which allowed us to change the order for constructors taking a container, so that the extents or mapping argument is always first.
...
- extents + allocator
- mapping + allocator
- extents + value + allocator
- mapping + value + allocator
- extents + container + allocator
- mapping + container + allocator

Note that this set does not include a constructor from iterators or ranges these can be written by passing in an immediate constructed container:

```c++
mdarray<...> a(extents{...}, vector{begin, end});
```

## Minimal Alternative

A question which came up is whether, it would be sufficient to always wrap things in a container and pass that in, for example for the use case of providing an allocator, or constructing an `mdarray` with a value to fill.
This would reduce the acceptable argument set to this:
...
However, it would make some common use cases very verbose to write when using values or allocators, due to the need to compute the size of the container. Constructing with a fill value would become this:
...
Yet another considered alternative would rely on argument forwarding by providing a constructor with variadic templates:

```c++
template<class ... Args>
mdarray(extents_type, Args&& ...);
```

This would reduce the overload set (other than conversion and from `mdspan`) to this:

- Integrals
- extents
- mapping
- extents + container-ctor-args
- mapping + container-ctor-args

However, using this approach results in some awkward and hard to understand situations.
To begin with, the constructor actually has two situations.
...
The constructor needs to check whether the container is constructible from a size and the provided arguments.
Besides the value case this is important for the usecase of providing an allocator.

However, that leads to an unfortunate situation depending on the container type and its scalar type.

```c++
mdarray<float, dextents<int,2>> a(extents{10, 10}, 200);
```

creates a rank-2 mdarray where every value will be set to 200.

On the other hand:
```c++
mdarray<pair<int, int>, dextents<int,2>> a(extents{10, 10}, 200);
```

creates a rank-2 mdarray with an underlying container of size 200, and the values are default initialized.
If one would to use 99 instead of 200, this second constructor would have a precondition violation, because the constructor arguments result in container of too small size.

We believe that this situation is not tenable, and should be avoided.
...
We considered various strategies to deal with move behavior of `mdarray` in a way that the invariants are guaranteed, or at a minimum no invalid operation can happen without a user checkable precondition.

There are three primary options:

- add preconditions to relevant operations which will check that the containers size is still large enough
- don't provide any move semantics for `mdarray`
- add a bool member which tracks whether `mdarray` was moved from

We believe that the first option is preferrable at this point, since it introduces the least new concepts.
Move semantics for `mdarray` - in particular of `mdarray` with `array` container type and static extents - are very valuable.
Adding an extra member likely also makes the small static-size `mdarray` case produce less optimal code.

We did consider requiring emptying out the container in conjunction with option 1, but believe that is not necessary and adds undesirable overhead for custom container use cases.

In order to make the new precondition testable by users, we introduce an observer of the underlying container's size.
...
public:
  ...
  template<class Alloc>
    constexpr mdarray(const extents_type& ext, const container_type& c, const Alloc& a);
  template<class Alloc>
    constexpr mdarray(const mapping_type& m, const container_type& c, const Alloc& a);

  template<class Alloc>
    constexpr mdarray(const extents_type& ext, container_type&& c, const Alloc& a);
  template<class Alloc>
  ...
  constexpr auto container_size() const { return ctr_.size(); }

  friend constexpr void swap(mdarray& x, mdarray& y) noexcept;

  constexpr const extents_type& extents() const { return @_map\__@.extents(); }
  constexpr pointer container_data(); { return to_address(@_ctr\__@.begin()); }
  constexpr const_pointer container_data() const { return to_address(@_ctr\__@.cbegin()); }
  ...
  container_type&& extract_container() && { return std::move(@_ctr\__@); }
...
template<class IndexType, size_t ... Extents, class Container>
...
mdarray(const mdspan<ElementType, Extents, Layout, Accessor>&)
  -> mdarray<remove_cv_t<ElementType>, Extents, Layout>;

template<class IndexType, size_t ... Extents, class Container, class Alloc>
...
[14]{.pnum} *Constraints:* `is_constructible_v<mapping_type, const extents_type&>` is `true`, and

[15]{.pnum} *Preconditions:* `c.size() >= mapping_type(ext).required_span_size()` is `true`.

[16]{.pnum} *Effects:*

   * [16.1]{.pnum} Direct-non-list-initializes _`map_`_ with `ext`, and

   * [16.2]{.pnum} Direct-non-list-initializes `ctr_` with `c`.
...
[21]{.pnum} *Effects:*
   ...
   * [21.2]{.pnum} Direct-non-list-initializes `ctr_` with `std::move(c)`.
...
[23]{.pnum} *Effects:*
   ...
   * [23.2]{.pnum} Direct-non-list-initializes `ctr_` with `std::move(c)`.
...
[25]{.pnum} *Constraints:* 
   ...
   * [25.2]{.pnum} `is_constructible_v<container_type, OtherContainer>` is `true`.

[26]{.pnum} *Preconditions:* For each rank index `r` of `extents_type`, `static_extent(r) == dynamic_extent || static_extent(r) == other.extent(r)` is `true`.

[27]{.pnum} *Effects:*

   * [27.1]{.pnum} Direct-non-list-initializes `ctr_` with `other.ctr_`, and

   * [27.2]{.pnum} Direct-non-list-initializes _`map_`_ with `other.`_`map_`_.


[28]{.pnum} *Remarks:* The expression inside `explicit` is:
```c++
...
[29]{.pnum} *Mandates:* `is_constructible_v<extents_type, OtherExtents>` is `true`.

[30]{.pnum} *Constraints:*

   * [30.1]{.pnum} `is_constructible_v<value_type, Accessor::reference>` is `true`,
...
[31]{.pnum} *Preconditions:*
   ...
   * [31.1]{.pnum} For each rank index `r` of `extents_type`, `static_extent(r) == dynamic_extent || static_extent(r) == other.extent(r)` is `true`, and

   * [31.2]{.pnum} if `container_type` is a specialization of `array`, then `container_type().size() >= other.mapping().required_span_size()`.

[32]{.pnum} *Effects:*

   * [32.1]{.pnum} Direct-non-list-initializes _`map_`_ with `other.mapping()`;

   * [32.2]{.pnum} If `is_constructible_v<container_type, size_t>` is `true`, direct-non-list-initializes `ctr_` with `container_type(other.mapping().required_span_size())`, otherwise default constructs _`ctr_`_; and

   * [32.3]{.pnum} For all unique multidimensional indices `i...` in `other.extents()`, assigns `other[i...]` to `ctr_[`_`map_`_`(i...)]`.
...
[13]{.pnum} *Returns:* `mdspan(container_data(),`_`map_`_)`
...
[16]{.pnum} *Returns:* `mdspan<element_type, extents_type, layout_type, OtherAccessorType>(container_data(), `_`map_`_`, a)`
...
[19]{.pnum} *Returns:* `mdspan<const element_type, extents_type, layout_type, OtherAccessorType>(container_data(), `_`map_`_`, a)`
