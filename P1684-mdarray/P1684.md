...
but we don't want full sequence container requirements (also there is no spelled out requirements for a constructor which takes a integral only).

These requirements probably need to go into the general container requirements, or be a new named requirement?
Maybe there is also another way to require these semantics for containers used with `mdarray`?
...
But in that case it is not clear why having unused elements at the end should be different from having unused elements somewhere in the range.

# Wording

<b> Size constructible container  [container.requirements.sizeconstr]</b>

[1]{.pnum} A *size constructible container* is a container which if it is constructible from an integral argument, an `initializer_list`, or a range, and an optional allocator, will have a size after construction is equal to the integral argument, the size of the `initializer_list` or the size of the range.
...
[2]{.pnum} A type `C` meets the size constructible container requirements if
  ...
  * `C` meets the contiguous container requirements, and 

  * for those containers for which the following types and expressions are well-formed, the expressions have the specified semantics
...
[9]{.pnum} *Postcondition:* `C(from_range, r, a).size() == ranges::distance(r)` is `true`
...
public:
  ...
  template<class... OtherSizeTypes>
    explicit constexpr mdarray(OtherSizeTypes... exts);
  ...
  template<class... OtherSizeTypes>
    explicit constexpr mdarray(const container_type& c, OtherSizeTypes... exts);
  ...
  template<class... OtherSizeTypes>
    explicit constexpr mdarray(container_type&& c, OtherSizeTypes... exts);
  constexpr mdarray(container_type&& c, const extents_type& ext);
  constexpr mdarray(container_type&& c, const mapping_type& m);

  template<class... OtherSizeTypes>
    explicit constexpr mdarray(initializer_list<element_type> init, OtherSizeTypes... exts);
  ...
  template<@_container-compatible-range_@<element_type> R, class... OtherSizeTypes>
    explicit constexpr mdarray(from_range_t, R&& rg, OtherSizeTypes... exts);
  ...
  template<class OtherSizeType, size_t N>
    constexpr reference operator[](span<OtherSizeType, N> indices);
  template<class OtherSizeType, size_t N>
    constexpr reference operator[](const array<OtherSizeType, N>& indices);
  template<class... OtherSizeTypes>
    constexpr const_reference operator[](OtherSizeTypes... indices) const;
  template<class OtherSizeType, size_t N>
    constexpr const_reference operator[](span<OtherSizeType, N> indices) const;
  template<class OtherSizeType, size_t N>
    constexpr const_reference operator[](const array<OtherSizeType, N>& indices) const;
...
[4]{.pnum} `Container` shall meet the requirements of size constructible container.
...
[1]{.pnum} *Constraints:*
   ...
   * [1.2]{.pnum} `(is_nothrow_constructible_v<size_type, OtherSizeTypes> && ...)` is `true`,

   * [1.3]{.pnum} `is_constructible_v<extents_type, OtherSizeTypes...>` is `true`,
...
[3]{.pnum} *Preconditions:* If `container_type` is a specialization of `array`, `mapping_type(extents_type(static_cast<size_type>(std::move(exts)...))).required_span_size() == size(container_type())` is `true`.

[2]{.pnum} *Effects:*

   * [2.1]{.pnum} Direct-non-list-initializes _`map_`_ with `extents_type(static_cast<size_type>(std::move(exts))...)`, and
...
[7]{.pnum} *Constraints:*
   ...
   * [7.1]{.pnum} `(is_convertible_v<OtherSizeTypes, size_type> && ...)` is `true`,

   * [7.2]{.pnum} `(is_nothrow_constructible_v<size_type, OtherSizeTypes> && ...)` is `true`,

   * [7.3]{.pnum} `is_constructible_v<extents_type, OtherSizeTypes...>` is `true`,
...
[15]{.pnum} *Constraints:*
   ...
   * [15.1]{.pnum} `(is_convertible_v<OtherSizeTypes, size_type> && ...)` is `true`,

   * [15.2]{.pnum} `(is_nothrow_constructible_v<size_type, OtherSizeTypes> && ...)` is `true`,

   * [15.3]{.pnum} `is_constructible_v<extents_type, static_cast<size_type>(OtherSizeTypes)...>` is `true`,
...
```c++
template<class... OtherSizeTypes>
  explicit constexpr mdarray(std::initializer_list<element_type> init, OtherSizeTypes... exts);
```

[22]{.pnum} *Constraints:*

   * [22.1]{.pnum} `(is_convertible_v<OtherSizeTypes, size_type> && ...)` is `true`,

   * [22.2]{.pnum} `(is_nothrow_constructible_v<size_type, OtherSizeTypes> && ...)` is `true`,

   * [22.3]{.pnum} `is_constructible_v<extents_type, static_cast<size_type>(OtherSizeTypes)...>` is `true`,
...
[23]{.pnum} *Preconditions:* `init.size() >= mapping_type(extents_type(static_cast<size_type>(std::move(exts))...)).required_span_size()` is `true`.

[24]{.pnum} *Effects:*

   * [24.1]{.pnum} Direct-non-list-initializes _`map_`_ with `extents_type(static_cast<size_type>(std::move(exts))...)`, and
...
[31]{.pnum} *Constraints:*
   ...
   * [31.1]{.pnum} `(is_convertible_v<OtherSizeTypes, size_type> && ...)` is `true`,

   * [31.2]{.pnum} `(is_nothrow_constructible_v<size_type, OtherSizeTypes> && ...)` is `true`,

   * [32.3]{.pnum} `is_constructible_v<extents_type, static_cast<size_type>(OtherSizeTypes)...>` is `true`,
...
[35]{.pnum} *Preconditions:* `ranges::distance(rg) >= mapping_type(extents_type(static_cast<size_type>(std::move(exts))...)).required_span_size()` is `true`.

[36]{.pnum} *Effects:*

   * [36.1]{.pnum} Direct-non-list-initializes _`map_`_ with `extents_type(static_cast<size_type>(std::move(exts))...)`, and
...
template<class OtherSizeType>
  constexpr reference operator[](span<OtherSizeType, rank()> indices);
template<class OtherSizeType>
  constexpr const_reference operator[](span<OtherSizeType, rank()> indices) const;
template<class OtherSizeType>
  constexpr reference operator[](const array<OtherSizeType, rank()>& indices);
template<class OtherSizeType>
  constexpr const_reference operator[](const array<OtherSizeType, rank()>& indices) const;
```
...
```c++
  template<class OtherAccessorType>
    constexpr mdspan<const element_type, extents_type, layout_type, OtherAccessorType>
      view(const OtherAccessorType& a = default_accessor<const element_type>()) const;
```
