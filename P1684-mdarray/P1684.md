...
LEWG's review of P1684R4 asked us to consider adding a way
to "extract" the container from an `mdarray`.
This does *not* mean just getting a reference to the container.
R1 of the proposal included both const and nonconst overloads
of a member function `container()` returning
`const container_type&` resp. `container_type&`.
We removed this in R2, because "it's a bad idea
to allow someone to resize the container owned by an `mdarray`."

What LEWG meant by "extraction" was a way to reuse
an `mdarray`'s existing storage and reinterpret it,
for example with different extents or layout.
In the example below, functions `f0` and `f1`
interpret the storage represented by a `std::vector<float>`
in different ways.  Neither of the two functions allocate,
and each passes along the `std::vector<float>` to the next operation.
The hypothetical `extract_container()` member function
of `mdarray` would move the container out of the `mdarray`
and return the moved-from container as `mdarray::container_type&&`.
...
1. They want to reuse the container's *storage*,
    but not the *elements*.

2. They want to reuse both *storage* and *elements*.
...
Users may want to reuse the container's *storage*,
but not the *elements*.
...
subclass that wraps a fixed-size contiguous buffer.
The following shows how that might change
the above `f0`, `f1` example.

```c++
class custom_memory_resource : public std::pmr::memory_resource {
public:
  // Use the given span of char as a fixed-size memory pool.
  // do_allocate needs to bless allocations with start_lifetime_as_array.
  template<size_t Extent>
  custom_memory_resource(std::span<char, Extent> buffer);
...
pmr::memory_resource*
f0(pmr::memory_resource* resource)
{
  pmr::polymorphic_allocator<float> allocator{resource};
  mdarray m0{pmr::vector<float>{24, allocator}, 2, 3, 4};
  // ... compute with m0 ...
  return resource;
}

pmr::memory_resource*
f1(pmr::memory_resource* resource)
{
  pmr::polymorphic_allocator<float> allocator{resource};
  mdarray m1{pmr::vector<float>{24, allocator}, 6, 4};
  // ... compute with m1 ...
  return resource;
}
...
int main()
{
  vector<float> storage{24, 0.0f};
  pmr::memory_resource mbr{
    span{storage.data(), sizeof(float) * storage.size()}};
  pmr::memory_resource* mbr_ptr = f1(f0(&mbr));
...
The second point means that users would not
be able to call functions that use a container
with an incompatible allocator type.
This is somewhat mitigated by `mdspan` being
the preferred function parameter type for array views.

That last sentence calls for elaboration.
If users want a function to take a read-only view
of a contiguous one-dimensional array of `T`,
the idiomatic parameter type is `span<const T>`,
not `const vector<T>&`.
If users want a function to take a writeable view
of a contiguous one-dimensional array of `T`,
the idiomatic parameter type is `span<T>`,
not `vector<T>&`.
The only reason to pass a `vector` by nonconst reference
is because the function needs to call member functions
that change its size: e.g., `resize` or `push_back`.
As `span` is to `vector`, so `mdspan` is to `mdarray`.
There is even less reason to pass `mdarray` by nonconst reference,
because `mdarray` has no member functions
that change its extents.  (This is a deliberate choice
that ensures consistent behavior of all `mdarray`s,
whether or not they have any run-time extents.)

Two reasons remain to expose `mdarray` as a function parameter.

1. The caller is transferring ownership of the `mdarray`
    to the function. This implies passing `mdarray`
    by rvalue reference.

2. The `mdarray` is small enough that passing by value
    is inexpensive (e.g., the object can fit in registers).

Otherwise, the correct idiom is to pass an `mdspan`
that views the elements of the mdarray.
...
int main()
{
  // Only one vector allocation, created as a temporary,
  // and moved into used_storage.
  vector<float> used_storage(f1(f0(vector<float>(24, 0.0f))));
  // ... use used_storage for other things ...
  return 0;
}
```

Here are some important points in the above example.

1. `mdspan` views the elements in `storage`.
    It does *not* take ownership of or even store the container.

2. The above example only passes the `vector` along
    in order to look as similar as possible
    to the first `f0`, `f1` example.
    The functions here do not *need* to pass the `vector` along;
    they just need to make sure that the `vector`'s lifetime
    contains the lifetimes of all the `mdspan` that view its elements.

#### "Reshape" reuses storage and elements

Reusing storage and elements
is equivalent to the "reshape" facility
found in other programming languages and libraries.
This facility reinterprets an existing array
as an array with a different (multidimensional) "shape."
...
Thus, the only way to implement a reshape operation
...
but users could no longer rely on
any other properties of the `mdarray`.
It would effectively be in a "valid but unspecified state,"
just like its container.

This means that generic code should treat
an extracted-from `mdarray` like any moved-from object:
the only safe operation is to let it be destroyed.
Given that `mdarray` already has
...
#### Possible states of moved-from containers
...
1. the size could change arbitrarily (or not change at all),
...
#### Associative containers' `extract`, `insert`, and `merge`
...
* The postcondition is that the only safe operation
    the `mdarray` can do is to be destroyed.

The operation is analogous to smashing a piggy bank
...
However, if the container is a `std::array`
...
If the interface is a member function,
it must have return type `container_type&&` and be nonconst.
Making it rvalue reference qualified
might incorrectly suggest that the layout mapping were moved-from.
The layout mappings that the Standard provides
do not even have move constructors.
