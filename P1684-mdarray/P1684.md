...
public:
  ...
  constexpr pointer data() { return to_address(ctr\_.begin()); }
  constexpr const_pointer data() const { return to_address(ctr\_.begin()); }
...
[6]{.pnum} `Container` shall meet the requirements of ContiguousContainer.
           If `is_same_v<typename Container::value_type, ElementType>` equals `false`, then the program is ill-formed.
           If `is_same_v<decltype(to_address(declval<typename Container::iterator>())),ElementType*>` equals `false`, then the program is ill-formed.
