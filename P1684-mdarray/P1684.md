...
<b>22.7.� Class template `mdarray` [mdarray]</b>

<br/>
<b>22.7.�.1 `mdarray` overview [mdarray.overview]</b>
...
public:
  ...
  template<class... SizeTypes>
    explicit constexpr mdarray(initializer_list<T> init, SizeTypes... exts);
...
[6]{.pnum} The default type for `Container` is `vector<ElementType>` if `Extents::dynamic_rank()>0` is `true`. 
           Otherwise, the default type is `array<ElementType,N>`, where `N` is the product of `Extents::static_extent(r)` 
           for `r` in the range of $[$`0, Extents::rank()`$)$.
...
<b>22.7.�.1 `mdarray` Constructors  [mdarray.ctors]</b>
...
[1]{.pnum} *Constraints:*
   ...
   * [1.4]{.pnum} `is_constructible_v<container_type, size_t>` is `true`.

[2]{.pnum} *Effects:*

   * [2.1]{.pnum} Direct-non-list-initializes _`map_`_ with `Extents(static_cast<size_type>(std::move(exts))...)`, and

   * [2.2]{.pnum} Direct-non-list-initializes `ctr_` with `container_type(`_`map_`_`.required_span_size())`.
...
[3]{.pnum} *Constraints:*
   ...
   * [3.2]{.pnum} `is_constructible_v<container_type, size_t>` is `true`.

[4]{.pnum} *Effects:*

   * [4.1]{.pnum} Direct-non-list-initializes _`map_`_ with `ext`, and

   * [4.2]{.pnum} Direct-non-list-initializes `ctr_` with `container_type(`_`map_`_`.required_span_size())`.
...
[5]{.pnum} *Constraints:* `is_constructible_v<container_type, size_t>` is `true`.

[6]{.pnum} *Effects:*

   * [6.1]{.pnum} Direct-non-list-initializes _`map_`_ with `m`, and

   * [6.2]{.pnum} Direct-non-list-initializes `ctr_` with `container_type(`_`map_`_`.required_span_size())`.
...
[43]{.pnum} *Effects:*
   ...
   * [43.2]{.pnum} Direct-non-list-initializes `ctr_` with `std::move(c)` as the first argumnet and `a` as the second argument.
...
<br/>
<b>22.7.�.2 `mdarray` members [mdarray.members]</b>
