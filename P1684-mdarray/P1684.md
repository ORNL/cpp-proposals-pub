...
[7]{.pnum} *Constraints:*
   ...
   * [7.2]{.pnum} `is_constructible_v<extents_type, static_cast<size_type>(OtherSizeTypes)...>` is `true`,
...
[15]{.pnum} *Constraints:*
   ...
   * [15.2]{.pnum} `is_constructible_v<extents_type, static_cast<size_type>(OtherSizeTypes)...>` is `true`,
...
[8]{.pnum} *Preconditions:* `ranges::distance(rg) >= mapping_type(extents_type(static_cast<size_type>(std::move(exts))...)).required_span_size()` is `true`.
...
[11]{.pnum} *Preconditions:* `ranges::distance(rg) >= mapping_type(ext).required_span_size()` is `true`.
...
[13]{.pnum} *Preconditions:* `ranges::distance(rg) >= m.required_span_size()` is `true`.
...
[5]{.pnum} *Constraints:*
   ...
   * [5.3]{.pnum} `sizeof...(OtherSizeTypes) == rank()` is `true`.
...
[11]{.pnum} *Constraints:*
   ...
   * [11.1]{.pnum} `is_convertible_v<const OtherSizeType&, size_type>` is `true`, and
   
   * [11.2]{.pnum} `is_nothrow_constructible_v<size_type, const OtherSizeType&>` is `true`, and
