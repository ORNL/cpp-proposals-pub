...
[14]{.pnum} *Returns:* `mdspan(container_data(), `_`map_`_`, a)`
...
[15]{.pnum} *Constraints:* 
   ...
   * [15.1]{.pnum} `is_assignable_v<mdspan<const element_type, extents_type, layout_type>, mdspan<const element_type, extents_type, layout_type, OtherAccessorType>>` is `true`.

   * [15.2]{.pnum} `is_same_v<const_pointer, const element_type*>` is `true`.

[12]{.pnum} *Preconditions:* `container_size() >= `_`map_`_`.required_span_size()` is `true`.

[16]{.pnum} *Returns:* `mdspan(container_data(), `_`map_`_`, a)`
