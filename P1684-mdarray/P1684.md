...
**This Work:**
```cpp
mdarray<double, dextents<int, 3,3>> matrix(N,M);
```
...
There are numerous options of how to design the constructor overlaod set for `mdarray`.
...
There are two alternatives we considered:
...
This set is almost the same as in the previous revision of the paper, however we changed the order for constructors taking a container, so that the extents or mapping argument is always first.
...
```c++
some_mdarray_t a(extents{...}, vector{begin, end});
```
...
- extents
- mapping
- extents + container_ctor_args
- mapping + container_ctor_args
...
[12]{.pnum} *Returns:* `mdspan(container_data(),`_`map_`_)`
...
[11]{.pnum} *Constraints:* `is_assignable_v<pointer, typename OtherAccessorType::data_handle_type>` is `true`.

[12]{.pnum} *Preconditions:* `container_size() >= `_`map_`_`.required_span_size()` is `true`.

[14]{.pnum} *Returns:* `mdspan<element_type, extents_type, layout_type, OtherAccessorType>(container_data(), `_`map_`_`, a)`
...
[15]{.pnum} *Constraints:* `is_assignable_v<const_pointer, typename OtherAccessorType::data_handle_type>` is `true`.

[12]{.pnum} *Preconditions:* `container_size() >= `_`map_`_`.required_span_size()` is `true`.

[16]{.pnum} *Returns:* `mdspan<const element_type, extents_type, layout_type, OtherAccessorType>(container_data(), `_`map_`_`, a)`
