...
public:
  ...
  // [mdspan.extents.ctors], Constructors and assignment
  ...
  constexpr size_t @_fwd-prod-of-extents_@(rank_type) const noexcept; // exposition only
  constexpr size_t @_rev-prod-of-extents_@(rank_type) const noexcept; // exposition only
...
private:
  ...
  template<class OtherSizeT, size_t... OtherExtents>
  static constexpr bool @_are-valid-extents_@(extents<OtherSizeT, OtherExtents>) noexcept; // @_exposition only_@

  template<class OtherSizeT, size_t N>
  static constexpr bool @_are-valid-extents_@(array<OtherSizeT, N>) noexcept; // @_exposition only_@
...
*Returns*: 
  ...
  - `true` if `((values > 0) && ...)` is `true` and each element of `values` is a representable value of `size_type`, otherwise
           
  - `false`.

```c++
template<class OtherSizeT, size_t... OtherExtents>
static constexpr bool @_are-valid-extents_@(extents<OtherSizeT, OtherExtents> e) noexcept; // @_exposition only_@
```
...
*Returns*: 
  ...
  - `true` if `e.extent(r) > 0` is `true` for all rank index `r` of `e` and
           `e.extent(r)` is a representable value of `size_type` for all rank index `r` of `e`, otherwise
           
  - `false`.

```c++
template<class OtherSizeT, size_t N>
static constexpr bool @_are-valid-extents_@(array<OtherSizeT, N> arr) noexcept; // @_exposition only_@
```
...
*Returns*: 
  ...
  - `true` if for all `r` in the range of $[$`0`,`N`$)$
           `arr[r] > 0` is `true` and `arr[r]` is a representable value of `size_type`, otherwise
...
* Replace section with: 
  ...
  `E::rank() == Rank && E::rank() == E::rank_dynamic()`  is `true` and `typename E::size_type` denotes `SizeT>.

## In subsection 22.7.X.3.1 [mdspna.layout.left.ctors]
...
*Preconditions*: 
  ...
  - `other.required_span_size()` is a representable value of `size_type` ([basic.fundamental]).
...
*Preconditions*: 
  ...
  - If `extents_type::rank() > 0` is `true` then for all $r$ in the range $[$`0, extents_type::rank()`$)$, `other.stride(r)` equals `extents().`_rev-prod-of-extents_`(r+1)`, and
...
## In subsection 22.7.X.1 [mdspan.mdspan.ctors] 
