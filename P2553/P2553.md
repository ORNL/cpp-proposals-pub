...
* In the synopsis add the following public static exposition only member functions:
...
```c++
  ...
  template<class OtherSizeT, size_t N>
  static constexpr bool @_are-valid-extents_@(span<OtherSizeT, N>) noexcept; // @_exposition only_@
```

* Before paragraph 2 insert a new paragraph 2:

*Mandates:* `SizeT` is an integral type other than `bool`.
...
*Returns*: 
  ...
  - `true` if `((values >= 0) && ...)` is `true` and each element of `values` is a representable value of type `size_type`, otherwise
...
*Returns*: 
  ...
  - `true` if `e.extent(r) >= 0` is `true` for all rank index `r` of `e` and
           `e.extent(r)` is a representable value of type `size_type` for all rank index `r` of `e`, otherwise
...
template<class OtherSizeT, size_t N>
static constexpr bool @_are-valid-extents_@(span<OtherSizeT, N> arr) noexcept; // @_exposition only_@
```
...
*Returns*: 
  ...
  - `true` if for all `r` in the range of $[$`0`,`N`$)$
           `arr[r] >= 0` is `true` and `arr[r]` is a representable value of type `size_type`, otherwise
...
* Replace section with: 
  ...
  `E::rank() == Rank && E::rank() == E::rank_dynamic()`  is `true` and `E::size_type` denotes `SizeT`.
