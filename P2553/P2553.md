...
template<class SizeT, class... Integrals>
constexpr bool @_extents-representable-by_@(Integrals... values) noexcept; // @_exposition only_@

template<class SizeT, class OtherSizeT, size_t... OtherExtents>
constexpr bool @_extents-representable-by_@(extents<OtherSizeT, OtherExtents>) noexcept; // @_exposition only_@

template<class SizeT, class OtherSizeT, size_t N>
constexpr bool @_extents-representable-by_@(array<OtherSizeT, N>) noexcept; // @_exposition only_@
}
```
...
```c++
template<class SizeT, size_t ... Extents>
constexpr size_type @_fwd-prod-of-extents_@(extents<SizeT, Extents...> e, size_t i) noexcept; // @_exposition only_@
```
...
```c++
template<class SizeT, size_t ... Extents>
constexpr size_type @_rev-prod-of-extents_@(extents<SizeT, Extents...> e, size_t i) noexcept; // @_exposition only_@
```

* Add new paragraphs after 14:
...
*Returns*: - `true` if `sizeof...(Integrals) == 0` is `true`, otherwise
           - `true` if `((values > 0) && ...)` is `true` and the product of `values` is representable as a `SizeT`, otherwise
           - `false`.

```c++
template<class SizeT, class OtherSizeT, size_t... OtherExtents>
constexpr bool @_extents-representable-by_@(extents<OtherSizeT, OtherExtents> e) noexcept; // @_exposition only_@
```

*Returns*: - `true` if `sizeof...(OtherExtents) == 0` is `true`, otherwise
           - `true` if `e.extent(r) > 0` is `true` for all rank index `r` of `e` and
             the product of `e.extent(r)` for all rank index `r` of `e` is representable as a `SizeT`, otherwise
           - `false`.

```c++
template<class SizeT, class OtherSizeT, size_t N>
constexpr bool @_extents-representable-by_@(array<OtherSizeT, N> arr) noexcept; // @_exposition only_@
```

*Returns*: - `true` if `N == 0` is `true`, otherwise
           - `true` if for all `r` in the range of $[$`0`,`N`$)$
             the product of `arr[r]` is representable as a `SizeT` and `arr[r] > 0` is `true`, otherwise
           - `false`.
...
*Preconditions:*
  ...
  - For each rank index `r` of `*this`, `static_extent(r) == dynamic_extent || static_extent(r) == other.extent(r)` is `true`.

  - _`extents-respresentable-by`_`<size_type>(other)` is `true`.
...
(@_extents-max-size_@<size_type> < @_extents-max-size_@<OtherSizeT>)
```

* Change Paragrpah 7 to:

*Preconditions*:

- If `sizeof...(SizeTypes) != rank_dynamic()` is `true`, `exts_arr[r]` equals $E_r$ for each $r$ for which $E_r$ is a *static extent*.

- _`extents-representable-by`_`<size_type>(exts...)` is `true`.


* Change Paragrpah 10 to:

*Preconditions*:

- If `N > rank_dynamic()` is `true`, `exts[r]` equals $E_r$ for each $r$ for which $E_r$ is a *static extent*.
...
## In subsection 22.7.X.5.1 [mdspan.layout.layout_stride]

* In paragraph 3 add a new sub bullet

- The product of `static_cast<size_t>(e.extent(r) - 1) * stride(r)` for all rank indicies `r` of `e` is representable by `extents_type::size_type`.
